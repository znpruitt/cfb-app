// Operator CLI for the live-scores RECONCILIATION (slow) trigger schedule (PLATFORM-102 slice 4).
//
// // A SECOND QStash schedule against the same unchanged `/api/cron/live-scores`
// route, covering the reconciliation tail — kickoff + 24h — at an hourly cadence
// while the dense schedule covers the game hours at three minutes. One cron
// expression cannot carry both: a single minute-set applies to every hour it
// matches, so the union of the two phases is two rectangles, not one. QStash
// identity is the arbitrary `Upstash-Schedule-Id`, so two ids may target one
// route.
//
...
//
// Usage:
//   tsx scripts/manage-live-scores-slow-schedule.ts [inspect]          # READ-ONLY: read back + verify
//   tsx scripts/manage-live-scores-slow-schedule.ts upsert --apply     # write the schedule
//   tsx scripts/manage-live-scores-slow-schedule.ts pause  --apply     # pause deliveries
//   tsx scripts/manage-live-scores-slow-schedule.ts resume --apply     # resume deliveries
//
...
import { LIVE_SCORES_SLOW_CONTRACT } from './lib/plannerScheduleContracts.ts';
import { createPlannerIntentReader } from './lib/plannerIntentReader.ts';

const CONTRACT = LIVE_SCORES_SLOW_CONTRACT;
