...
test('PLATFORM-076: debug/scores loads conferences + effective aliases via the shared loader', async () => {
  ...
  const conferencesToken: { value: string | null } = { value: null };
  setMockFetch(async (input: URL | string, init?: RequestInit) => {
    const url = typeof input === 'string' ? input : input.toString();
    const req = new URL(url);
    seen.add(req.pathname);
    if (req.pathname === '/api/conferences') {
      // The admin credential must reach the conference sub-request so a cold
      // cache does not 503 and silently degrade to present-day classification.
      conferencesToken.value = new Headers(init?.headers).get('x-admin-token');
  ...
  const res = await GET(
    new Request('http://localhost/api/debug/scores?year=2025', {
  ...
  assert.equal(
    conferencesToken.value,
    'diag-token',
    'the admin credential is forwarded to the conference sub-request'
  );
  assert.match(
    String(aliasUrl),
    /[?&]scope=effective(&|$)/,
