...
test('the uncached compute is not re-exported or aliased under another name', async () => {
  ...
  // ANY second exported binding of the function defeats the scan above: a caller
  // could import the alias without ever naming `computeCanonicalStandingsUncached`,
  // so the scan would return a clean result over a genuinely reachable function.
  // A mutation adding `export const __leak = computeCanonicalStandingsUncached`
  // to the defining file survived the first version of this test, which only
  // looked for the `export { x as y }` form.
  const aliasing = lines.filter(
    (line) =>
      line.includes(SYMBOL) &&
      /^\s*export\s/.test(line) &&
      !/^\s*export function computeCanonicalStandingsUncached\(/.test(line)
  );
  assert.deepEqual(aliasing, [], 'the only export of the symbol is its own declaration');
  ...
  // The control: the declaration itself IS matched by the export filter, so an
  // empty `aliasing` list means no alias rather than a filter that matches
  // nothing.
  assert.ok(
    lines.some((line) => /^\s*export function computeCanonicalStandingsUncached\(/.test(line)),
    'the declaration was found, so the filter is looking at the right lines'
  );
});
