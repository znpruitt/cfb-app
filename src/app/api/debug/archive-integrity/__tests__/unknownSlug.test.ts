...
/**
 * #778 — `archive-integrity` must refuse a `leagueSlug` no league holds, BEFORE it
 * reads the archive.
 *
...
const KNOWN_SLUG = 'integrity-known';
const UNKNOWN_SLUG = 'integrity-absent';
const YEAR = 2024;
...
function request(slug: string, year: number): Request {
  return new Request(
    `http://localhost/api/debug/archive-integrity?leagueSlug=${slug}&year=${year}`
  );
}
...
test('#778: archive-integrity answers 404 league-not-found for a slug no league holds', async () => {
...
test('CONTROL: archive-integrity serves the SAME planted archive once a league holds the slug', async () => {
  await plantRegistry([KNOWN_SLUG]);
  ...
  const body = await res.json();
  assert.equal(body.summary.leagueSlug, KNOWN_SLUG);
  assert.equal(body.summary.year, YEAR);
});
