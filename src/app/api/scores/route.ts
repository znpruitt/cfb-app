 ...
 * instead of the raw week child, so a 90-second browser poll can neither overwrite
 * nor miss an admin score correction that landed only in the aggregate — the
 * served rows match the reconciled view standings use. Cache-only; no provider call.
 */
