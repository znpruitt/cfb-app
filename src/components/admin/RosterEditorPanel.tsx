...
export default function RosterEditorPanel({ slug, year, teams }: Props): React.ReactElement {
  ...
  return (
    <div className="rounded-lg border border-zinc-700 bg-zinc-900 p-5 space-y-5">

      {/* ---- Toolbar ---- */}
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          {hasChanges && (
            <span className="rounded bg-amber-900/40 px-2 py-0.5 text-xs text-amber-400 border border-amber-700/40">
              Unsaved changes
            </span>
          )}
        ...
        <div className="flex gap-2">
          <button
            ...
            className="rounded border border-zinc-600 bg-zinc-800 px-3 py-1.5 text-sm text-zinc-300 hover:bg-zinc-700 disabled:opacity-40 disabled:cursor-not-allowed"
          >
            Discard Changes
          </button>
          <button
            ...
            disabled={!hasChanges || saving}
            className="rounded bg-blue-600 px-4 py-1.5 text-sm font-medium text-white hover:bg-blue-500 disabled:opacity-40 disabled:cursor-not-allowed"
          >
      ...
      <input
        ...
        className="w-full max-w-xs rounded border border-zinc-600 bg-zinc-800 px-3 py-1.5 text-sm text-zinc-100 placeholder-zinc-500"
      />
      ...
      <div className="overflow-x-auto rounded border border-zinc-700">
        <table className="min-w-full text-sm">
          <thead className="bg-zinc-800 text-xs text-zinc-400">
            <tr>
              <th
                className="px-4 py-2.5 text-left cursor-pointer select-none hover:text-zinc-200"
          ...
          <tbody className="divide-y divide-zinc-800">
            {filtered.map((team) => {
              ...
              return (
                <tr key={team.school} className={dirty ? 'bg-amber-950/20' : 'hover:bg-zinc-800/50'}>
                  <td className="px-4 py-2 text-zinc-100 whitespace-nowrap">{team.school}</td>
                  <td className="px-4 py-2 text-zinc-400 whitespace-nowrap">{team.conference}</td>
                  <td className="px-4 py-1.5">
                    <input
                      type="text"
                      value={owner}
                      onChange={(e) => handleOwnerChange(team.school, e.target.value)}
                      className="w-full rounded border border-transparent bg-transparent px-2 py-1 text-sm text-zinc-100 placeholder-zinc-600 hover:border-zinc-600 focus:border-zinc-500 focus:bg-zinc-800 focus:outline-none"
                      placeholder="—"
                    />
                  </td>
                </tr>
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={3} className="px-4 py-4 text-center text-sm text-zinc-500">
      ...
      {/* ---- Bulk Reassign ---- */}
      <div className="rounded border border-zinc-700 bg-zinc-800/40 p-4 space-y-3">
        <h3 className="text-sm font-medium text-zinc-100">Bulk Reassign</h3>
        <p className="text-xs text-zinc-400">
          Move all teams from one owner to another. Updates local state only — click Save Changes to persist.
        </p>
        <div className="flex flex-wrap items-end gap-3">
          <div className="flex flex-col gap-1">
            <label className="text-xs text-zinc-400">From owner</label>
            <input
              type="text"
              value={bulkFrom}
              onChange={(e) => setBulkFrom(e.target.value)}
              placeholder="Current owner name"
              className="rounded border border-zinc-600 bg-zinc-800 px-3 py-1.5 text-sm text-zinc-100 placeholder-zinc-500"
            />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs text-zinc-400">To owner</label>
            <input
              type="text"
              value={bulkTo}
              onChange={(e) => setBulkTo(e.target.value)}
              placeholder="New owner name (blank to unassign)"
              className="w-56 rounded border border-zinc-600 bg-zinc-800 px-3 py-1.5 text-sm text-zinc-100 placeholder-zinc-500"
            />
          </div>
          <button
            onClick={handleBulkReassign}
            disabled={!bulkFrom.trim()}
            className="rounded border border-zinc-600 bg-zinc-700 px-4 py-1.5 text-sm text-zinc-100 hover:bg-zinc-600 disabled:opacity-40 disabled:cursor-not-allowed"
          >
            Reassign all teams
          </button>
        </div>
