...
export default function ScoreAttachmentDebugPanel({
  ...
  const sortedReasons = useMemo(() => {
    if (!data) return [];
    return Object.entries(data.summary.reasons).sort((a, b) => b[1] - a[1]);
  }, [data]);
          ...
          <>
            ...
            <div className="flex flex-wrap gap-2">
              {sortedReasons.length === 0 ? (
                <span className="text-xs text-gray-500">No ignored rows.</span>
              ) : (
            ...
            <div className="overflow-x-auto rounded border border-gray-200 dark:border-zinc-700">
              <table className="min-w-full text-xs">
                ...
                <tbody>
                  {data.diagnostics.slice(0, 100).map((item, idx) => (
