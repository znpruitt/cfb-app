...
test('overview watchlist uses the shared scoreboard with records and one odds footer', () => {
  ...
  const html = renderToStaticMarkup(
    <OverviewPanel
      ...
      teamColorsById={new Map([['buffalo', '#005BBB']])}
  ...
  assert.doesNotMatch(scoreboard, /data-scoreboard-team-color="away"/);
  assert.match(scoreboard, /data-scoreboard-team-color="home"/);
});
...
test('overview scoreboards keep current records across scheduled, live, and final states', () => {
  ...
  assert.equal(
    (html.match(/data-scoreboard-team-color=/g) ?? []).length,
    6,
    'scheduled, live, and recent-final renderers must each colour both team lines'
  );
});
...
test('overview Featured renders its badge and existing tag in the final status row', () => {
  ...
  assert.equal((finalScoreboard.match(/data-scoreboard-team-color=/g) ?? []).length, 2);
