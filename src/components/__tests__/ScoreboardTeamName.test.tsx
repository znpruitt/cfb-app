...
test('SSR starts each measured label at one visible abbreviation with one stable accessible name', () => {
  ...
  for (const [marker, fullName, abbreviation] of [
    ...
    assert.ok(label && accessible && full && short);

    assert.equal(label.getAttribute('data-scoreboard-team-display'), 'abbreviation');
    assert.match(label.className, /overflow-visible/);
    assert.match(label.className, /whitespace-nowrap/);
    assert.doesNotMatch(label.className, /truncate|overflow-hidden/);
    ...
    assert.doesNotMatch(short.className, /invisible/);
...
test('Compact scoreboard gives the name its own box and isolates record and owner suffixes', () => {
  ...
  assert.match(label.className, /max-w-full/);
  assert.match(label.className, /shrink-0/);
...
test('Compact scoreboard leaves a null abbreviation as plain untruncated full-name text', () => {
  ...
  assert.match(away.className, /overflow-visible/);
  assert.match(away.className, /whitespace-nowrap/);
  assert.doesNotMatch(away.className, /truncate|overflow-hidden/);
