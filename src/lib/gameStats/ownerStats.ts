...
import type { AnalyticsGameStats, AnalyticsTeamStats } from './contract.ts';

/**
 * PLATFORM-086H3E3 — owner aggregation over the PROJECTED analytics type only.
 *
 * The input is `AnalyticsGameStats` — the final-and-complete, participant-
 * verified projection produced by `projectAnalyticsPartition` — never raw
 * persisted `WeeklyGameStats.games`. Narrowing the boundary to the projected
 * type makes accidentally passing a raw persisted row a COMPILE error: the
 * projection's required `source: 'v2' | 'legacy'` discriminant does not exist
 * on persisted rows, so `GameStats[]` is not structurally assignable. This is
 * the activation contract's compile-time regression seam (pinned by a
...
/**
 * Aggregate PROJECTED analytics rows across all weeks into per-owner season
 * totals.
 *
 * @param weeklyGames - Array of per-partition `projectAnalyticsPartition` outputs
...
export function aggregateOwnerSeasonStats(
  weeklyGames: AnalyticsGameStats[][],
