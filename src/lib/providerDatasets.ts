...
export const PROVIDER_DATASET_DESCRIPTORS: Record<ProviderDataset, ProviderDatasetDescriptor> = {
  scores: {
    ...
    currentAutomation:
      'Every 3 minutes (QStash `turfwar-live-scores-3m` → GET /api/cron/live-scores): schedule-armed, polling only kickoff-window games (~15 min before kickoff through 24 h after) while they remain unresolved — at most ONE billed CFBD /scoreboard or /games score request per run, above the 1,000-call monthly reserve, honoring the global pause and the Scores auto-refresh toggle. A newly committed final can additionally trigger the separately tracked, six-hour-floor-gated Team records refresh. Visible browser tabs read the full eligible partition set cache-only every 90 seconds while any game is inside its fixed kickoff window without positive final score evidence, then every 3 minutes; missing or ambiguous evidence stays fast until the hard 8-hour ceiling. Browser reads are not controlled by provider automation settings.',
    plannedPolicy:
      'Active (PLATFORM-086B2B): QStash provider polling uses a fixed 3-minute schedule-armed cadence controlled by the global pause and Scores auto-refresh toggle. Visible browser tabs read the full eligible partition set cache-only on a 90-second/3-minute cadence: positive final score evidence can end the fast tier early, while 8 hours after kickoff is its hard ceiling. Browser reads are not controlled by provider automation settings. All cadences are version-controlled, not admin-editable.',
  ...
  schedule: {
    ...
    hasActiveAutomation: true,
    currentAutomation:
      'Preseason transition probe (season-transition cron, lifecycle-critical) plus the active weekly in-season route GET /api/cron/schedule-refresh (QStash `turfwar-schedule-weekly`, Tuesdays 12:00 UTC): each active season year gets one complete regular+postseason refresh through the shared full-season authority. Ordinary weekly maintenance honors the global pause and this toggle; the postseason-boundary maintenance that establishes the season-rollover boundary (from 7 days before the latest regular-season kickoff, while leagues remain in season) is lifecycle-critical and exempt. PLATFORM-757a adds a THIRD automation on this toggle ONCE ITS SCHEDULE IS INSTALLED (an owner step, runbook §8i — the route ships dormant and no QStash schedule exists until then): the standalone schedule-presentation job (`turfwar-schedule-presentation-weekly`, Tuesdays 13:00 UTC) refreshes the media and venue caches and is never lifecycle-critical, so the global pause and this toggle stop it outright.',
    plannedPolicy:
      'Active (PLATFORM-086E1B): fixed weekly Tuesday 12:00 UTC external trigger — the QStash schedule and cadence are version-controlled, never admin-editable. The toggle pauses ordinary weekly maintenance and, once its schedule is installed, the whole PLATFORM-757a presentation job; the preseason transition and postseason-boundary maintenance remain exempt.',
