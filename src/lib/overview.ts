...
export function deriveOverviewSnapshot(params: {
  ...
  const allSeasonItems = [...allSections.ownerMatchups, ...allSections.secondaryGames].map(
    (bucket) => toOverviewItem(bucket, scoresByKey[bucket.game.key])
  );

  const liveItems = allSeasonItems
