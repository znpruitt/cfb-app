...
/**
 * The same shape as `draft`, pointed the other way.
 *
 ...
 * draft results outrank it once they exist, being new, timely and event-based.
 *
 * Full weight through `postseason` (reached only when the season is genuinely
 * over), `fresh_offseason` and `offseason`; reduced in `preseason`. `0.55` is
 * derived rather than picked: it is the largest factor that keeps a typical
 * champion margin (125 + margin x 4) below the membership cards reporting the
 * draft (80-84) while staying above the career and milestone pool (60-74), so
 * the recap is present rather than buried. A very large margin can still reach
 * that band; rotation (INSIGHTS-018) is what removes the need to tune this.
 */
