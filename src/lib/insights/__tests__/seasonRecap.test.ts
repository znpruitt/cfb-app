...
test('an ARCHIVED year loads analytics from its archive, not from live caches', async () => {
  // Mutation-found gap: nothing observed WHICH provenance the archived year ran
  // against. `loadOwnerSeasonStats` pairs a season with ONE source, and
  // `{ kind: 'live' }` rebuilds analytics from mutable schedule and score caches,
  // so a later cache or identity change could make historical cards disagree with
  // the immutable archive beside them.
  //
  // Observable because the two provenances fail differently: an archive carrying
  // a valid slate is AVAILABLE, while live analytics in a store with no cached
  // game-stat partitions is not. So a non-null result can only have come from
  // the archive.
  await addLeague({
    slug: 'prov',
    displayName: 'Provenance League',
    year: YEAR,
    createdAt: '2026-01-01T00:00:00.000Z',
    foundedYear: 2021,
    // NOT preseason: `loadOwnerSeasonStats` is skipped there, so the call under
    // test would never happen.
    status: { state: 'season', year: YEAR },
  });
  await setAppState('standings-archive:prov', String(PRIOR), {
    leagueSlug: 'prov',
    year: PRIOR,
  ...
  await setAppState('owners:prov:2026', 'csv', 'team,owner\nGeorgia,Zoe\nClemson,Yuri');
  // A cached partition must exist for EITHER provenance to get past
  // `no-cached-partitions`, which is the gate both were failing on before —
  // making the two indistinguishable and the assertion below vacuous.
  ...
  const archived = await buildLeagueInsightContext('prov', PRIOR, new Date());
  assert.notEqual(
    archived.ownerGameStats,
    null,
    'an archived year must resolve against its archive, which carries a slate'
  );

  // Control: the CURRENT year has no archive, so it runs live — and live is
  // unavailable here. This is what proves the assertion above discriminates.
  const live = await buildLeagueInsightContext('prov', YEAR, new Date());
...
test('the context describes the REQUESTED year, with matching provenance', async () => {
  ...
  // `league.year` stays put, so anything keyed off `currentYear` — league
  // records, rookie detection, the recap's own title — described one season
  // using another's number.
  await seedLeague();
  const requested = await buildLeagueInsightContext(SLUG, PRIOR, new Date());
  assert.equal(
    requested.currentYear,
    PRIOR,
    'the context must be numbered for the season it was BUILT from'
  );

  // Control: the ordinary path is unchanged, so this is a divergence fix rather
  // than a redefinition of `currentYear`.
