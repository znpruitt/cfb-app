...
1. **API-first schedule + scores**
   ...
   - **Complete-before-commit for transition schedule refreshes (PLATFORM-085B).** A refresh that fetches multiple schedule partitions (the season-transition cron fetches regular + postseason and writes the shared `${year}-all-all` key) must validate that ALL requested partitions resolved before publishing durable schedule/probe state. A partition that throws, returns a non-array, or normalizes a nonempty payload to zero rows is **uncertainty** — retain prior-good durable state and report the partial result; do not commit partial rows as a complete schedule. A partition that fetches successfully with zero rows is **valid absence** (e.g. postseason before bowls). The schedule route already enforces this for `all` requests via `hasRequiredSeasonTypeFailure`; reuse it rather than re-deriving completeness.
