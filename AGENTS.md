...
2. **Every production lifecycle transition is GUARDED inside the registry transaction** (PLATFORM-086F2H1). One shared primitive (`guardedLifecycleWrite`) covers registry read → expected-state validation → state derivation → write in a single `withAppStateKeyTransaction('leagues', 'registry', …)` callback, so no caller may derive a lifecycle year from a snapshot read outside the lock, and a refusal writes nothing and is returned as a typed outcome rather than a silent overwrite. The guarded operations are `beginPreseasonTransition` (requires `offseason`; derives `year + 1` under the lock, so a double submission or stale tab cannot increment twice), `completePreseasonSetup` (requires `preseason` at EXACTLY the submitted year, so a stale setup form cannot move the year — an already-complete match is a typed no-op that rewrites nothing UNLESS the top-level year projection is stale, in which case it still commits the healing write the pre-F2H1 unconditional rewrite performed), `completeSeasonTransition` (requires `preseason` at exactly the target year; the daily cron's authority), `completeSeasonRollover` (invariant 5), and `initializeMissingLifecycleStatus` (invariant 4). The unguarded `updateLeagueStatus` applies no precondition and is reserved for the `test` league's own lifecycle controls (`src/app/admin/[slug]/actions.ts`); a source-scan test (`src/lib/__tests__/leagueRegistry.lifecycleCallers.test.ts`) pins that allowlist, that rollover was not rerouted through an unrestricted mutation, and that the synchronized projection exists in exactly one module. The season-transition cron counts ONLY confirmed transitions: a refusal is reported truthfully (`lifecycle-transition-refused`, per-year `partial`, and a `refusedLeagues` field absent on the ordinary path) and never as success. A genuinely stale refusal is ALWAYS `partial`, never `no-op` — `no-op` would be untruthful when the same run made a billed provider call and durably committed a schedule, and System Health raises a scheduler-execution issue only for `failure`/`partial`, so `no-op` would silently hide a fully-stale target set while a mixed one surfaced. A league already in `season` at the target year is NOT a refusal: it is the separate benign `already-in-target-season` outcome, because the schedulers deliver at-least-once and an overlapping duplicate invocation reaching the desired end state is idempotent, not an anomaly. The cron deliberately applies **no** `setupComplete` gate — it still transitions near kickoff when setup is incomplete; changing that is a product decision, not a safety fix.
