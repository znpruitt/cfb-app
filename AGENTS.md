...
2. **Every production lifecycle transition is GUARDED inside the registry transaction** (PLATFORM-086F2H1). One shared primitive (`guardedLifecycleWrite`) covers registry read → expected-state validation → state derivation → write in a single `withAppStateKeyTransaction('leagues', 'registry', …)` callback, so no caller may derive a lifecycle year from a snapshot read outside the lock, and a refusal writes nothing and is returned as a typed outcome rather than a silent overwrite. The guarded operations are `beginPreseasonTransition` (requires `offseason`; derives `year + 1` under the lock, so a double submission or stale tab cannot increment twice), `completePreseasonSetup` (requires `preseason` at EXACTLY the submitted year, so a stale setup form cannot move the year — an already-complete match is a typed no-op that rewrites nothing UNLESS the top-level year projection is stale, in which case it still commits the healing write the pre-F2H1 unconditional rewrite performed), `completeSeasonTransition` (requires `preseason` at exactly the target year; the daily cron's authority), `completeSeasonRollover` (invariant 5), and `initializeMissingLifecycleStatus` (invariant 4). The state-unguarded `updateLeagueStatus` applies no expected-state precondition and exists ONLY for the `test` league's own lifecycle controls (`src/app/admin/[slug]/actions.ts`). That restriction is enforced AT RUNTIME inside the function, which rejects any slug other than `TEST_LEAGUE_SLUG` **before opening a transaction or performing any registry read or write** — so no non-test slug can reach a lifecycle write through it however the call is constructed (aliased import, barrel re-export, dynamic `import()`, indirect reference). A source-scan test (`src/lib/__tests__/leagueRegistry.lifecycleCallers.test.ts`) additionally pins the caller allowlist, that rollover was not rerouted through an unrestricted mutation, and that the synchronized projection exists in exactly one module; that scan is defense-in-depth, never the sole enforcement. The season-transition cron counts ONLY confirmed transitions: a refusal is reported truthfully (`lifecycle-transition-refused`, per-year `partial`, and a `refusedLeagues` field absent on the ordinary path) and never as success. A genuinely stale refusal is ALWAYS `partial`, never `no-op` — `no-op` would be untruthful when the same run made a billed provider call and durably committed a schedule, and System Health raises a scheduler-execution issue only for `failure`/`partial`, so `no-op` would silently hide a fully-stale target set while a mixed one surfaced. A league already in `season` at the target year is NOT a refusal: it is the separate benign `already-in-target-season` outcome, because the schedulers deliver at-least-once and an overlapping duplicate invocation reaching the desired end state is idempotent, not an anomaly. The cron deliberately applies **no** `setupComplete` gate — it still transitions near kickoff when setup is incomplete; changing that is a product decision, not a safety fix.

3. **Rendering never persists lifecycle state.** Admin pages/GETs read legacy missing-status records through the read-only `{ state: 'season', year: league.year }` inference; initialization or repair is an explicit operation, never a render side effect.

4. **Legacy missing-status recovery is narrow, explicit, and non-substitutable** (PLATFORM-086F2H1). `initializeMissingLifecycleStatus` installs exactly the read-only compatibility interpretation (`{ state: 'season', year: league.year }`) and ONLY when `status` is genuinely absent — meaning absent to every reader, so BOTH `undefined` and a persisted `null` qualify (`leagueStandings.ts` uses `league.status ?? …`; `rolloverTargeting.ts` uses `!status`), and a `null` record is repairable rather than refused as malformed. It refuses a record that already carries a status — valid (`status-already-present`) or malformed (`invalid-existing-status`, never repaired) — an unusable stored year (`invalid-legacy-year`), and the `test` league (`test-league-managed-separately`); it offers no state selection, never increments the year, never archives, and never bypasses rollover. Its only caller is the platform-admin `POST /api/admin/lifecycle-recovery` (authenticate first; canonical slug + literal `confirmed: true`; allowlisted response carrying no credential material or raw storage detail; `400 lifecycle-recovery-invalid-request`, `404 lifecycle-recovery-league-not-found`, `409 lifecycle-status-already-present` / `test-league-lifecycle-managed-separately` / `lifecycle-recovery-invalid-legacy-record`, `503 lifecycle-recovery-unavailable`). There is no GET and no generic lifecycle setter. Legacy missing-status records stay excluded from rollover targeting.
