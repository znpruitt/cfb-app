...
4. **API routes use `requireAdminAuth(req)`** — this helper checks Clerk JWT first, then the transitional `ADMIN_API_TOKEN` fallback. It is a drop-in replacement for the old `requireAdminRequest()`. All new admin API routes must support Clerk JWT from day one.

5. **`ADMIN_API_TOKEN` fallback is transitional and planned for removal** — it remains only for backward-compatible machine/admin API callers. Remove it as part of the reviewed commissioner/member authorization work, when commissioner-scoped Clerk roles replace the remaining token-based fallback. Do not build new flows that depend on it, and do not remove it before the replacement authorization model is implemented.
