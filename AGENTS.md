...
1. **`league.status` is the lifecycle authority; `league.year` is only its synchronized projection.** All lifecycle writes live in `src/lib/leagueRegistry.ts` and share ONE projection (`applyLifecycleStatus`): `season`/`preseason` set `status` AND synchronize `league.year = status.year` in ONE registry write; `offseason` sets `status` and writes the last authoritative season year — the outgoing `status.year` when one exists — into `league.year` (healing any desynchronized legacy top-level year rather than carrying it forward). ALL registry mutations serialize on the registry key via `withAppStateKeyTransaction` (whole-array read-modify-write; without the lock, concurrent mutators would drop one another's update). Generic `updateLeague` and the league-configuration PATCH (`/api/admin/leagues/[slug]`) must not mutate `year` or `status` (the API rejects with `409 league-year-lifecycle-managed` / `league-status-lifecycle-managed`) — never reintroduce a second year authority. New leagues are created with an explicit `status: { state: 'season', year }`.

2. **The supported season-year range is an INGRESS rule, never a transition rule** (PLATFORM-086F2H1). `isSupportedSeasonYear` (`src/lib/league.ts`, the single exported predicate + constants) is enforced where a lifecycle year is MINTED — league creation (`POST /api/admin/leagues`) — so the app cannot create new corrupt records. The guarded transitions deliberately do NOT apply it to already-persisted records: they tolerate any structurally valid (integer) stored year, because refusing one froze legitimate legacy data through every surface while the daily cron re-reported it as an anomaly forever. Only `beginPreseasonTransition`, which DERIVES `year + 1`, requires structural validity — and only to avoid writing a non-year.

3. **Every production lifecycle transition is GUARDED inside the registry transaction** (PLATFORM-086F2H1). One shared primitive (`guardedLifecycleWrite`) covers registry read → expected-state validation → state derivation → write in a single `withAppStateKeyTransaction('leagues', 'registry', …)` callback, so no caller may derive a lifecycle year from a snapshot read outside the lock, and a refusal writes nothing and is returned as a typed outcome rather than a silent overwrite. The guarded operations are `beginPreseasonTransition` (requires `offseason`; derives `year + 1` under the lock, so a double submission or stale tab cannot increment twice), `completePreseasonSetup` (requires `preseason` at EXACTLY the submitted year, so a stale setup form cannot move the year — an already-complete match is a typed no-op that rewrites nothing UNLESS the top-level year projection is stale, in which case it still commits the healing write the pre-F2H1 unconditional rewrite performed), `completeSeasonTransition` (requires `preseason` at exactly the target year; the daily cron's authority), `completeSeasonRollover` (invariant 6), and `initializeMissingLifecycleStatus` (invariant 5). The state-unguarded `updateLeagueStatus` applies no expected-state precondition and exists ONLY for the `test` league's own lifecycle controls (`src/app/admin/[slug]/actions.ts`). That restriction is enforced AT RUNTIME inside the function, which rejects any slug other than `TEST_LEAGUE_SLUG` **before opening a transaction or performing any registry read or write** — so no non-test slug can reach a lifecycle write through it however the call is constructed (aliased import, barrel re-export, dynamic `import()`, indirect reference). An AST-based source scan (`src/lib/__tests__/leagueRegistry.lifecycleCallers.test.ts`, TypeScript compiler API — never regex over text, which was defeated by an aliased import and blinded by `//` inside string literals) additionally pins the caller allowlist, that rollover was not rerouted through an unrestricted mutation, and that the synchronized projection exists in exactly one module; that scan is defense-in-depth, never the sole enforcement. The season-transition cron counts ONLY confirmed transitions: a refusal is reported truthfully (`lifecycle-transition-refused`, per-year `partial`, and a `refusedLeagues` field absent on the ordinary path) and never as success. The cron keeps three non-transitioned dispositions INDEPENDENT — `already-in-target-season` (a benign idempotent redelivery, which still busts standings so an invocation killed between its durable commit and its cache bust is recovered), `league-removed` (a normal admin deletion), and a genuinely stale refusal. Only the last is an anomaly, and `transitioned` reports the year's COMPLETION state rather than merely this run's actions. A genuinely stale refusal is ALWAYS `partial`, never `no-op` — `no-op` would be untruthful when the same run made a billed provider call and durably committed a schedule, and System Health raises a scheduler-execution issue only for `failure`/`partial`, so `no-op` would silently hide a fully-stale target set while a mixed one surfaced. A league already in `season` at the target year is NOT a refusal: it is the separate benign `already-in-target-season` outcome, because the schedulers deliver at-least-once and an overlapping duplicate invocation reaching the desired end state is idempotent, not an anomaly. The cron deliberately applies **no** `setupComplete` gate — it still transitions near kickoff when setup is incomplete; changing that is a product decision, not a safety fix.

4. **Rendering never persists lifecycle state.** Admin pages/GETs read legacy missing-status records through the read-only `{ state: 'season', year: league.year }` inference; initialization or repair is an explicit operation, never a render side effect.

5. **Legacy missing-status recovery is narrow, explicit, and non-substitutable** (PLATFORM-086F2H1). `initializeMissingLifecycleStatus` installs exactly the read-only compatibility interpretation (`{ state: 'season', year: league.year }`) and ONLY when `status` is genuinely absent — meaning absent to every reader, so BOTH `undefined` and a persisted `null` qualify (`leagueStandings.ts` uses `league.status ?? …`; `rolloverTargeting.ts` uses `!status`), and a `null` record is repairable rather than refused as malformed. It tolerates any structurally valid stored year, so a legacy `1999` is REPAIRABLE (refusing it by supported range left the record detected-but-unfixable). It refuses a record that already carries a status — valid (`status-already-present`) or malformed (`invalid-existing-status`, never repaired) — a stored value that is not a year at all (`invalid-legacy-year`), and the `test` league (`test-league-managed-separately`); it offers no state selection, never increments the year, never archives, and never bypasses rollover. Its only caller is the platform-admin `POST /api/admin/lifecycle-recovery` (authenticate first; canonical slug + literal `confirmed: true`; allowlisted response carrying no credential material or raw storage detail; `400 lifecycle-recovery-invalid-request`, `404 lifecycle-recovery-league-not-found`, `409 lifecycle-status-already-present` / `test-league-lifecycle-managed-separately` / `lifecycle-recovery-invalid-legacy-record`, `503 lifecycle-recovery-unavailable`). There is no GET and no generic lifecycle setter. Legacy missing-status records stay excluded from rollover targeting.

6. **Season rollover — manual AND automatic — is per-year, strict, and shared.** Target selection goes through `groupRolloverTargets` (`src/lib/rolloverTargeting.ts`): non-test leagues with `status.state === 'season'`, grouped exclusively by `status.year` (never `league.year`, the first registered league, or the calendar). Both `/api/admin/rollover` and `/api/cron/season-rollover` execute only behind `resolveNationalChampionshipRollover` (structured `cfbd-structured` CFP national championship + confirmed complete final + seven-day delay; cache-only, no provider calls), re-evaluated on every manual POST — a previously generated preview never authorizes execution. Archive-before-status is guaranteed in both paths, at two deliberate granularities: the MANUAL route is group-atomic two-stage (all archives first; any archive failure prevents every status transition for the year group), while the automatic cron preserves its per-league isolation (each league transitions only after its own archive save; one league's archive failure skips only that league) — in neither path can a league transition without its own durable archive. Every season→offseason transition goes through the guarded `completeSeasonRollover` (inside the serialized registry transaction, the league must STILL be in `season` for the exact requested year — a stale request can never clobber a league another actor already rolled or advanced). Partial status failures are reported truthfully. There is no force/emergency bypass; an exceptional forced recovery would require a separately reviewed operation.
