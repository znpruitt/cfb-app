...
## 8g) PLATFORM-086C2 activation — automatic Odds polling operator sequence — ⏳ NOT YET EXECUTED

**Status: this activation sequence has NOT been performed — the Odds cron ships DORMANT.** Merging PLATFORM-086C2 makes `GET /api/cron/odds` **operator-activatable** but does NOT start automatic Odds polling: no `turfwar-odds-hourly` QStash schedule exists until step 4 below, so the route is never invoked on a schedule. This section is the **prescribed activation procedure** (mirroring §8f) plus the ongoing **emergency-stop / `CRON_SECRET`-rotation reference** — execute it as a **distinct post-merge phase**, only after the PR is merged and deployed, and only when you intend to turn automatic Odds polling on. Do NOT run any step merely to validate the merged build.

The C2 build (PR TBD) converged the manual `GET /api/odds?refresh=1` route and the new automatic cron onto ONE shared server-side execution authority (`executeOddsRefresh`), closed the pre-existing `ODDS_API_KEY` credential-exposure seam (upstream URL/message redaction), made public/member Odds reads durable-cache-only with a bounded (120 s) cross-instance memo, and activated the Odds provider descriptor (`hasActiveAutomation: true`, `autoRefreshSettingConsumed: true`) — all resting on the DORMANT PLATFORM-086C1 refresh authority (durable per-target lease + observation ordering + atomic commit). **Promoting the build does NOT start server-side Odds automation.** What promotion DOES change, harmlessly: public/member `/api/odds` now serves the **durable cache only** (never a self-fetch, never quota) and reflects cross-instance cron commits within the 120 s memo window; until the QStash schedule is provisioned, that cache is updated only by manual admin refresh, so the freshness label reads stale between manual refreshes.

The **server-side** automation — the QStash `turfwar-odds-hourly` schedule that makes `GET /api/cron/odds` decide cadence and, only when a refresh is DUE, issue at most ONE billed `/odds` request — is gated by BOTH `isAutoRefreshAllowed('odds')` conditions (`globalPause == false` AND the `odds` dataset `enabled != false`); a scheduled delivery arriving while EITHER gate is closed returns HTTP `200` with a provider-free `skipped / automation-paused-or-disabled`. The hourly cadence is a CEILING, not the request rate: the pure policy issues a request only when the freshest completed signal is older than the cadence threshold (baseline 6 h; 2 h inside the 6 h before a slate's first America/Chicago kickoff), an eligible in-horizon game exists, and the durable automatic backoff is not active — so most hourly deliveries are provider-free `skipped / refresh-not-due` or `skipped / no-eligible-target`. On any CLI exit `4` (indeterminate durability), inspect read-only and STOP; never blind-retry.
