...
cache-only score partition set every 90 seconds while at least one eligible game's kickoff has
passed and it is not final, and every three minutes otherwise. The cron remains the only writer on
its unchanged three-minute cadence, so the live browser tier improves expected display staleness
from about 180 seconds to 135 seconds — roughly 45 seconds or 25%, not 2× — without doubling reads
through the inactive 24-hour finals tail. Browser reads are provider-free, not cost-free: they still
