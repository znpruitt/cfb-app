...
## 8d) PLATFORM-086H3E activation — the complete operator sequence (documented, NOT executed during implementation)
...
2. **Forced full-year schedule refreshes for 2021–2025 plus the activation current season** (deduplicate the year list), with the OLD build still serving: `/api/schedule?year=<year>&bypassCache=1` (admin-authenticated; never the `force: false` Historical Data Cache button). Verify each response is full-year, provider-backed, non-partial, and nonempty; verify the exact year-scoped provider status succeeded; recheck each year cache-only. These refreshes persist the numeric `homeId`/`awayId` participant ids that evidence validation and the E1 archive snapshots require — **they MUST precede the backfills in step 4** or the snapshots would bake in null ids and fail closed.
3. **Read-only participant/parity audit** (the established procedure; no CLI exists). Require: positive numeric home/away ids for every addressable stat-producing activation-scope game; **zero** `participant-validation-unavailable`; **zero** unexpected `identity-mismatch`; archive/canonical parity with only the accepted 2022 game `401506450` excluded (upstream analytics-incomplete evidence). **STOP on any other residual.**
...
7. **Smoke test (NO provider calls)**: authenticated cache-only `/api/game-stats` reads (admin-only on every request; distinct absence/read-failure/context outcomes; projector-only wire with no internal metadata), Insights/history/career analytics over the backfilled archives, evidence-based diagnostics, and writer refusals (a `bypassCache=1` refresh in `armed` fails `game-stats-unavailable` with both writers refusing — this spends its usage probe + provider call; alternatively defer refresh smoke to step 9's proof).
...
10. **Re-enable automatic game-stats refresh** (step 1's toggle) only after the manual proof is clean.
11. **Verify one scheduled 15-minute run** honors the kickoff-window target bound (at most one partition fetch), the 1,000-call reserve, the auto-refresh setting, and the exact status mapping — with no score automation.
12. **Post-activation**: the emergency stop is `active → read-only-safe`; resume only via `read-only-safe → active`; **never return to `legacy`**. Complete a separate docs-only status update recording the exact deployed commit, control transitions, refreshed years, audit evidence, archive backfills, manual proof, first scheduled proof, quota state (including the `/info` cost finding), and final control state.
