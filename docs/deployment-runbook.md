...
The E3 build (final atomic wiring, PR pending) changes serving behavior when it deploys, but **writing stays operator-gated**: the fenced legacy writer persists only under writer-control `legacy`, H2 only under `active`, and in `armed` both refuse. Execute these steps **in order**; on ANY unexpected residual, refusal anomaly, or CLI exit `4` (indeterminate durability — reread with a dry run and STOP; never blind-retry), stop and investigate.

1. **Disable automatic game-stats refresh** via the admin provider-status panel's per-dataset toggle (the only dataset consuming the auto-refresh setting).
2. **Complete the §8d PLATFORM-086H3E4 sequence first** (deploy of the merged collision correction → forced full-year 2021–2025 refreshes → identity verification → collision-audit rerun → full parity-audit rerun). Its refreshes and dual audits subsume this sequence's historical-year prerequisites; **additionally refresh the activation current season** the same way (`/api/schedule?year=<year>&bypassCache=1`; verify full-year, provider-backed, non-partial, nonempty, year-scoped status succeeded, cache-only recheck). The refreshes persist the numeric `homeId`/`awayId` participant ids that evidence validation and the E1 archive snapshots require — **they MUST precede the backfills in step 4** or the snapshots would bake in null ids and fail closed.
3. **Confirm the §8d audits are clean for the activation scope** (positive numeric home/away ids for every addressable stat-producing activation-scope game; **zero** `participant-validation-unavailable`; **zero** unexpected `identity-mismatch`; archive/canonical parity with only the accepted 2022 game `401506450` excluded). **STOP on any other residual.**
4. **Archive backfills** through the established `POST /api/admin/backfill` preview → explicit confirmed flow for every relevant existing archive year. Each rebuilt archive gains its `gameStatSlate` snapshot paired with that archive's own `scoresByKey`; verify the accepted 2022 residual remains the only parity difference. (Doing this BEFORE activation means archived-year analytics work the moment E3 serves — no user-visible degradation window.)
5. **Writer control to `armed`**: `npm run init:writer-control` (read-only inspection; use `-- --apply` only if the row is genuinely absent and the approved procedure calls for initialization); then dry-run and apply `npm run transition:writer-control -- --from legacy --to armed [--apply]`. In `armed` the old build's legacy writer refuses — automated refreshes were already disabled in step 1.
6. **Controlled release of the exact reviewed build** (verified against current Vercel docs): BEFORE merging the approved E3 PR, disable **Auto-assign Custom Production Domains** (Vercel → Project → Settings → Environments → Production → Branch Tracking). Merging then builds a true Production-environment deployment in **Staged** state that serves no traffic. With control `armed`, **Promote** that exact staged deployment (dashboard ellipsis → Promote, or `vercel promote`) — instant, no rebuild. While the toggle is off, every `main` merge stages rather than serves; re-enable it after the post-activation docs update. **Pre-`active` rollback order (mandatory): Instant Rollback to the prior production deployment FIRST (still `armed`), verify the old fenced writer is serving, then `armed → legacy`. Never transition to `legacy` while the E build serves.**
   **External trigger note (PLATFORM-086H3E scheduler migration):** the 15-minute game-stats poll is NOT a Vercel cron — `vercel.json` carries only the two daily lifecycle crons, and the poll is triggered by an external QStash schedule that calls the unchanged `GET /api/cron/game-stats` (Vercel Hobby rejects sub-daily crons at deploy time, so there is **no Vercel-plan requirement** for the `*/15` cadence). The QStash schedule is provisioned in step 10a — AFTER the manual proof — and is NOT part of this build's promotion.
7. **Smoke test (strictly NO provider calls)**: authenticated cache-only `/api/game-stats` reads (admin-only on every request; distinct absence/read-failure/context outcomes; projector-only wire with no internal metadata), Insights/history/career analytics over the backfilled archives, and evidence-based diagnostics. Verify writer refusal WITHOUT provider spend: confirm the control record reads `armed` via a CLI dry-run — in `armed` both writers refuse by construction (suite-proven). Do NOT exercise a `bypassCache=1` refresh here (it would spend a usage probe plus a provider call); the provider-path proof is step 9, after `armed → active`.
8. **`armed → active`**: dry-run then apply `npm run transition:writer-control -- --from armed --to active --apply`. Exit `4`: reread and stop.
9. **One controlled manual provider proof**: explicit `bypassCache=1` for one approved current-season `(year, providerWeek, seasonType)` target. Verify ingestion (`refresh.outcome`/`reason`), the confirmed durable reread in `durable`, projection, analytics gating, exact scoped provider status, and quota accounting. **Determine empirically whether `/info` spends quota** (compare `remaining` before/after a usage probe) and record the result — the reserve's 2-call margin already covers the counted case.
10. **Provision the external QStash trigger** — only AFTER the manual proof is clean and while automatic refresh is STILL disabled, so an early delivery returns the provider-free paused result. `QSTASH_TOKEN` (management-only; operator-held, never in Vercel or the repo) and `CRON_SECRET` (the deployed route credential Vercel holds, forwarded by QStash) must be in the operator's environment.
    - **10a. Upsert the schedule**: `npm run manage:game-stats-schedule upsert --apply`. The CLI emits the FIXED contract only — schedule id `turfwar-game-stats-15m`, `POST /v2/schedules/https://turfwar.games/api/cron/game-stats`, `Upstash-Cron: */15 * * * *`, `Upstash-Method: GET`, `Upstash-Retries: 0`, `Upstash-Forward-Authorization: Bearer <CRON_SECRET>`, no callback/failure-callback/queue/workflow/scheduler-retry. Exit `4` (unconfirmed) → inspect (read-only) before any retry; never blind-retry.
    - **10b. Inspect to verify the readback**: `npm run manage:game-stats-schedule` (read-only default). Exit `0` requires the readback to match the fixed contract AND the forwarded Authorization to equal `Bearer <CRON_SECRET>` exactly (absent `CRON_SECRET` → exit `2` INCOMPLETE, never a false "verified"); a PAUSED note is printed if the schedule is paused. Confirm destination, cadence (`*/15`), method (`GET`), zero retries, exactly one forwarded Authorization, and no callbacks/queue/retry-delay.
11. **Re-enable automatic game-stats refresh** (step 1's toggle) only after the manual proof AND the QStash provisioning/inspection are clean.
12. **Verify one scheduled 15-minute delivery** through QStash logs (filter by schedule id) AND application status: exactly one delivery with `maxRetries: 0`, an authenticated route response, at most one partition fetch honoring the kickoff-window bound, the 1,000-call reserve, the auto-refresh setting, and the exact status mapping — with no score automation.
13. **Post-activation**: the emergency stop order is **disable automatic refresh → `npm run manage:game-stats-schedule pause --apply` (stop QStash deliveries) → then the writer-control emergency transition `active → read-only-safe`** where required; resume via `read-only-safe → active` then `npm run manage:game-stats-schedule resume --apply`; **never return to `legacy`**. Any rollback after schedule provisioning: disable automatic refresh and pause the QStash schedule FIRST. **Coordinated `CRON_SECRET` rotation**: disable automation → pause the schedule → update the deployed secret in Vercel and re-run `upsert --apply` (which re-forwards the new value) → inspect to verify while provider-disabled → resume. Complete a separate docs-only status update recording the exact deployed commit, control transitions, refreshed years, audit evidence, archive backfills, manual proof, QStash schedule provisioning + inspection, first scheduled delivery, quota state (including the `/info` cost finding), and final control state.
