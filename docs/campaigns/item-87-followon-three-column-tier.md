...
**Where 1300 comes from:** three 400px columns plus two 40px gaps. 400px is the width the longest row needs without clipping — team name, record, owner suffix and right-anchored score. The stress case is a row like *Middle Tennessee State (3–5) Shambaugh · 13*; the mockup carries one in Live for exactly this reason.

The value is arithmetic, not a round number picked for looks. If the row anatomy changes — a sixth element on the team line, or logos taking the line-start slot — the minimum column width changes and 1300 moves with it.
