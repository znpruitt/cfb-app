...
| 30 | Item 143 — **CLOSED BY OWNER RULING 2026-09-15: Matchups carries broadcast.** The ruling is the authority; the measurement below is why no decision was owed in the first place, and it stands as the correction to this row's premise. | **THE PREMISE WAS FALSE, MEASURED.** This row said *"the mockup omits it on every Matchups row while the same games on Schedule carry it; no document decides it"*, restating an annotation the mockup makes about itself. **Both halves are false about the mockup's own markup.** Classified all 23 `sb-status` rows in `mockups/matchups-schedule-mockup.html` (100% of the population, Matchups = the `owner-grid` span, Schedule = from the filter bar on): **Matchups carries broadcast on 1 of 10 rows** — a live row with `<span class="tv">ESPN2</span>` — and **Schedule on 3 of 13**, not on the same games throughout. Each section has exactly two live rows, one with a carrier and one without, so the mockup is inconsistent in the SAME way in both places: sloppiness, not a designed distinction. **And `DESIGN.md` is not silent, which is what this row assumed.** *Compact scoreboard header metadata* (`DESIGN.md:201`) binds the component: *"Broadcast renders for scheduled, live, and awaiting rows, but not finals"* — stated per STATE with no surface exception, and Matchups is a `CompactGameScoreboard` consumer. `AGENTS.md`'s rule that a campaign decision absent from `DESIGN.md` is unsettled cuts the other way here: the canonical document decides, and a mockup does not override it. **No owner decision is owed; [#723](https://github.com/znpruitt/cfb-app/issues/723) is a conformance fix.** The lesson is row 3's: a document's claim ABOUT ITSELF is a measurement, not a description — this one survived into an index row and blocked a slice. | measured against `mockups/matchups-schedule-mockup.html`, 2026-09-14; `DESIGN.md:201` |
