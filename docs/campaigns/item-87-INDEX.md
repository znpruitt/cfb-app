...
| 17 | **LIVE — ANSWERED 2026-09-09, and the code is wrong.** Measured: THREE backgrounds exist, not two. `#0a0a0a` (canvas, `globals.css:29`), `#09090b` (zinc-950, the nearest painted surface) and **`#171718` — the OWNER-ROW tint**, produced by `rgba(255,255,255,0.055)` over zinc-950 (`CompactGameScoreboard.tsx:53`). `teamColors.ts:23` normalises against `#0A0A0A`. Canvas-vs-zinc-950 is noise (0.5%); **the owner-row tint costs 9.5%, so a colour normalised to exactly 3:1 renders at 2.72:1 on every owner row.** **Normalise against `#171718`, the WORST CASE** — then every row clears regardless. | **Pick one background constant and state it** (`#0A0A0A` vs `#161616`) before either change ships. | `team-colour.md` → *One background constant* (50%) |
