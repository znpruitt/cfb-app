...
A game occupies exactly one state section: **Scheduled → Live on kickoff → Recent finals on completion.** Sections hide when empty; Featured remains a separate state-independent axis. The watchlist cap applies after routing, so a promoted or excluded row backfills from the next eligible future game rather than leaving a dead slot.

Recent finals and recap are independent except for exact rendered-result deduplication. During the recap window (06:00 ET the day after the week's last game-date through Thursday 06:00 ET), only a canonical game key actually rendered as a recap game highlight is removed from Recent finals. While the recap response is unresolved, target-week finals are withheld instead of painted and then removed; an unavailable or mismatched response owns no final. At Thursday 06:00 ET, finals from the week whose recap window ended expire. A final from the newly active week may still promote immediately. The boundary reuses `selectWeeklyRecapTileState`; no second cutoff is defined.

`gameStateFromScore` (`gameUi.ts:51`) returns a fourth value, `unknown`. **Decided:** a future kickoff routes to the watchlist. A past kickoff with no usable score **stays in Live and reads "Awaiting score"**, moving to Recent finals only when a final score attaches. The bounded row contributes to the Live count and retains the compact scoreboard's live label/state because the known start time has elapsed; "Awaiting score" qualifies that provider progress remains indeterminate. Routing it to Recent finals would assert the game finished, while returning it to Upcoming would assert a future kickoff. "Awaiting score" is confirmed in the game-level status row rather than a per-team anchor, because it describes the game rather than either side.
