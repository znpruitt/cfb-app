...
| [`docs/prompts/`](prompts/) | Full text of prompts dispatched to another agent, one file per `PROMPT_ID`, while the work is open — readable from any git worktree via `git show main:docs/prompts/<id>.md`. Moved to `archive/prompts/` at closeout. Not a ledger: `prompt-registry.md` still owns which IDs exist, `next-tasks.md` still owns what is NEXT | Current |
