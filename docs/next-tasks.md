...
| **4** | **47** ([#627](https://github.com/znpruitt/cfb-app/issues/627)) | admin authorization for the Insights diagnostic bypass — **IMPLEMENTED, awaiting review 2026-09-13** on `claude/627-insights-bypass-authorization`. `?bypassSuppression=1` now requires a platform admin; **DECIDED: 401 when the parameter is present and the caller is not**, placed after the existing 404 blend-in so a passworded or unknown league is still indistinguishable. **The issue's disclosure claim does not reproduce** — both `shouldSuppressGenerator` entries carry a non-bypassable copy inside their own generator, so the two responses differ in `generatedAt` alone (measured through the route, with positive controls that fire). **#627's body says a visitor can "receive withheld editorial output"; that is false against this code** and the owner updates the issue at merge. What was live is the COMPUTE half: the uncached rebuild past a 300 s cache, ~157 ms median on a 900-game file-backed fixture and higher on Neon, reachable anonymously and unboundedly. The disclosure value is now prospective — a future engine-only rule would have been liftable by query string. The prompt's preview trap did not land: `ADMIN_API_TOKEN` is configured for Production AND Preview, so the inert window is local dev and the test runner only, pinned on both sides. Both generators' duplicate checks KEPT. Residue: [#768](https://github.com/znpruitt/cfb-app/issues/768) (should the parameter exist at all), [#769](https://github.com/znpruitt/cfb-app/issues/769) (`engine.ts` bypass sort drops the id tiebreak). **Owed to planning at merge:** `AGENTS.md` Season Launch invariant 4's "NOT admin-enforced today" clause and its "do not write new guidance that assumes the flag is privileged" instruction both go false — the registry entry names the exact sentences |
