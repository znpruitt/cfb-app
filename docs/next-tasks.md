...
| **2b** | Matchups odds footer | **#715** | `gameCardPresentation.ts`, `MatchupsWeekPanel.tsx` | **IMPLEMENTED AND REVIEWED — [PR #806](https://github.com/znpruitt/cfb-app/pull/806) open, not merged.** Scheduled rows render the compact line or content-bearing `Line not posted`; all other states remain footer-free. The formatter derives the side from signed spreads and prints the row's own name, never canonical `favorite`; only equal `0/0` sides are pick'em. Seven tests added, one existing assertion retargeted without weakening; two reviewer rounds leave no credible in-scope P0/P1/P2. Prompt `docs/prompts/platform-715-matchups-odds-footer-codex-v1.md` |
