...
| 8 | Compact header wrap at phone width | **#797** (carries **#758**) | `CompactGameScoreboard.tsx` | **QUEUED, kickoff written 2026-09-23.** At phone width the header may wrap only on `scheduled`, so on `live`/`awaiting`/`unavailable` the metadata span (`flex-auto min-w-0 overflow-clip`) absorbs the whole squeeze from a `flex-none` tag slot — and broadcast, last in the metadata order, clips first. #725 revealed a second tag on every state and #723 added the label that goes missing. **#758 folded in by owner 2026-09-22 — same condition, one asks the question and the other is its consequence.** `DESIGN.md` carries the ruling: the exemption follows the LAYOUT, not the status; at phone width cards stack, so the across-the-grid-row alignment the single-line contract protects cannot be violated. **Two things the issue itself gets wrong:** its citations predate #832, and it names ONE condition where there are THREE (`:258`, `:266`, `:274`) — widening only the `flex-wrap` grant permits a wrap without making either child take a full line. **The overflow has never been OBSERVED, only derived from the flex properties**; the browser harness (`npm run test:browser:required`) is the instrument, and "no clipping at shipped widths" is a valid outcome that ends the slice with a finding rather than a fix. Kickoff: [`docs/prompts/platform-797-header-wrap-codex-v1.md`](prompts/platform-797-header-wrap-codex-v1.md). |
...
| 5 | Third-column tier | **#726** | both panels | **UNBLOCKED BY #750's METHOD, still LAST.** Matchups (1372px) and Schedule (1320px) carry the same unmeasured provenance, and Schedule's arithmetic does not reproduce (Item 152). It applies #750's fixture and harness per surface rather than inheriting the arithmetic |
