...
| ✅ | ~~Matchups caller work~~ | ~~#723, #725~~ | `MatchupsWeekPanel.tsx` | **MERGED `539982ee` (PR #800) 2026-09-15; #723 and #725 closed. PROMOTED then ROLLED BACK 2026-09-15 — NOT LIVE**, collateral to #801's outage rollback rather than any fault of its own. Broadcast is enumerated at the call site for scheduled, live and awaiting — **`unavailable` EXCLUDED**, after the receipt refuted both premises my ruling rested on: a third caller (`selectors/gameWeek.ts:327-330`) enumerates and excludes it, making the precedent 2-1 rather than split; and `DESIGN.md` was never silent — the fifth state is defined in the SAME bullet, and the justification for `awaiting` carrying broadcast is scoped to a state the app STILL BELIEVES, which `No score reported` is defined as no longer being. **Planning had amended `DESIGN.md` to bless the component's behaviour and thereby INVERTED THE AUTHORITY — an implementation disagreeing with the canonical document is a defect in the implementation; reverted `7dcb31e9`.** The `hidden sm:inline-flex` secondary-tag cap is gone, #671's ruling one surface over. **#715 SPLIT OUT** — no `CombinedOdds` display formatter exists anywhere in `src/`, so it is not caller work; owner ruled `Pick'em · O/U 48.5`, and the formatter must render the ROW'S OWN name and never print `favorite`, which is a `canonicalName` and diverges from the row's `csvHome`/`csvAway` whenever the resolver canonicalises an alias. Residue: [#795](https://github.com/znpruitt/cfb-app/issues/795), [#796](https://github.com/znpruitt/cfb-app/issues/796), [#797](https://github.com/znpruitt/cfb-app/issues/797) (NOT #758 — different population), [#798](https://github.com/znpruitt/cfb-app/issues/798). **One owner-approved step-6 round, and its condition earned its keep:** the first rename was **as unfalsifiable as the name it replaced**, because a final-row assertion discriminates nothing when the caller gate and the component policy both suppress finals. Prompt `docs/prompts/platform-723-725-715-matchups-caller-work-codex-v1.md` |
| **2b** | Matchups odds footer | **#715** | `gameCardPresentation.ts`, `MatchupsWeekPanel.tsx` | **NEXT — kickoff written 2026-09-15, `docs/prompts/platform-715-matchups-odds-footer-codex-v1.md`. OWNER ASSIGNED TO CODEX**, not the Claude session in `cfb-app-codex` — planning has no channel to a Codex CLI, so the owner dispatches it. Split out of slice 2 at the receipt: the issue's "caller work only" claim is false, no `CombinedOdds` display formatter exists in `src/`. Owner ruled `Pick'em · O/U 48.5`. **The trap: `favorite` is a `canonicalName` and the row renders raw provider names, so the formatter must resolve the side from the signed spreads and print the ROW'S own name.** All three unshown cases measured at zero production instances across 305 records |
| **3** | Schedule presentation | **#728**, **#730**, **#731** | `GameWeekPanel.tsx` | **NEXT after #715** — ready, no decision owed. #728 is the slot confusion Item 173a already fixed on Featured |
