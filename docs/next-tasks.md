...
14. **A store failure during a pick now returns 500 instead of a clean refusal** (found by review
...
15. **The database connection pool never gives up waiting** (raised by review during PLATFORM-102,
...
16. ✅ **CLOSED 2026-08-13 — preview now gets its own database.** The owner configured the
...
17. ✅ **PLATFORM-095 — COMPLETE** (PR #475, `7d7b4c62`, 2026-08-13). Owner decisions taken during the preview
...
18. **PLATFORM-097 — assignment-method and draft-recovery states.** Split out of PLATFORM-095 after
...
19. ✅ **PLATFORM-096 — COMPLETE** (PR #476, `6b0b8eca`, 2026-08-14). Owner-designed 2026-08-13,
    ...
    **Scope boundary, from the owner's earlier ruling:** this is for corrections BEFORE publication.
    ...
    link after publication, which is why it read as absent. See item 20.
20. **PLATFORM-098 — the owner roster is the membership authority after publication.** Owner
...
21. ✅ **PLATFORM-099 — COMPLETE** (PR #477, `9537f7e8`, 2026-08-14). Re-derived from clean `main`
...
22. ✅ **PLATFORM-100 — COMPLETE** (PR #478, `c5293a14`, 2026-08-14). A confirmed roster spells
...
23. **Findings from the 2026-08-14 demo dry run** (main line held end to end; recorded so they are
...
24. ✅ **INSIGHTS-029 — stop suppression draining the feed. MERGED** via PR #479 (`49c76ee9`,
    ...
    **Known gap this leaves open, owned by items 25–26 below.** The loader serves up to
...
25. ⏸️ **INSIGHTS-018 — rotation and the NEW tag. DEFERRED, with an explicit trigger.** Branch
    ...
    **Trigger: take this once INSIGHTS-023 (item 26) has widened the pool past the feed limit.** Not
    a vague "later" — 023 is what creates the tail that rotation exists to serve, and item 24's gap
    has no other owner.
...
26. **INSIGHTS-023 — preseason breadth. Now BEFORE rotation, not after** (reordered 2026-08-15 when
...
27. **INSIGHTS-024 — active-owner scoping.** After breadth. Correctness, not volume: on its own it
...
28. **INSIGHTS-025 — rookie/returning claims (owner decision required, invariant amendment).**
    ...
    **The question the amendment turns on, for the owner:** is a confirmed preseason owner list
    ...
    PLATFORM-098 (item 20) editing it after assignment silently diverges from the roster. The stronger
...
29. **INSIGHTS-026 — the pulse: a scheduled digest, and the insights stream's EVENT SOURCE.**
...
30. **INSIGHTS-027 — preseason content generators (NEW content, not re-enabled content).** Also
...
31. **Then, in order: INSIGHTS-019** (diagnostic endpoint — worth taking FIRST, ahead of 023: it is
    ...
    later.
32. **PLATFORM-092 follow-ups** (recorded so they are not rediscovered): (a) ✅ **CLOSED by
...
33. **League deletion does not delete data — data-retention and future multi-tenant privacy.**
...
34. 🟡 **PLATFORM-101 — `?bypassSuppression=1` is an uncached, invariant-skipping flag with no admin
    ...
    **The product question to settle first:** should the flag require platform admin, or should it be
    deleted from the public route and kept only on an admin/debug surface? INSIGHTS-019 (item 31) is
...
35. **Pre-existing flaky test** (not from any campaign): `insights-suppression.test.ts` → "record at
    ...
    retry.
36. **PLATFORM-091 follow-ups** (not queued as work; recorded so they are not rediscovered):
...
37. Nonblocking operational observation (not implementation work): the passive **PLATFORM-086E1C2
