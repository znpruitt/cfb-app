...
| **1** | [#816](https://github.com/znpruitt/cfb-app/issues/816) | **NEXT — the audit's second validation gate, which has never run.** Canonical standings cache with `revalidate: false`, and the served snapshot lives in the deployment's data cache, so no replica query and no agent session can see it — a fresh rebuild is computable, the thing to compare it against is not. Admin-gated read-only route returning cached vs fresh with a per-team delta. **The hard part is that reading the cached value CREATES it when absent, and a snapshot the route just warmed agrees by construction** — the route must report hit vs miss or its clean result is unreadable. **RECONSTRUCTION 2026-09-18, owner decision.** v1 stopped at `bbd37393` on its precommitment: rounds 1, 2 and 4 each added ~3 instances of one class — *the detector claims more than it observes* — and round 4, whose purpose was to remove that class, introduced three more. The comparison core and guards drew zero findings in three passes; the detector drew all of them. **v2 rebuilds the detector to PUBLISH OBSERVATIONS and classify nothing by elimination**, keeps the reviewed remainder, and carries one core correction (the history digest sorts, while a consumer derives rank from array order). Kickoff `docs/prompts/platform-816-standings-cache-delta-claude-v2.md`; v1 findings in its closeout `1a29024f` and [#817](https://github.com/znpruitt/cfb-app/issues/817)-[#819](https://github.com/znpruitt/cfb-app/issues/819). **The gate is still un-run**: no authenticated call, nothing against production data, and that only happens post-merge. Its result decides whether [#693](https://github.com/znpruitt/cfb-app/issues/693) stays parked |
