...
| **4** | Overview column breakpoint | **#750** | `OverviewPanel.tsx` | **NEXT — OWNER RULED 2026-09-18, option 2: derive the number.** The 416px target is unmeasured mockup prose; measured, a worst-case row uses 268.094px and keeps 104.844px of slack in a 422.656px column, and below the threshold Overview renders two ~870px columns carrying ~350px of content. **REVISED 2026-09-18 to v2 after the receipt measured it:** the derived breakpoint is `1341px` against a shipped `1348px` — 7px, invisible. The waste is inside the ROW, which stretches with its column because the score is right-anchored. v2 caps the row's content width and left-aligns it, then lands the derived number. Content-sized columns were considered and REJECTED: live-polled scores and state-dependent tags would resize a track while a member watches. Kickoff `docs/prompts/platform-750-overview-column-breakpoint-codex-v2.md`. Planning amends `DESIGN.md:405-423` before merge, naming Overview as the only surface adopting the cap. Truncation is [#821](https://github.com/znpruitt/cfb-app/issues/821), an owner decision this slice must not pre-empt |
