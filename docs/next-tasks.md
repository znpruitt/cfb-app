...
| **2** | Matchups caller work | **#723**, **#725**, **#715** | `MatchupsWeekPanel.tsx` | **DISPATCHED 2026-09-15 to `cfb-app-codex`, awaiting read receipt** — `docs/prompts/platform-723-725-715-matchups-caller-work-codex-v1.md`. #725 is #671's selector-owned cap ruling one surface over. **#723 CLOSED BY OWNER RULING 2026-09-15 — Matchups carries broadcast.** It had been blocked by Item 87 INDEX row 30, whose premise was false, measured. Row 30 said the mockup omits broadcast on every Matchups row — it carries it on **1 of 10**, against Schedule's **3 of 13**, with each section split one-and-one across its two live rows. `DESIGN.md:201` binds broadcast per STATE with no surface exception, so it is a conformance fix and no owner decision is owed; row 30 is resolved with the counts. **Two things the kickoff raises that could still reshape the slice:** `statusLabel="SCH"` is hardcoded at the call site (possible fourth defect, report-only), and there is **no display formatter for `CombinedOdds` anywhere in `src/`** — so #715's "caller work only" claim is unproven and the receipt is asked to recommend split or keep |
