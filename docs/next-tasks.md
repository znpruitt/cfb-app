...
| 6 | [#774](https://github.com/znpruitt/cfb-app/issues/774) | the history `[year]` bound — **MERGED `4c939fbf` (PR #779) 2026-09-13; #774 closed.** **Built but NOT promoted** — auto-promotion is off. Gates green on the merged tree (5,328 pass, 0 fail); delta **+34** against the parent. Evidence in `docs/prompt-registry.md` → PLATFORM-774-HISTORY-YEAR-BOUND-CLAUDE-v1. **Worse than #770's and differently shaped**: `Number()` behind a floor made the accepted set DENSE, so no enumeration bounds it — reproduced live at **8 cache entries from 11 requests**, and the RSC page carried its own copy of the same parser and minted two more. **The ceiling is the league's OPERATING YEAR, not `currentYear + 1`** — an archive of an unfinished season cannot exist — **with a `listSeasonArchives` disjunct**, because bounding on the archive list alone would refuse `tsc`'s genuine gap years at 2019 and 2020 and convert a designed empty state into a `notFound()`. **Not a new finding: `rolloverTargeting.ts:83` names `2026.5` and PLATFORM-086F2H1R4 hardened the WRITE path against it; only the READ path was left open.** Corrected #774's own body — there are TWO tags, the per-year one is dead and the slug tag is what sweeps. **Both reviewers gathered against `a96be611`** — Codex clean; `/code-review high` three low, all three accepted (the disjunct now guards on the floor so a store outage cannot turn a sub-2000 400 into a 500; the page suite asserts the `notFound()` digest rather than any rejection, which 5 tests prove discriminates; two admin-gated debug routes added to residue). Residue [#776](https://github.com/znpruitt/cfb-app/issues/776) (empty-state copy, coupled to this ceiling); `debug/archive-integrity` and `debug/archive-audit` reach the same cache unbounded with an UNVALIDATED slug, so their entries are permanent rather than swept — #770's shape, not #774's; mixed 400/404 shapes noted, not filed |
| 7 | [#778](https://github.com/znpruitt/cfb-app/issues/778) | the archive **slug** validation — **PENDING REVIEW** on `claude/778-archive-slug-validation` at `a4d53217`; gates green (lint/tsc/test each exit 0, delta **+12 measured at both ends**, 5,328 -> 5,340, zero failures either end). Evidence in `docs/prompt-registry.md` -> PLATFORM-778-ARCHIVE-SLUG-VALIDATION-CLAUDE-v1. **Reproduced live before the fix**: each debug route answered `404 No archive found` for a nonexistent slug and minted an on-disk entry anyway, tagged `archive:<slug>` + `archive:<slug>:<year>`; after, neither mints and a real slug still does. **The refusal went into the AUTHORITY, covering BOTH `getSeasonArchive` and `listSeasonArchives`** — the sibling carried the decision, not the call-site ratio: `listSeasonArchives` mints a SEPARATE cache identity under the same tag, so a guard in `getSeasonArchive` alone leaves it open for the next caller. It returns `null`/`[]` rather than throwing, because `null` is already the contract's answer for "this league has no archive". Routes keep an explicit `404 league-not-found`; mutation shows the two jobs are separable. **Three corrections to the prompt's own survey**: `revalidate: false` is a one-year TTL (`CACHE_ONE_YEAR`), not "no expiry"; the call-site split is **8 route-param / 3 query-string / 4 library**, not 11/4, so **two of three** query-string callers forgot; and the member-facing surface does **NOT** share the hole — all 8 route-param sites probed against a non-degenerate registry minted zero, with a positive control. **NOT an instance of a class** — all 4 cross-request caches in `src/` classified; standings is globally reclaimable and insights is TTL-bounded, neither widened. **The test bill was quoted as two files and was three** — the recap loader suite was missed, fittingly the one site protected by accident rather than a check. Residue: existing junk entries are not enumerable or purgeable; an absent/malformed registry now makes archives unreadable (pinned by test, unreachable in production); both routes echo the raw slug into their 404 body |
