...
- Attachment Regression Tests First (ODDS-001-ATTACHMENT-REGRESSION-TESTS-v1): test-only coverage locking schedule-canonical score/odds attachment + eligibility before the odds-matching fix. Documents the current pair-only odds misattach risk (same-pair rematch fan-out, duplicate provider events) with passing "current behavior" tests plus `test.skip` intended-invariant tests that **require ODDS-002-EVENT-DATE-AWARE-ATTACHMENT-v1** (event-centric/date-aware odds attachment). **ODDS-002-EVENT-DATE-AWARE-ATTACHMENT-v1 is the follow-up** that must make odds attachment event/date-aware and flip those skipped tests green.
