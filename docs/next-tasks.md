...
| **4c** | Abbreviation width fallback | **#832** | `CompactGameScoreboard.tsx` + callers | **NEXT — kickoff `docs/prompts/platform-832-abbreviation-fallback-codex-v1.md` (2026-09-20). #831 merged `41f688c5`, so the lookup ships with no consumer and this slice is it. — OWNER RULED 2026-09-20 on [#821](https://github.com/znpruitt/cfb-app/issues/821): a team name is NEVER truncated; the abbreviation is the fallback.** `DESIGN.md` amended the same day. CSS cannot measure text, so the swap is driven by container tier plus a per-name marker — it fires at a WIDTH, not at the clipping pixel. Per name or per row is the slice's to pin |
