...
| **4** | **47** ([#627](https://github.com/znpruitt/cfb-app/issues/627)) | admin authorization for the Insights diagnostic bypass — **IN REVIEW** on `claude/627-insights-bypass-authorization`; both reviewers gathered 2026-09-13. Evidence in `docs/prompt-registry.md` → PLATFORM-627-INSIGHTS-BYPASS-AUTHORIZATION-CLAUDE-v1. **Blocker on the owner:** review found `parseYear` has no upper bound, so the anonymous uncached rebuild this item was filed for stays reachable via `?year=` without the parameter — needs an issue, and bounding it conflicts with this prompt's byte-identical acceptance boundary |
