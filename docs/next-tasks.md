...
| **4b** | Abbreviation lookup | **#831** | new store + fetch script | **NEXT — kickoff `docs/prompts/platform-831-abbreviation-lookup-codex-v1.md` (2026-09-20). Prerequisite for #832.** CFBD has abbreviations for all 238 renderable names (2-4 chars, zero gaps); this app has 138 FBS teams only. **The catalog is the wrong home:** `getTeamDatabaseItems` is called at 19 sites across 17 files (planning first said 41 by counting imports; corrected on the issue) and THREE are draft surfaces, so widening it would make FCS schools draftable |
| 4c | Abbreviation width fallback | **#832** | shared scoreboard + selectors | **QUEUED behind #831 — OWNER RULED 2026-09-20 on [#821](https://github.com/znpruitt/cfb-app/issues/821): a team name is NEVER truncated; the abbreviation is the fallback.** `DESIGN.md` amended the same day. CSS cannot measure text, so the swap is driven by container tier plus a per-name marker — it fires at a WIDTH, not at the clipping pixel. Per name or per row is the slice's to pin |
