...
| **3** | Schedule presentation | **#728**, **#730**, **#731** | `GameWeekPanel.tsx` | **NEXT — KICKOFF WRITTEN 2026-09-16**, `docs/prompts/platform-728-730-731-schedule-presentation-codex-v1.md`. #728 is the slot confusion Item 173a already fixed on Featured. **"No decision owed" was wrong:** #730 removes the per-game separator that `DESIGN.md:272-275` names as the date rule's reference, so that clause needs an owner ruling and a planning amendment; and the divider lives in the SHARED `CompactGameScoreboard`, so the removal mechanism is ruled at receipt |
