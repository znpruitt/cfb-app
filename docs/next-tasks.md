...
> **PROMOTION 2026-09-20 — `dpl_48dpusfVKAmRGVMQ6kS9VXcocWwn`, and it carried a production fix that
> is NOT a code change.** Eight merged slices went live in one promotion: #708, #728/#730/#731,
> #729, #816, #750, #663, #831, and #833/#794. Each verified as an ancestor of the promoted build
> rather than assumed from its merge date.
>
> **The build is a REBUILD of `c03042ad`, not the build that commit originally produced, and the
> difference is the point.** `NEXT_PUBLIC_DEBUG` was set for Production and the upstream debug
> logger was live on production ([#697](https://github.com/znpruitt/cfb-app/issues/697)) — measured
> that day at 65 of 192 production log lines carrying strings that only emit when the gate is open.
> The variable was one record targeting all three environments, so it was PATCHED to
> `['preview','development']` rather than removed; `vercel env rm <name> production` would have
> taken the whole record, and the value is encrypted, so Preview and Development could not have been
> restored.
>
> **`NEXT_PUBLIC_*` inlines at BUILD time, so the env change was inert until something rebuilt.**
> The first promotion shipped the eight slices and kept the debug logging, because that build
> predated the env change. The sequence that works is **set the variable → rebuild → promote the
> rebuild**, and it is worth writing down because the intuitive order silently does nothing.
>
> **The verification nearly went the wrong way, and the reason generalises.** Grepping the client
> bundle for debug-only strings is BACKWARDS as a test: when the variable is SET, Next inlines the
> literal and folds the comparison, so the flag name disappears; when it is UNSET, the lookup stays
> at runtime and the guarded strings REMAIN, inert. Their presence had been read as "still on" and
> reported as a failed rebuild. **Read the quantity, not the residue:** the deployment's env key
> list (159 keys, no DEBUG key) and the literal `NEXT_PUBLIC_DEBUG` surviving in the bundle are both
> direct reads of the state; the guarded strings are not.
>
> **#697 is NOT closed.** Preview and Development keep the flag, and `fetchUpstream.ts:392-405` still
> copies every provider response header verbatim into the log. The code fix — redact response
> headers the way `toHeaderObject:104-113` already redacts `authorization` — is still the ask.
...
| 2 | [#755](https://github.com/znpruitt/cfb-app/issues/755) | the CFBD `/info` quota probe had no deadline — **MERGED `7b70ee6c` (PR #765) 2026-09-13; #755 closed.** **PROMOTED 2026-09-14** in `c6b4137` — live. Both phases now share a 40,000 ms ceiling via `fetchUpstreamJson`. Reproduced at **301.3s** before undici's stock 300s `headersTimeout`, so the hang was two 300s timers rather than none. Value derived from the `provider-usage` series, not borrowed from the `/games` ceiling, and held in its own constant after both reviewers found the alias rebuilt the coupling. Residue: [#764](https://github.com/znpruitt/cfb-app/issues/764) (`systemHealth` races its loader without aborting it); `cfbd` pacing on the probe is undecided and deliberately not swept in |
| 3 | **20** ([#625](https://github.com/znpruitt/cfb-app/issues/625)) | bounded database waits — **MERGED `ce5e1b4d` (PR #789) 2026-09-14; #625 closed. PROMOTED 2026-09-15, THEN ROLLED BACK THE SAME DAY — NOT LIVE.** **FORWARD FIX MERGED `3f8f19f4` ([PR #802](https://github.com/znpruitt/cfb-app/pull/802)) 2026-09-16** — the opener is one template literal; merged tree identical to the gated tip, 5,392 pass / 0 fail. **PROMOTED 2026-09-16 — LIVE.** `turfwar.games` resolves to `dpl_BjGVXsqtaYqdDeAkm5vxeL4ERCkn`, built from `3f8f19f4`, which shipped #789, #791, #800, #806 and #802 together. First look, 14:05-14:09 CDT, 31 requests: authenticated `/league/tsc` pages and their APIs returned 200, with zero `42601`, `57014`, `55P03` or `store-unavailable`. That window is minutes long and does not yet show whether the +2 round trips raise contention on `max: 3`. [#801](https://github.com/znpruitt/cfb-app/issues/801) stays open — mechanism unestablished, and the six other two-literal `+` sites are correct in the current build but unguarded. It caused a **total production outage** and was reverted by promoting `c6b41378`. **The code was correct and the BUILD was not:** the minifier drops the semicolon-and-space between the first template literal's final interpolation and its closing backtick, so the emitted bundle carried `statement_timeout = 15000set local lock_timeout` — valid JavaScript, invalid SQL, Postgres `42601`, 29 errors in 2 minutes across every authenticated page that reads `app_state`. Filed as [#801](https://github.com/znpruitt/cfb-app/issues/801), which also carries the sweep: the same construct appears in ≥10 other places in `src/`, sampled as cosmetic but **not measured**. **THE UNBOUNDED-WAIT HANG #625 CLOSED IS OPEN AGAIN IN PRODUCTION** until the fix ships. When it does: every store statement now runs inside a bounded transaction, **+2 round trips**, and a pooled client holds three round trips instead of one, which RAISES contention on `max: 3`. If store latency or `store-unavailable` rates move, this is the first place to look. Residue [#788](https://github.com/znpruitt/cfb-app/issues/788) (payload statement, `COMMIT` and `ROLLBACK` still unbounded client-side; four 15 s bounds compose to ~60 s for one `getAppState`) and [#785](https://github.com/znpruitt/cfb-app/issues/785) (`55P03` and `57014` flattened to one code). Gates green on the MERGED tree, whose tree hash is identical to the gated branch tip (5,369 pass, 0 fail; delta **+15** from 5,354), **both reviewers gathered twice** — against `c1d5383b` and again against `fd61d0c3`, Codex clean on the second. Evidence in `docs/prompt-registry.md` -> PLATFORM-625-DATABASE-TIMEOUTS-CLAUDE-v1. **The values verified; the DELIVERY CHANNEL did not, and that was the slice.** Of the three mechanisms the prompt offered, the two that look most correct fail worst: the `pg` client option is **silently inert** on both endpoints (Neon's proxy discards the startup parameter) and `?options=` is a **hard `08P01` connect failure** on the pooler, i.e. a total outage. A session `SET` is **17/18 wrong** through a transaction pooler. `SET LOCAL` riding with each `BEGIN` is the only mechanism that applies on both, so every store statement now runs inside a bounded transaction — **+2 round trips, and a pooled client held three round trips instead of one, which RAISES contention on `max: 3`**. **Acceptance bullet 3 is recorded UNMET, not relaxed** (owner amendment `bec9147e`): Postgres distinguishes `55P03` from `57014`, but every transactional caller flattens both into `store-unavailable` with a fixed code — [#785](https://github.com/znpruitt/cfb-app/issues/785). **[#595](https://github.com/znpruitt/cfb-app/issues/595) is BOUNDED, NOT FIXED** — the nested-read structure is unchanged, only its blast radius; `standingsCacheWarmer.ts:125` is that structure on the longest-holding lock, the one path where `lock_timeout` changes behaviour rather than bounding a hang, and its fallback logs nothing and reports success. Review found two defects in the new code, both reproduced against production: **a checked-out pooled client had no `'error'` listener**, so a socket reset mid-query took the process down; and **the opener is the one statement no server-side bound can cover, because it is the statement that installs them** (measured 20,062 ms against a 5,000 ms connect timeout). Residue [#788](https://github.com/znpruitt/cfb-app/issues/788) — a per-request deadline; the payload statement, `COMMIT` and `ROLLBACK` are still unbounded client-side, and four 15 s bounds compose to ~60 s for one `getAppState`. A naive global `query_timeout` is NOT the fix: measured, it **poisons the pool** |
| ✅ | ~~47~~ ([#627](https://github.com/znpruitt/cfb-app/issues/627)) | admin authorization for the Insights diagnostic bypass — **MERGED `d7a97917` (PR #772); #627 closed.** Status corrected 2026-09-16; the text below is the pre-merge record — both reviewers gathered and remediated 2026-09-13. Evidence in `docs/prompt-registry.md` → PLATFORM-627-INSIGHTS-BYPASS-AUTHORIZATION-CLAUDE-v1. **#627 closes the bypass-specific rebuild, NOT the anonymous unbounded one it was filed for** — `parseYear` has no upper bound, so `?year=` reaches it without the parameter: [#770](https://github.com/znpruitt/cfb-app/issues/770). Residue [#768](https://github.com/znpruitt/cfb-app/issues/768), [#769](https://github.com/znpruitt/cfb-app/issues/769), [#771](https://github.com/znpruitt/cfb-app/issues/771) |
| ✅ | [#770](https://github.com/znpruitt/cfb-app/issues/770) | the Insights `?year=` bound — **MERGED `31fc892a` (PR #775); #770 closed.** Status corrected 2026-09-16; the text below is the pre-merge record — gates green, **both reviewers gathered against `32b226e1` and remediated 2026-09-13** (Codex clean; `/code-review` two low, one fixed and one adjudicated as pre-existing). Evidence in `docs/prompt-registry.md` → PLATFORM-770-INSIGHTS-YEAR-BOUND-CLAUDE-v1. A caller-supplied year is bounded to `[MIN_SEASON_YEAR, currentYear + 1]` **or the league's own operating year**, and a rejected one returns 400 instead of falling back. **The prompt's second-consumer claim was refuted with a control** — `isWeeklyRecapActiveSeason` is an equality check, so the recap path is inert for an absurd year; **the real second consumer is canonical standings**, `revalidate: false`, and each distinct year minted TWO cache identities. **Reachability: all three production leagues carry a password**, so the live vector is a password-holding member, not an anonymous caller. Residue [#774](https://github.com/znpruitt/cfb-app/issues/774) — same shape on `/api/history/[slug]/[year]`, whose cached `null` has no TTL |
| 6 | [#774](https://github.com/znpruitt/cfb-app/issues/774) | the history `[year]` bound — **MERGED `4c939fbf` (PR #779) 2026-09-13; #774 closed.** **PROMOTED 2026-09-14** in `c6b4137` — live. Gates green on the merged tree (5,328 pass, 0 fail); delta **+34** against the parent. Evidence in `docs/prompt-registry.md` → PLATFORM-774-HISTORY-YEAR-BOUND-CLAUDE-v1. **Worse than #770's and differently shaped**: `Number()` behind a floor made the accepted set DENSE, so no enumeration bounds it — reproduced live at **8 cache entries from 11 requests**, and the RSC page carried its own copy of the same parser and minted two more. **The ceiling is the league's OPERATING YEAR, not `currentYear + 1`** — an archive of an unfinished season cannot exist — **with a `listSeasonArchives` disjunct**, because bounding on the archive list alone would refuse `tsc`'s genuine gap years at 2019 and 2020 and convert a designed empty state into a `notFound()`. **Not a new finding: `rolloverTargeting.ts:83` names `2026.5` and PLATFORM-086F2H1R4 hardened the WRITE path against it; only the READ path was left open.** Corrected #774's own body — there are TWO tags, the per-year one is dead and the slug tag is what sweeps. **Both reviewers gathered against `a96be611`** — Codex clean; `/code-review high` three low, all three accepted (the disjunct now guards on the floor so a store outage cannot turn a sub-2000 400 into a 500; the page suite asserts the `notFound()` digest rather than any rejection, which 5 tests prove discriminates; two admin-gated debug routes added to residue). Follow-up [#776](https://github.com/znpruitt/cfb-app/issues/776) **MERGED `9764b70f` (PR #791) 2026-09-15, closed; PROMOTED then ROLLED BACK 2026-09-15** (collateral to #801): the false range sentence is gone while the gap-year empty state remains. `debug/archive-integrity` and `debug/archive-audit` still reach the same cache unbounded with an UNVALIDATED slug, so their entries are permanent rather than swept — #770's shape, not #774's; mixed 400/404 shapes noted, not filed |
| 7 | [#778](https://github.com/znpruitt/cfb-app/issues/778) | archive **slug** validation — **MERGED `99a41fbe` (PR #783) 2026-09-14; #778 closed.** **PROMOTED 2026-09-14** in `c6b4137` — live. Evidence in `docs/prompt-registry.md` -> PLATFORM-778-ARCHIVE-SLUG-VALIDATION-CLAUDE-v1. Residue [#782](https://github.com/znpruitt/cfb-app/issues/782) — Route-Handler registry reads per archive year; owner-ruled to accept and ship |
| ✅ | [#816](https://github.com/znpruitt/cfb-app/issues/816) | standings cache-versus-fresh diagnostic — **MERGED `e920e7de` (PR #826) 2026-09-19; #816 closed. **LIVE 2026-09-20** — promoted in `dpl_48dpusfVKAmRGVMQ6kS9VXcocWwn`, a rebuild of `c03042ad` (verified: that merge is an ancestor of the promoted build).** v1 stopped on its precommitment and was reconstructed; v2 publishes observations and classifies nothing by elimination. Four open findings in the precommitment class ship documented — [#817](https://github.com/znpruitt/cfb-app/issues/817), [#822](https://github.com/znpruitt/cfb-app/issues/822), [#823](https://github.com/znpruitt/cfb-app/issues/823), [#824](https://github.com/znpruitt/cfb-app/issues/824) — plus a test gap on [#820](https://github.com/znpruitt/cfb-app/issues/820). **PROMOTED 2026-09-19 and THE GATE HAS RUN.** Against production on a Week 3 Saturday: `tsc`/2026 returned two genuine `hit`s with every owner matching, and the snapshot's `generatedAt` advanced 6m 42s between them without either call publishing it — the invalidate→rewarm cycle observed working. Also found: `unstable_cache` keys include the callback's minified source, so a promotion that changes it orphans every standings snapshot. **[#693](https://github.com/znpruitt/cfb-app/issues/693) STAYS PARKED as a latent risk** — the failure it describes cannot be shown by sampling a healthy system. Evidence on #693 |
| ✅ | [#663](https://github.com/znpruitt/cfb-app/issues/663) | targeted schedule repairs never converge (audit A1) — **MERGED `74959f26` (PR #835) 2026-09-20; #663 closed. **LIVE 2026-09-20** — promoted in `dpl_48dpusfVKAmRGVMQ6kS9VXcocWwn`, a rebuild of `c03042ad` (verified: that merge is an ancestor of the promoted build).** Ruled AGGREGATE-ONLY: the targeted partition path is removed, so schedule has one target and a repair lands in the key every reader already reads. Evidence: 7 whole-year aggregates and 0 partitions in production, no in-repo caller could construct a targeted request, and the canonical precedence had been written out three times. `AGENTS.md`'s two invariants amended in `91aa30a3` — intent kept, mechanism dropped, both saying not to restore it. Residue [#833](https://github.com/znpruitt/cfb-app/issues/833) |
| ✅ | [#833](https://github.com/znpruitt/cfb-app/issues/833) + [#794](https://github.com/znpruitt/cfb-app/issues/794) | **MERGED `c03042ad` ([PR #839](https://github.com/znpruitt/cfb-app/pull/839)) 2026-09-20; both closed. **LIVE 2026-09-20** — promoted in `dpl_48dpusfVKAmRGVMQ6kS9VXcocWwn`, a rebuild of `c03042ad` (verified: that merge is an ancestor of the promoted build).** #663's dead symbols removed, the `LeagueStatusPanel` record-versus-row defect routed through the shared `canonicalScheduleAggregateServes` predicate, and the historical-repair refusal now fails closed on a malformed registry. **[#813](https://github.com/znpruitt/cfb-app/issues/813) was REMOVED from this branch and goes back — the stop was the deliverable.** It shipped a per-field `eventKey` guard that two review rounds walked around four times: `postseason-classify.ts:216` reads `row.homeTeam.trim()` inside `looksEmptyRow`, on the FIRST line of `classifyScheduleRow` (`:275`), ahead of the guard. **The unit is the ROW, not the field** — and planning's v1 ruling chose the field, so the miss is planning's. v2 kickoff `docs/prompts/platform-813-row-validation-claude-v2.md` (`56169efe`) validates at the `loadCachedScheduleItems` boundary, which has **13 production consumers**. Sibling filed: [#838](https://github.com/znpruitt/cfb-app/issues/838) |
| ✅ | [#875](https://github.com/znpruitt/cfb-app/issues/875) | a test ended by exhausting a budget rather than by asserting — **MERGED `e2b3ac90` (PR #878) 2026-09-25; #875 closed manually, the merge did not.** Tree hash verified identical to the gated commit. The test went 713-883 ms → **9.17-10.95 ms**, the file 1833-1959 ms → **920-1186 ms**. `:743` keeps its 714-904 ms BY RULING, both rejected alternatives recorded. **Three findings outlived the fix.** Planning's central trap was wrong — it said there was no clock to drive while acceptance 1 instructed mocking `setTimeout`, which creates it; the real trap is the inverse, ticking advances the mocked `Date` and in that file `Date` IS what the budget assertions read. **A mutation reddened the wrong assertion**: dropping `'setTimeout'` reddens the tick CAP, not the real-clock assertion, because the loop burns ticks in microseconds while real sleeps are pending — stopping there would have reported acceptance 1 as proven when it was not. And **`# fail 0` is real but the runner exits 1**: `--test-timeout` is per FILE, so a file summing past 30s is cancelled mid-flight — the summary is what lies, not the gate. Residue: 56 `Date`-only enables in `rankings/__tests__/`, recorded as UNMEASURED rather than swept. `stall.test.ts:877` mocks `Date` but not `setTimeout`, so ~750 ms of REAL wall-clock runs against a 30 s file budget and exhaustion kills the FILE rather than failing an assertion. Observed once in 350 runs. **#872's `tickUntilSettled` does NOT transfer** — there is no clock to advance, so the fix must BOUND the wait. **The positive control is the hard part:** the failure cannot be induced by shrinking a budget, only by starving the process, and “none that is honest” is an acceptable receipt answer where a fabricated control is not. Kickoff: [`docs/prompts/platform-875-real-time-stall-claude-v1.md`](prompts/platform-875-real-time-stall-claude-v1.md). |
| 1 | [#836](https://github.com/znpruitt/cfb-app/issues/836) | a malformed registry reads as empty, and one path WRITES OVER IT — **QUEUED, kickoff written 2026-09-26. Highest-severity open platform item: a fail-open on a data-integrity check.** `getLeagues()` flattens absent and malformed alike, so league creation's duplicate-slug check (`admin/leagues/route.ts:67`) passes vacuously and a new league adopts the previous occupant's rosters, drafts and archives — the consequence is already written in the `PLATFORM-086F2I` comment directly below it. **The issue names ONE fail-open; planning found TWO.** `mutateRegistry` (`leagueRegistry.ts:92-93`) repeats the collapse INSIDE the transaction, so `addLeague` writes `[newLeague]` over the corrupt value — **converting a recoverable corruption into an unrecoverable registry that reports healthy**, with every other league's data surviving under its slug and unreachable. Fixing only the route leaves the destructive half. **The file contains both the guard and its violation twenty lines apart:** `readLeagueRegistry`'s docblock says classifying malformed as empty is “the collapse this reader exists to prevent”. Precedent is settled — `seasonArchive.ts:193-209` refuses on malformed rather than reading as none. `getLeagues()` itself stays UNCHANGED; 69 modules depend on its array contract. Kickoff: [`docs/prompts/platform-836-registry-fail-closed-claude-v1.md`](prompts/platform-836-registry-fail-closed-claude-v1.md). |
| ⛔ | [#693](https://github.com/znpruitt/cfb-app/issues/693) | a failed standings invalidation can leave results stale indefinitely — **KEPT PARKED by owner decision 2026-09-16; not re-dispatched, and #805 is not scheduled.** **SHELVED 2026-09-15 at `355cd286`, NOT MERGED. Production retains the original silent-failure behaviour.** Stopped by a **precommitted rule agreed before the evidence arrived** — *another HIGH from this round's reviews and we shelve* — which Codex's P1 triggered. The lane held to it and declined both available arguments, including the true-but-irrelevant one that the finding was pre-existing rather than round-introduced. **Seven rounds each ended with "this is the last" and then a reason the next was different; the precommitment is the only thing that broke it.** The stopping finding is **#693's own defect class at the recording layer**: `recordPendingStandingsInvalidation` swallows its failure, and the loss is CORRELATED with the condition that creates it — the walk fails `registry-failed` when `readLeagueRegistry` can't read `app_state`, and the recorder writes `app_state`. **The recurring defect, named once: the module publishes certainty where the truth is unknown** — and the correct pattern (`attempted: null`) was in this module from round 1 and then violated four times. **Full shelf record is a comment on #693**, including what STOPPED being a question (the token generation guard, the compile-time tile gate, the two-trigger reachability argument, the drain's placement) so a successor does not re-litigate it. Diagnosis to start from: [#805](https://github.com/znpruitt/cfb-app/issues/805) — a per-invocation receipt is the wrong home for standing state; staleness can be mitigated by qualifying a claim, a race cannot. **RECONSTRUCTION HISTORY 2026-09-15, owner decision: rebuild the pending-state MODULE, keep the reviewed remainder.** Prompt `docs/prompts/platform-693-pending-state-reconstruction-claude-v2.md`. Two remediation rounds and both reviewers still returned credible P1/P2 findings, which is `AGENTS.md`'s literal reconstruction trigger; **round 2 also required owner approval under step 6 and never got it — planning failed to count the rounds.** Rebuilt: `standingsInvalidationPending.ts` and its two test files. Kept: both triggers, the reporting seam, the receipt plumbing and the authority changes, all confirmed by both reviewers in round 2. v1 history below, retained because the reconstruction carries every accepted finding forward as specification. **ORIGINAL v1 RECORD:** implemented, awaiting both reviews on `claude/693-standings-invalidation` at `4efd3c19`. Gates green, each its own command: `lint:all` 0, `tsc --noEmit` 0, `npm test` 0; delta **5,372 → 5,385 pass, 0 fail at both ends**. Prompt `docs/prompts/platform-693-standings-invalidation-claude-v1.md`. **THE RECEIPT MOVED THE SCOPE, AND THE PROMPT'S SCOPE WAS WRONG.** I scoped the fix to two blocks in `api/schedule/route.ts`; the lane found a third identical site at `lib/schedule/fullSeasonScheduleRefresh.ts:239` and — the fact that decided it — `cron/schedule-refresh/route.ts:548` calls `refreshFullSeasonSchedule` **directly**, so the unattended daily run reaches ONLY the site my SCOPE excluded. Three of that helper's four callers never touch either in-scope block. Ruled **(b)**: all three, which share one seam, `invalidateStandingsForYearReporting`. **Surface is `schedulerExecutionStatus`, not `provider-refresh-status`** — CARRIES forbids marking a succeeded refresh failed, and the refresh DID succeed; only the cache bust failed. The lane then improved on my ruling's carrier: `scoreSweepFailedPartitions` is an existing post-commit non-fatal outcome already riding `FullSeasonScheduleRefreshResult` through `cronExecutionLog`, so it followed that live chain rather than mirroring season-rollover's shape from a different contract. **Over-invalidation REJECTED, and recorded on the seam as deliberate** — `insightsCacheTags` (`loadInsights.ts:206-211`) literally returns `[ALL_STANDINGS_TAG, standingsSlugTag(slug), standingsYearTag(slug, year)]`, so busting the all-tag on every year-scoped schedule commit recomputes **every league's Insights**; the blast radius would be unrelated to the change, which no rebuild measurement could fix. **Counts only, never slugs** — the first implementation returned `failedSlugs` and two allowlists tripped correctly, because the value serializes into `cronExecutionLog`, whose own policy on `invalidLifecycleTargets` says *a count only*. `attempted: null` on `registry-failed` is deliberate: the population is UNKNOWN, and 0 would assert there were no leagues. **Highest-risk part of the diff, found by the lane and not by a gate: adding `partial` to the per-year union broke `aggregateScheduleCronResult`**, which matched no branch and fell through to `skipped` — an all-partial run reported that NOTHING RAN for a run that committed rows for every target year, and a mixed run reported clean `success`. Fixed, pinned by four tests including a positive control on the `skipped` path. **The `unstable_cache` bypass under `node:test` means staleness cannot be reproduced in-process; stated in the test file and the commit, NOT worked around.** Residue [#792](https://github.com/znpruitt/cfb-app/issues/792) — the two route-local HTTP blocks log structurally but write no durable receipt, because giving that path one means building its reader too |
| ✅ | [#757a](https://github.com/znpruitt/cfb-app/issues/757) | schedule presentation becomes its own scheduled job — **MERGED `d3874dc6` ([PR #863](https://github.com/znpruitt/cfb-app/pull/863)) 2026-09-23; #757 stays OPEN (757a is one slice of it).** Standalone `/api/cron/schedule-presentation`, Tuesdays 13:00 UTC, own receipt and System Health row; year selection relocated to `lib/activeSeasonTargets.ts`; all three inline callers intact, so the merge changes no behaviour. **SHIPS DORMANT — not in `vercel.json`, no QStash schedule.** **INSTALLED IN PRODUCTION 2026-09-24** — `turfwar-schedule-presentation-weekly`, Tuesdays 13:00 UTC, after production was promoted to `37354e6d` (carries `6f2c8f5a`, so the #861 gate passed). **757b stays gated until one standalone receipt is OBSERVED**, not merely until the schedule exists — first delivery Tuesday 2026-09-29. **The #861 gate is DISCHARGED (merged `41652614`, PR #865) — and the condition was never MERGE, it is PROMOTION of `6f2c8f5a`.** Auto-promotion is off, so between merge and promotion production still serves the pre-fix job and installing the schedule then is the exposure the gate existed for. §8i carries a runnable predicate (`--is-ancestor` against the promoted deployment's `githubCommitSha`; `vercel inspect` does NOT print it). Between promotion and install, System Health reports `schedule-presentation` delivery as missing — expected, per §8i. **Five remediation rounds where `AGENTS.md` allows one, self-reported**; the rule defect it exposed is now recorded in `AGENTS.md` (`cb43d6b5`) — the narrow-defect exception regenerates itself, and the sizing signal fired first (~2,700 insertions against an approved ~1,500, before round 1). Follow-ups: #861, 757a2, 757b (gated on one observed receipt), #857, #858 |
| ✅ | [#861](https://github.com/znpruitt/cfb-app/issues/861) | the presentation budget decided on a stale elapsed — **MERGED `41652614` (PR #865) 2026-09-24; #861 closed. NOT LIVE until promoted.** One remediation round, both reviewers clean. `elapsedMs` is captured at `route.ts:421`, `venueRefreshDue()` is awaited at `:434`, and the admission check at `:438` reuses the pre-await value — under a degraded store, up to 15s under-counted, once per admission (the loop re-reads the clock each iteration, so it does not compound). **THE TRAP: do NOT fold a store term into the reservation.** That was implemented and reverted in 757a round 4 — a 45s allowance makes an owed venue leg reserve 287s against a 250s budget, so no second year could ever start, which is round 1's starvation exactly; two existing tests caught it. Re-measuring is safe where inflating is not, because the reservation is unchanged and the first year is ungoverned regardless. **Follow-up [#866](https://github.com/znpruitt/cfb-app/issues/866) MERGED `119fa6d2` (PR #867) 2026-09-24, from a post-merge review** — the bounded store read is **~60s, four composed bounds, not 15s**, and that figure is a FLOOR: 242s is CFBD-only, so under the same degradation the year's own round trips are slow too and the ceiling is reachable at every figure. **Planning's escalation ("the worst case breaches 300s") was REFUTED at review** — it composed the store read's terms and left the year's uncomposed, which is recorded in `AGENTS.md`. The surviving claim is the narrow true one: **the budget admits a year it was designed to refuse**, which #861 repairs. **The lane's own guard was VACUOUS and it caught it before any reviewer** — a check that every test name cited in a comment resolves, whose v1 grepped the test file's TEXT and passed under its own mutation, because the guard's comment quoted the dangling name to explain itself. Third instance of that shape here; recorded in `AGENTS.md` (`cafaf948`). It also improved §8i beyond what planning specified: `--is-ancestor`, not SHA equality, which would be wrong for every later promotion. Kickoff: [`docs/prompts/platform-861-elapsed-capture-claude-v1.md`](prompts/platform-861-elapsed-capture-claude-v1.md). |
...
| --- | --- | --- | --- | --- |
| ✅ | ~~Matchups derived state~~ | ~~#722, #724~~ | `matchups.ts`, `ownerView.ts`, selector seams | **MERGED `ebc4ec79` (PR #777) 2026-09-13**, with the `unavailable`-eligibility remediation merged `c6b41378` (PR #784) 2026-09-14. **#712 and #766 closed consequentially.** The owner-ruling follow-ups #776 and #780 merged `9764b70f` (PR #791) 2026-09-15 |
| ✅ | ~~Matchups caller work~~ | ~~#723, #725~~ | `MatchupsWeekPanel.tsx` | **MERGED `539982ee` (PR #800) 2026-09-15; #723 and #725 closed. PROMOTED then ROLLED BACK 2026-09-15 — NOT LIVE**, collateral to #801's outage rollback rather than any fault of its own. Broadcast is enumerated at the call site for scheduled, live and awaiting — **`unavailable` EXCLUDED**, after the receipt refuted both premises my ruling rested on: a third caller (`selectors/gameWeek.ts:327-330`) enumerates and excludes it, making the precedent 2-1 rather than split; and `DESIGN.md` was never silent — the fifth state is defined in the SAME bullet, and the justification for `awaiting` carrying broadcast is scoped to a state the app STILL BELIEVES, which `No score reported` is defined as no longer being. **Planning had amended `DESIGN.md` to bless the component's behaviour and thereby INVERTED THE AUTHORITY — an implementation disagreeing with the canonical document is a defect in the implementation; reverted `7dcb31e9`.** The `hidden sm:inline-flex` secondary-tag cap is gone, #671's ruling one surface over. **#715 SPLIT OUT** — three `CombinedOdds` formatters existed, but none fit its compact row-name and explicit-empty-content contract, so it was not caller-only; owner ruled `Pick'em · O/U 48.5`, and the fourth formatter must resolve the side from signed spreads and print the ROW'S OWN name, never canonical `favorite`. Residue: [#795](https://github.com/znpruitt/cfb-app/issues/795), [#796](https://github.com/znpruitt/cfb-app/issues/796), [#797](https://github.com/znpruitt/cfb-app/issues/797) (NOT #758 — different population), [#798](https://github.com/znpruitt/cfb-app/issues/798). **One owner-approved step-6 round, and its condition earned its keep:** the first rename was **as unfalsifiable as the name it replaced**, because a final-row assertion discriminates nothing when the caller gate and the component policy both suppress finals. Prompt `docs/prompts/platform-723-725-715-matchups-caller-work-codex-v1.md` |
| ✅ | ~~Matchups odds footer~~ | ~~#715~~ | `gameCardPresentation.ts`, `MatchupsWeekPanel.tsx` | **MERGED `f62a47e3` ([PR #806](https://github.com/znpruitt/cfb-app/pull/806)) 2026-09-15; #715 closed.** Scheduled rows render the compact line or content-bearing `Line not posted`; all other states remain footer-free. The formatter derives the side from signed spreads and prints the row's own name, never canonical `favorite`; only equal `0/0` sides are pick'em. Seven tests added, one existing assertion retargeted without weakening; two reviewer rounds left no credible in-scope P0/P1/P2. Prompt `docs/prompts/platform-715-matchups-odds-footer-codex-v1.md` |
| ✅ | ~~Schedule presentation~~ | ~~#728, #730, #731~~ | `GameWeekPanel.tsx` | **MERGED `81f7e287` ([PR #809](https://github.com/znpruitt/cfb-app/pull/809)) 2026-09-16; #728, #730, #731 closed. **LIVE 2026-09-20** — promoted in `dpl_48dpusfVKAmRGVMQ6kS9VXcocWwn`, a rebuild of `c03042ad` (verified: that merge is an ancestor of the promoted build).** Tags and event context use their dedicated slots; games are neutral rounded blocks with Schedule-only divider/padding overrides; date headings use the settled quiet uppercase treatment. Planning's `e6f6b630` amended the stale date-rule justification before closeout. The selector cap remains #795. Prompt `docs/prompts/platform-728-730-731-schedule-presentation-codex-v1.md` |
| ✅ | ~~Schedule team-name form~~ | ~~#729~~ | `gameWeek.ts`, `DESIGN.md` | **MERGED `c9cb3195` ([PR #815](https://github.com/znpruitt/cfb-app/pull/815)) 2026-09-17; #729 closed. **LIVE 2026-09-20** — promoted in `dpl_48dpusfVKAmRGVMQ6kS9VXcocWwn`, a rebuild of `c03042ad` (verified: that merge is an ancestor of the promoted build).** Schedule and Postseason render the provider's full school name from `csv*`, the same field Overview and Matchups use; `DESIGN.md` carries the rule as of `377359f2`. Cross-surface test added — mutating Overview's `csv*` reads to `can*` reddens it. Evidence in `docs/prompt-registry.md` → PLATFORM-729-SCHEDULE-TEAM-NAMES-CODEX-v1. [#810](https://github.com/znpruitt/cfb-app/issues/810) is now UNBLOCKED — `scoreboardName` has no production reader |
| ✅ | ~~Overview column breakpoint~~ | ~~#750~~ | `OverviewPanel.tsx` | **MERGED `22a0607f` (PR #829) 2026-09-19; #750 closed. **LIVE 2026-09-20** — promoted in `dpl_48dpusfVKAmRGVMQ6kS9VXcocWwn`, a rebuild of `c03042ad` (verified: that merge is an ancestor of the promoted build).** Scoreboard grids inset by tier (`880.667px` / `1341px`), 40px gutters at every width, surplus at the section's right edge; one column stays full width. Target `403px` and headroom `52px` both derived from measurement, replacing Item 134's unmeasured `416px`. `DESIGN.md` amended in `95d658a8`. Two shapes were tried and rejected first — a per-row cap (~233px gaps) and content-sized tracks (would resize while a member watches a live game). Leaves [#726](https://github.com/znpruitt/cfb-app/issues/726), [#821](https://github.com/znpruitt/cfb-app/issues/821), [#827](https://github.com/znpruitt/cfb-app/issues/827) open |
| ✅ | ~~Overview standings records~~ | ~~#827~~ | `overview.ts`, `OverviewPanel.tsx` | **MERGED `2b167998` ([PR #859](https://github.com/znpruitt/cfb-app/pull/859)) 2026-09-22; #827 closed. NOT LIVE until promoted.** The condensed table now takes its record, Win%, Diff, GB and rank from the same live rows as the podium, so one screen no longer names two leaders (the 2026-09-19 production sighting: podium Chamness 17–6, table BHooper 15–4). Movement stays anchored on resolved weeks and said so — the caption shipped as **Movement · through W2** (**superseded by #855**, which makes it the constant `Movement`; this row records what #827 shipped, not current policy), with arrows keyed by owner rather than by displayed position, and an accessible name naming both boundaries. Acceptance 1 failed first in the production shape before the fix. Evidence in `docs/prompt-registry.md` → PLATFORM-827-OVERVIEW-LIVE-RECORDS-CODEX-v1 and `docs/campaigns/platform-827-overview-live-records-closeout.md`. Residue [#851](https://github.com/znpruitt/cfb-app/issues/851), [#855](https://github.com/znpruitt/cfb-app/issues/855), [#856](https://github.com/znpruitt/cfb-app/issues/856) |
| 7 | Movement caption + arrow palette | **#855**, **#856** | `OverviewPanel.tsx` | **IMPLEMENTED — confirming independent reviews pending.** Keep this slice in the UI lane until confirmation and merge; no new dispatch is needed. Both design rulings are settled. [Execution/review record](prompt-registry.md#platform-855-856-movement-caption-codex-v1); [kickoff](prompts/platform-855-856-movement-caption-codex-v1.md). |
| ✅ | ~~Compact header wrap at phone width~~ | ~~#797, #758~~ | `CompactGameScoreboard.tsx` | **MERGED `37354e6d` (PR #864) 2026-09-24; #797 and #758 closed. NOT LIVE until promoted.** Four review rounds. Infrastructure follow-up #870 deferred; the `DESIGN.md` rationale was wrong and planning corrected it in `dd69113f` — `max-sm:` is a viewport query and the grid collapse is a container query, so they cannot coincide. **Two lessons outlived the slice and live in `AGENTS.md`/`DESIGN.md`, not here:** a fix can be false-clean for the false-clean hazard it closes, and four passes can clear soundness while none checks completeness. [Execution record](prompt-registry.md#platform-797-header-wrap-codex-v1). |
| 9 | Overview shared-row back-application | **#669** | `OverviewPanel.tsx` | **QUEUED, kickoff written 2026-09-25 — the last audit-residue item with genuine member-visible weight.** Six row decisions were taken once for the SHARED row, recorded under a Schedule heading, and never applied back; `presentation-decisions.md:21` says the header governs all four surfaces, so this is back-application, not six design questions. **Finding 1 is a slot choice:** the watchlist puts its chips in `CompactGameScoreboard`'s `contextSlot` (`OverviewPanel.tsx:969`) rather than `tagSlot`, so a tagged card renders THREE header lines where the design has one. The grid misalignment in the issue title is the SMALLER symptom — invisible at desktop width, appearing only when tags wrap. **Moving the chips into `tagSlot` brings the watchlist under #797's phone-width wrap for the first time**, on a slot that can carry a reason label plus two tags — wider than anything #797 measured. **Must precede #673 and #718**, which both tune a row this slice may relocate. Carries CARRY row 25's `margin-left: auto` trap, which bites harder here because some rows are tag-only. Kickoff: [`docs/prompts/platform-669-overview-shared-row-codex-v1.md`](prompts/platform-669-overview-shared-row-codex-v1.md). |
| 6 | Delayed game reads as live | **#830** | `scores/cache.ts` reader, shared scoreboard | **QUEUED, presentation defect (owner 2026-09-19).** A weather-delayed game keeps rendering `LIVE · Q4 8:11` forever: the provider's status is coarse (`in_progress`), so one sample cannot tell a delay from a pause. `itemUpdatedAtById` already stores each row's last-material-change instant, so `now − stamp` is the available signal. Threshold must be DERIVED (halftime, quarter breaks, poll cadence), the label must describe the observation not the cause, and a new row state is a `DESIGN.md` decision |
| ✅ | ~~Abbreviation lookup~~ | ~~#831~~ | `src/data/team-abbreviations.json`, `teamAbbreviations.ts` | **MERGED `41f688c5` (PR #834) 2026-09-20; #831 closed. **LIVE 2026-09-20** — promoted in `dpl_48dpusfVKAmRGVMQ6kS9VXcocWwn`, a rebuild of `c03042ad` (verified: that merge is an ancestor of the promoted build).** A pinned 2026 artifact of all 682 provider schools (679 abbreviations, 3 explicit nulls) plus a client-safe `getTeamAbbreviation(school)`. **The catalog is untouched** — the draft-visibility observer is the safety proof, not the row count. Unblocks [#832](https://github.com/znpruitt/cfb-app/issues/832) |
| ✅ | ~~Abbreviation width fallback~~ | ~~#832~~ | `ScoreboardTeamName.tsx`, `CompactGameScoreboard.tsx`, recap | **MERGED `a1d0f3e9` ([PR #850](https://github.com/znpruitt/cfb-app/pull/850)) 2026-09-22; #832 closed. **LIVE 2026-09-22** as `dpl_4eZ4C9b29fJTVMCQgPLkxugdagsU`, verified from the live league bundle: `data-scoreboard-team-accessible`, `"SEMO"` and `Southeast Missouri State` present, all three absent before promotion, and the bundle up ~35 KB against the measured ~32 KB lookup cost.** The server renders the abbreviation and the client measures the name's own box in the viewer's font, then upgrades to the full name when it fits; there is no threshold table. v1's width-tier design was withdrawn after three rounds, because the shipped font is the viewer's system stack. Owner-ruled priority order (`DESIGN.md`): the full name gives way first, the owner next, and the abbreviation and record never, down to the measured 219px floor. Evidence in `docs/prompt-registry.md` → PLATFORM-832-ABBREVIATION-FALLBACK-CODEX-v2. **Unblocks [#726](https://github.com/znpruitt/cfb-app/issues/726)**, which re-derives the column tiers now that the fallback ships. Residue [#845](https://github.com/znpruitt/cfb-app/issues/845) |
| ✅ | ~~Third-column tier~~ | ~~#726, #681~~ | `GameWeekPanel.tsx`, `MatchupsWeekPanel.tsx` | **MERGED `a9569438` ([PR #874](https://github.com/znpruitt/cfb-app/pull/874)) 2026-09-25; #726 and #681 closed. NOT LIVE until promoted.** **Two numbers, not one** — the surfaces differ in card padding (34px vs 20px) and Schedule carries no records, so "one shape, two surfaces" means one METHOD. Matchups also converted from `lg:`, a VIEWPORT query, to container queries, so the three surfaces finally share a coordinate system. **Schedule came DOWN from the documented 1320 and Matchups went UP from 1372** — the evidence the number was re-measured rather than inherited, which is what retires #681's "one number, three sources, no two alike". Schedule 1162px / Matchups 1469px container tiers; approved Matchups second tier 976px. Owner authorized a second and final proof-only round; production remained frozen. [Measurements, population exclusions, conditions and review disposition](campaigns/platform-726-third-column-tier-closeout.md). No production promotion claimed. Overview stays with #873. |
...
here; #758 (phone-width tag relief) came out of 173b and is now folded into #797 above by owner ruling.
...
| ✅ | [#708](https://github.com/znpruitt/cfb-app/issues/708) | every CFP first-round game shared one `eventKey` — **MERGED `68fbf397` ([PR #814](https://github.com/znpruitt/cfb-app/pull/814)) 2026-09-17; #708 closed. **LIVE 2026-09-20** — promoted in `dpl_48dpusfVKAmRGVMQ6kS9VXcocWwn`, a rebuild of `c03042ad` (verified: that merge is an ancestor of the promoted build).** Each first-round game now carries its own `eventId`, so one label override reaches one game. 8 games change `eventId`/`key` across seven stored seasons and 0 others do; 0 stored overrides, measured twice, so none is orphaned. Evidence in `docs/prompt-registry.md` → PLATFORM-708-CFP-FIRST-ROUND-IDENTITY-CLAUDE-v1. Residue [#811](https://github.com/znpruitt/cfb-app/issues/811), [#812](https://github.com/znpruitt/cfb-app/issues/812), [#813](https://github.com/znpruitt/cfb-app/issues/813), [#808](https://github.com/znpruitt/cfb-app/issues/808) |
...
| **190** | [#693](https://github.com/znpruitt/cfb-app/issues/693) | a failed standings invalidation can leave results stale indefinitely — IMPLEMENTED 2026-09-14, awaiting both reviews at `4efd3c19`. **This is the LAST open finding of the 2026-09-08 audit**; R1, R2, R4, S1 and S2 are all closed. Residue [#792](https://github.com/znpruitt/cfb-app/issues/792) |
