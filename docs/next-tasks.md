...
| 2 | [#875](https://github.com/znpruitt/cfb-app/issues/875) | a test ends by exhausting a budget rather than by asserting — **QUEUED, kickoff written 2026-09-25.** `stall.test.ts:877` mocks `Date` but not `setTimeout`, so ~750 ms of REAL wall-clock runs against a 30 s file budget and exhaustion kills the FILE rather than failing an assertion. Observed once in 350 runs. **#872's `tickUntilSettled` does NOT transfer** — there is no clock to advance, so the fix must BOUND the wait. **The positive control is the hard part:** the failure cannot be induced by shrinking a budget, only by starving the process, and “none that is honest” is an acceptable receipt answer where a fabricated control is not. Kickoff: [`docs/prompts/platform-875-real-time-stall-claude-v1.md`](prompts/platform-875-real-time-stall-claude-v1.md). |
