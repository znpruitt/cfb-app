...
- Attachment Regression Tests First (PLATFORM-001A-ATTACHMENT-REGRESSION-TESTS-FIRST): test-only coverage locking schedule-canonical score/odds attachment + eligibility before the odds-matching fix. Documents the current pair-only odds misattach risk (same-pair rematch fan-out, duplicate provider events) with passing "current behavior" tests plus `test.skip` intended-invariant tests that **require PLATFORM-001B** (event-centric/date-aware odds attachment). **PLATFORM-001B is the follow-up** that must make odds attachment event/date-aware and flip those skipped tests green.
