...
| ✅ | [#757a](https://github.com/znpruitt/cfb-app/issues/757) | schedule presentation becomes its own scheduled job — **MERGED `d3874dc6` ([PR #863](https://github.com/znpruitt/cfb-app/pull/863)) 2026-09-23; #757 stays OPEN (757a is one slice of it).** Standalone `/api/cron/schedule-presentation`, Tuesdays 13:00 UTC, own receipt and System Health row; year selection relocated to `lib/activeSeasonTargets.ts`; all three inline callers intact, so the merge changes no behaviour. **SHIPS DORMANT — not in `vercel.json`, no QStash schedule.** Installing it is an owner step (`docs/deployment-runbook.md` §8i) and is **GATED ON [#861](https://github.com/znpruitt/cfb-app/issues/861)**, the one known budget defect: elapsed is captured before an awaited durable read, so a degraded store can under-count by up to 15s. Between promotion and install, System Health reports `schedule-presentation` delivery as missing — expected, per §8i. **Five remediation rounds where `AGENTS.md` allows one, self-reported**; the rule defect it exposed is now recorded in `AGENTS.md` (`cb43d6b5`) — the narrow-defect exception regenerates itself, and the sizing signal fired first (~2,700 insertions against an approved ~1,500, before round 1). Follow-ups: #861, 757a2, 757b (gated on one observed receipt), #857, #858 |
| 1 | [#861](https://github.com/znpruitt/cfb-app/issues/861) | the presentation budget decides on a stale elapsed — **QUEUED, kickoff written 2026-09-23**, and it **gates installing §8i**, so it comes before any other platform work. `elapsedMs` is captured at `route.ts:421`, `venueRefreshDue()` is awaited at `:434`, and the admission check at `:438` reuses the pre-await value — under a degraded store, up to 15s under-counted, once per admission (the loop re-reads the clock each iteration, so it does not compound). **THE TRAP: do NOT fold a store term into the reservation.** That was implemented and reverted in 757a round 4 — a 45s allowance makes an owed venue leg reserve 287s against a 250s budget, so no second year could ever start, which is round 1's starvation exactly; two existing tests caught it. Re-measuring is safe where inflating is not, because the reservation is unchanged and the first year is ungoverned regardless. Kickoff: [`docs/prompts/platform-861-elapsed-capture-claude-v1.md`](prompts/platform-861-elapsed-capture-claude-v1.md). |
