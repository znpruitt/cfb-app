...
| 8 | Compact header wrap at phone width | **#797** (carries **#758**) | `CompactGameScoreboard.tsx` | **REVIEWED, awaiting merge in [PR #864](https://github.com/znpruitt/cfb-app/pull/864).** Owner accepted the browser receipt and corrected the kickoff: header-only wrapping fixes clipping but leaves intrinsic-width, left-aligned tags. All three conditions now move for cross-state alignment consistency. One test-only remediation; both confirming reviews clean. [Execution record](prompt-registry.md#platform-797-header-wrap-codex-v1); [measurements and mutation evidence](campaigns/platform-797-header-wrap-closeout.md). |
