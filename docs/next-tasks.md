...
- **PLATFORM-086A — provider-refresh observability foundation — ✅ shipped (PLATFORM-086A-REFRESH-OBSERVABILITY-v1).** The operational foundation for PLATFORM-086 automation: durable per-dataset refresh status (`provider-refresh-status`) recorded truthfully at every refresh entry point (six routes + season-transition & game-stats crons) — a failed attempt never advances last-success; durable operator settings (`provider-refresh-settings`: global pause + per-dataset enable, consumed today by the game-stats cron, with the lifecycle-critical season-transition cron exempt); a unified `/admin/diagnostics` **Provider Data Status** panel (`GET/POST /api/admin/provider-status`) with per-dataset freshness/error/rows/partial state, cache-only missing-data diagnostics, manual refresh for all six datasets, and a read-only current-vs-planned automation summary; and a reusable user-facing freshness primitive (`FreshnessLabel`) shown as a subtle "Odds updated …" chip. CFBD usage stays tier-derived (never a hardcoded 1,000). **No new cron cadence** — the live-score/odds/schedule/rankings automation is deferred to **PLATFORM-086B–086E** (see below). See `docs/operations/diagnostics.md` → "Provider Data Status panel".
