...
| **4a** | Overview standings records | **#827** | `overview.ts`, `OverviewPanel.tsx` | **READY — kickoff `docs/prompts/platform-827-overview-live-records-codex-v1.md` (2026-09-20), takeable as soon as #832 frees the lane. — OWNER RULED 2026-09-19: live records, resolved movement.** The condensed table took its record values from the latest resolved week, so on game days it disagrees with the podium above it. `DESIGN.md` amended the same day. Open question for the slice: what the rank arrow means once rows are live, and why the full Standings page's MOVE column already differs from Overview's |
