...
- **PLATFORM-085A — provider cache commit order — ✅ shipped (PLATFORM-085A-PROVIDER-CACHE-COMMIT-ORDER-v1).** Made every provider refresh path durable-first: `await setAppState(...)` before updating the process-local cache and before invalidating standings, so a failed durable write never publishes process-only "fresh" schedule/scores/odds. Touched the scores, schedule, odds, and conferences routes plus the durable-odds store and odds-usage memo; added a test-only `__setAppStateWriteFailureForTests` seam. Codified as AGENTS.md Core rule #1. See `docs/architecture/storage-and-caching.md` → "Durable-first commit order".
