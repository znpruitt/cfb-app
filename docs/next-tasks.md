...
| 7 | [#778](https://github.com/znpruitt/cfb-app/issues/778) | archive **slug** validation — **BLOCKED on an owner ruling** on `claude/778-archive-slug-validation`: both reviewers gathered and remediated, but the guard costs extra `leagues::registry` reads per archive year in Route Handlers (`React.cache` does not dedupe there), and every contained fix leaves the prompt's scope. Evidence in `docs/prompt-registry.md` -> PLATFORM-778-ARCHIVE-SLUG-VALIDATION-CLAUDE-v1 |
