...
| **3** | [#833](https://github.com/znpruitt/cfb-app/issues/833) + [#813](https://github.com/znpruitt/cfb-app/issues/813) + [#794](https://github.com/znpruitt/cfb-app/issues/794) | **NEXT on the platform lane — kickoff `docs/prompts/platform-833-813-794-trust-cleanup-claude-v1.md` (2026-09-20).** Three bounded cleanups sharing one root: a reader trusting what it never validated. #663's dead symbols and a docstring claiming a retired policy, plus the live `LeagueStatusPanel` record-versus-row defect; the unguarded `eventKey` read whose blast radius is the whole build; and a malformed registry silently narrowing the historical-repair refusal. **All three unreachable in production**, so the tests are the only evidence |
