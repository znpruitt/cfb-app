...
| **121** | [#708](https://github.com/znpruitt/cfb-app/issues/708) | every CFP first-round game shares one `eventKey` — **NEXT on the platform lane; kickoff `docs/prompts/platform-708-cfp-first-round-identity-claude-v1.md` (2026-09-16).** Measured: the React key half no longer reproduces (the merge helper suffixes contested keys); the shared `eventId` still makes one override hit all four games. Zero overrides stored in production |
