...
- **PLATFORM-085B — season-transition schedule safety / partial provider-result handling — deferred.** Rejecting partial regular/postseason transition results, retaining prior-good schedule on partial failure, and the transition probe/state-machine work remain a follow-up (only the durable-first commit-order rule was applied to existing writes).
