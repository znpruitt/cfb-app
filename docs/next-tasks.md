...
| **2** | [#663](https://github.com/znpruitt/cfb-app/issues/663) | targeted schedule repairs never converge on the whole-season snapshot (audit A1) — **NEXT on the platform lane; kickoff `docs/prompts/platform-663-schedule-convergence-claude-v1.md` (2026-09-19). PHASE A IS DESIGN ONLY**, receipt then ruling. Measured 2026-09-19: production holds 7 schedule keys, every one a whole-year aggregate, and ZERO week partitions — the defect is latent, so the work is a contract that makes the divergence impossible, not a repair. `/api/schedule` can commit `${year}-${week}-${seasonType}`, but ~9 whole-season readers take `${year}-all-all` or the regular/postseason pair and only that route reads a child key |
