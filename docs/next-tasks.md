...
| 5 | Third-column tier | **#726** (carries **#681**) | `GameWeekPanel.tsx`, `MatchupsWeekPanel.tsx` | **QUEUED, kickoff written 2026-09-24; the gate is DISCHARGED.** #678 and #681 are closed and every row-content slice merged (#723/#725/#715/#728/#730/#731, #832, #797), so the reason it was held — a breakpoint derived before row content is final has to be derived twice — no longer applies. **Three things the issue predates.** (a) **The surfaces are on different coordinate systems**: Overview and Schedule use CONTAINER queries, Matchups uses `lg:`, a VIEWPORT query, with no explicit one-column rule — so a "1320px breakpoint" means two different things, and converting Matchups is the larger half of the slice. (b) **#821 INVERTED the derivation** (ruled 09-20, shipped by #832 on 09-22, both after this issue was filed): the column no longer has to hold the longest name, because the abbreviation is the width fallback — so re-deriving from `OverviewPanel.tsx:83`'s 403px model would reproduce a replaced one. (c) **Overview's own `403px`/`1341px` predate that ruling and nobody has asked whether they are stale** — receipt item 5, planning rules, the lane must not touch Overview. Kickoff: [`docs/prompts/platform-726-third-column-tier-codex-v1.md`](prompts/platform-726-third-column-tier-codex-v1.md). **Superseded note, kept for provenance:** **UNBLOCKED BY #750's METHOD, still LAST.** Matchups (1372px) and Schedule (1320px) carry the same unmeasured provenance, and Schedule's arithmetic does not reproduce (Item 152). It applies #750's fixture and harness per surface rather than inheriting the arithmetic |
