...
| 8 | Compact header wrap at phone width | **#797** (carries **#758**) | `CompactGameScoreboard.tsx` | **Awaiting merge; latest correction gates/reviews in [PR #864](https://github.com/znpruitt/cfb-app/pull/864).** Owner accepted the browser receipt and corrected the kickoff: header-only wrapping fixes clipping but leaves intrinsic-width, left-aligned tags. All three conditions now move for cross-state alignment consistency. Round 2 exposed a vertical-clipping blind spot after round-1 confirmation. Owner authorized a second harness correction, then a third limited to the desktop fixture width and template-token separator. Final conditional-space correction and merge authorized; infrastructure finding deferred as #870, and the owner resolved the DESIGN rationale in `dd69113f`. [Execution record](prompt-registry.md#platform-797-header-wrap-codex-v1); [measurements and mutation evidence](campaigns/platform-797-header-wrap-closeout.md). |
