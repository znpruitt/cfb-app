...
| 7 | Movement caption + arrow palette | **#855**, **#856** | `OverviewPanel.tsx` | **QUEUED, one-line copy change — owner ruled 2026-09-22: the caption reads `Movement`, with no week.** `· through W3` was redundant in **every reachable render**, not merely some: caption and week headers both come from `positionDeltaData` through one label function (`:1885-1886`), and that object is `null` or carries a non-empty `weeks` array (`:1817`), so the caption can never name a week the last column header does not. The per-row arrow already names BOTH boundaries in its accessible name (`:744`). `DESIGN.md` → *Overview standings row hierarchy* carries the ruling and the general rule (a column-group label says what values MEAN, not what the headers show). **Cheapest slice in this table and the `OverviewPanel.tsx` context is warm from #827 — take it next.** **FULLY RULED 2026-09-22, nothing open:** the caption is UNCONDITIONAL, so the `latestWeek == null` branch and the `Movement · awaiting first resolved week` string both go — the label names the block whether or not a week has resolved yet. **#855's premise that absent history and zero-resolved-weeks need SEPARATE decisions was wrong** — `:1811` and `:1817` both return `null`, so the component cannot tell them apart; the unconditional caption makes the distinction moot rather than resolving it. **#856 joins it** — same file, edits fifty lines apart: the inline arrow's `emerald-300`/`amber-300` (`:738-741`) becomes `deltaTextColor`, which `DESIGN.md:101` already settles (delta in green/red; amber is for COUNTS). Kickoff: [`docs/prompts/platform-855-856-movement-caption-codex-v1.md`](prompts/platform-855-856-movement-caption-codex-v1.md). |
