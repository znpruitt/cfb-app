...
- **PLATFORM-075** — provider quota hardening: anon stale-serve for odds, CFBD quota guard on scores, season in the in-memory odds cache key, remove dead `dayKey` (V13/V10, P2). ✅ Done (branch `platform/platform-075-provider-quota-hardening`, commits `6c5827d`…`3e5d139`). Product call resolved: **public `/api/odds` + `/api/scores` are pure cache readers** — anonymous requests serve fresh/stale cache or a controlled empty/stale response and NEVER trigger a cold-cache upstream fetch; only an authorized `refresh=1` (gated by `requireAdminAuth`, so platform admin / server cron / `ADMIN_API_TOKEN`) may spend CFBD/Odds quota; league-password access grants no fetch authority; public freshness is best-effort. Season-wide scores reads reconcile the season snapshot with per-week caches in one bounded year-prefixed store read (no client fan-out), deduped by canonical game identity via `teamIdentity` (pair + UTC date, so provider/canonical week aliases and cross-provider labels collapse to one game; empty week entries can't mask populated season rows). Filtered (non-canonical) odds reads build only from their own fetched/cached events and never seed the full durable snapshot. Admin manual refresh, `GlobalRefreshPanel`, and the `/api/debug/*` score diagnostics all forward `refresh=1` + admin auth so authorized surfaces keep fetching. Eight Codex rounds converged (final clean). Next correctness task is **PLATFORM-076**.
...
- Deferred product decisions surfaced by the audit: CSV current-season guard vs sanctioned override; delete vs wire `AdminDebugSurface`; ~~public odds/scores fetch policy~~ (resolved in PLATFORM-075: public = pure cache reader, upstream fetch requires authorized `refresh=1`); owner-identity mapping across seasons; whether to schedule PLATFORM-040.
