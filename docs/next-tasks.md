...
| 4b | Abbreviation lookup | **#831** | new store + fetch script | **QUEUED — prerequisite for #832.** CFBD has abbreviations for all 238 renderable names (2-4 chars, zero gaps); this app has 138 FBS teams only. **The catalog is the wrong home:** `getTeamDatabaseItems` has 41 call sites and two are the draft board, so widening it would make FCS schools draftable |
