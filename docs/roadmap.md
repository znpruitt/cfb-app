...
- **PLATFORM-086I — settings feedback ✓ Complete (PR #413, shipping — docs closeout in the PR).** The stored global-pause and interactive dataset auto-refresh toggle mutation errors now render as accessible `role="alert"` regions beside their control (conditional `aria-describedby`), authoritative-state preserved; client-only, no diagnostics-page redesign (that stays 086F).
