...
- **PLATFORM-086G1 — CFBD score & quota truthfulness (implemented, in review — branch `platform/086g1-cfbd-score-quota-truthfulness`).** Contextual target-scoped Scores empty classification (`cfbd-empty-unexpected` failures retain prior-good data; legitimate empties stay no-ops); CFBD quota missing/malformed fields resolve to unavailable, never false exhaustion.
