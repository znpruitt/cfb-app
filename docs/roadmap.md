...
- **PLATFORM-086H — game-stats recovery (in progress — staged).** 086H1 data-contract foundation ✓ complete (PR #396, dormant — strict parsing, typed classification, inventory-proven legacy compatibility; nothing activated). Remaining staged PRs: durable merge service, atomic contract activation + recovery integration (schedule-derived completeness, partial-week retry), diagnostics + panel no-op wording, legacy-row migration.
