...
- **PLATFORM-086H — game-stats recovery (in progress — staged).** 086H1 data-contract foundation ✓ complete (PR #396). 086H2 durable merge service ✓ complete (PR #397; single-client advisory-lock transaction, observation fencing, conservative field merge). 086H3 atomic contract activation + recovery integration — **implemented, review pending, not merged** (branch `platform/086h3-atomic-game-stats-contract-activation`, incl. the folded canonical-coverage-recovery remediation): every game-stats writer routes through the durable merge authority, with canonical participant/classification attachment (teamIdentity-validated, FCS-vs-FCS excluded, placeholders deferred), committed-state coverage evaluated after every confirmed merge via one finalize path (partial commits publish partial status; unexpected-empty is a stable failure), a persisted bounded recovery disposition with backoff and candidate rotation, the canonical analytics projection (incl. archive-integrity points eligibility), truthful coverage-aware read availability, AST-based activation guards, and a metadata-free public wire (legacy rows byte-equivalent, no migration required). Deferred: 086H4 diagnostics + panel no-op wording, legacy-row migration.
