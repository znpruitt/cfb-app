...
- **PLATFORM-086E2 — rankings publication refresh, SPLIT into E2A + E2B (mirrors the E1A/E1B authority-then-activation shape).** Scheduling boundary corrected: external provider polling runs on **QStash** (one fixed `0 4,22 * * *` UTC heartbeat in E2B; the application's cache-only publication policy decides whether provider work is due) — `vercel.json` keeps only the lifecycle crons; the earlier "cadence fixed in code/`vercel.json`" note was stale. Never admin-editable cadence.
  ...
  - **E2B — publication-aware cron + QStash contract + descriptor activation ✓ complete, MERGED to `main` via PR #428 (merge commit `1c34352`, 2026-07-30); NOTHING PROVISIONED — §8j activation pending.** `GET /api/cron/rankings` (auth → Rankings gate → registry years → cache-only context → E2A publication classifier → durable exact-window claim with immutable completed windows → fresh `/info` ≥1,007 per due year → the E2A authority with `trigger:'automatic'`; one secret-safe `rankings-cron` event), the fixed `turfwar-rankings-publication` QStash contract (`0 4,22 * * *`, retries 0, redacted forwarded auth) in the shared inspect-first CLI, and the rankings descriptor/toggle activation (conferences is the only remaining planned dataset). Merging activated nothing — runbook §8j (provision + staged proofs) is the remaining activation checkpoint; after it, PLATFORM-086F2 is the last provider-campaign implementation item.
