...
- **PLATFORM-086H — game-stats recovery (in progress — staged).** 086H1 data-contract foundation ✓ complete (PR #396, dormant). 086H2 durable merge service ✓ implemented and verified (dormant; single-client advisory-lock transaction, observation fencing, conservative field merge — PR pending). Next: 086H3 atomic contract activation + recovery integration (schedule-derived completeness, partial-week retry; all consumers flip together). Deferred: 086H4 diagnostics + panel no-op wording, legacy-row migration. Nothing activated yet.
