...
| 086F2 | Admin control-plane IA redesign (F2A audit/IA doc + F2B–F2J implementation slices; plan: `docs/architecture/admin-control-plane.md`) | In progress (F2A–F2D ✅ merged, PRs #430–#434; F2E1/F2E2A/F2E2B scheduler receipts + reader ✅ merged, PRs #435–#437; F2F system-health read model ✅ merged (PR #438); F2G System Health UI ✅ merged (PR #439); F2G1 draft-assistance retirement (SP+/win-totals removed from the draft) ✅ merged (PR #440); F2H split into F2H1 guarded lifecycle transitions + recovery authority (implemented, pre-merge, PR #441) → F2H2 archive/backfill safety → F2H3 Season Management UI consolidation — queue in `docs/next-tasks.md`) |
