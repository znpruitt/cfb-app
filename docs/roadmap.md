...
- **PLATFORM-086H — game-stats recovery (in progress — staged).** 086H1 data-contract foundation ✓ complete (PR #396). 086H2 durable merge service ✓ complete (PR #397; single-client advisory-lock transaction, observation fencing, conservative field merge). 086H3 atomic contract activation + recovery integration — **implemented, review pending, not merged** (branch `platform/086h3-atomic-game-stats-contract-activation`, incl. five folded Codex remediation rounds): every game-stats writer routes through one orchestration boundary into the durable merge authority, with AUTHORITATIVE-identity-only canonical attachment (normalized text never an identity; unknown identity/classification defers; explicit FBS/FCS allowlist, FCS-vs-FCS excluded), committed-state coverage evaluated after every confirmed merge via one finalize path (partial commits publish partial status; unexpected-empty is a stable failure; commit stamps captured at COMMIT so stalled finalizers cannot reorder status), atomic fenced recovery claims with token-conditional finalization, lease expiry, coverage/schedule-fingerprint progress, and cross-run backoff/rotation (overlapping crons cannot double-fetch), full weekly-envelope validation with a schema-safe public wire (unsupported schema withheld, never legacy-laundered; legacy rows byte-equivalent), shared coverage across reads/diagnostics/cache-state, the canonical analytics projection (incl. archive-integrity score evidence), module-ownership activation guards that also bar re-exports of guarded capabilities, durable cross-instance refresh-status ordering (an ENDURING schema-versioned revision ledger, sole ordinary allocator, co-committed atomically with the partition even on the file backend via staged multi-key transactions — ordering survives restoration/corruption and refuses ambiguous history rather than resetting; every status mutation is one typed durable transaction with a composite begin/terminal publication), post-claim eligibility rereads closing the stale-plan quota race (SANITIZED, stage-specific-coded revalidation errors through the cron with no raw storage detail), an always-constructed public wire (explicit allowlists at every level, `raw` restricted to recognized categories) with a public-visible duplicate selector (analytics-equal-but-public-different copies conflict, order-independent), complete degradation observability (distinct canonical-unresolved/classification-unknown states persisted in status), and route-visible recovery-metadata failures emitted only when a recovery operation actually failed (no migration required). Deferred: 086H4 diagnostics + panel no-op wording, legacy-row migration.
