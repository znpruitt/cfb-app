...
- **PLATFORM-086H — game-stats recovery 🔎 Implemented, in review (not merged).** Schedule-derived expected-game completeness (disrupted/placeholder/FCS-vs-FCS exclusions with positive evidence only), every incomplete completed week retried on the existing Monday cron with one bounded call each, non-destructive merge by canonical game id (prior-good rows never deleted; identical data is a truthful no-op), explicit manual-refresh outcome contract + panel no-op wording.
