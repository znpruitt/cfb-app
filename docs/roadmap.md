...
- **PLATFORM-086G2 — Odds boundary & usage truthfulness (implemented, in review — branch `platform/086g2-odds-boundary-usage-truthfulness`).** Malformed/schema-drift/unexpected-empty Odds payloads rejected before commit (`odds-invalid-payload` / `odds-schema-drift` / `odds-empty-unexpected`, prior-good retained; legitimate empties stay no-ops); odds-usage read failure now distinct from snapshot absence end to end. Separate PR from G1 (different provider family).
