...
- **Optional follow-up debt (non-blocking):** the §8j operator activation (the next checkpoint); delayed-delivery slot-skips are an accepted operational property (monitor `not-a-heartbeat-slot` frequency); completed-window record accumulation (~50–60/season-year, no TTL) is accepted housekeeping; after §8j, **PLATFORM-086F2** (diagnostics IA redesign) is the remaining provider-campaign implementation item; the passive E1C2 §8i observation still blocks nothing.
