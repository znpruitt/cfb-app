...
- **Status:** Complete. **MERGED to `main` via PR #418 (`platform/086b2b-live-score-activation`, merge commit `57fab82`, 2026-07-28); live-score automation DORMANT — no QStash schedule exists, the `GET /api/cron/live-scores` route is unscheduled, and the browser polling is cache-only; operational activation (runbook §8f) has NOT been performed.** Second slice of PLATFORM-086B2 (B2A = writer-lock convergence; B2B = activation). Two commits (implementation + pre-merge docs closeout); Claude `/code-review` self-review + **eight Codex review passes** (13 findings, 12 remediated / 1 deferred).
