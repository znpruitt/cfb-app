...
### PLATFORM-086C1 — Odds Refresh Authority & Writer Convergence (Dormant) — Pre-Merge

- **Status:** Implemented, **PRE-MERGE** — branch `platform/086c1-odds-refresh-authority`, PR to `main` open (awaiting owner merge). **DORMANT; no activation.** First of two PLATFORM-086C slices (C1 = refresh authority; C2 = polling activation). Independent Claude + Codex reviews (cycle 1); all credible in-scope findings remediated across two rounds. Full `npm test` **2376/2376**, `tsc`/`lint:all`/`build`/`git diff --check` clean; **no provider quota spent** (all fetches stubbed).
...
- **Optional follow-up debt (non-blocking):** PLATFORM-086C2 (Odds polling activation) — the future `GET /api/cron/odds`, structured execution log, QStash schedule manager, Odds descriptor flip, runbook activation section, and production rollout. Separately: redact `ODDS_API_KEY` from the `UpstreamFetchError` detail body on the Odds upstream-failure path (pre-existing, admin-gated).
