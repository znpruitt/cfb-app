...
**PROMPT_ID(s):** PLATFORM-086H3-ATOMIC-CONTRACT-ACTIVATION-RECOVERY-INTEGRATION-v1 (+ folded PLATFORM-086H3-CANONICAL-COVERAGE-RECOVERY-REMEDIATION-v1).
...
**Key outcomes:** Durable COMMIT and the committed-state reread strictly precede any publication; empty/invalid/mismatched/uncertain provider responses can never destructively clear prior durable evidence or a meaningful failure state; provider quota stays bounded within AND across runs (backoff + rotation, zero provider calls from ordinary reads, invalid targets fail pre-fetch). The activation guard is AST-based (TypeScript compiler API): aliased/renamed/namespace mutation calls, wrapper functions, const-derived `game-stats` scopes, independent transaction-lock users, mutation imports in writer routes, success publication outside the finalize path, resurrected legacy-normalizer imports, and disconnected lifecycle wiring all fail, with fixture self-tests per bypass class and documented structural guarantees. Deterministic suites cover canonical-attachment regressions, manual target validation, the merge-outcome/status matrix, committed-coverage e2e flows, and repeated-run quota regressions for every unresolved outcome (fake pg pool for transaction uncertainty; no sleeps, no live services). Full suite 1875/1875; `tsc` and `lint:all` clean.
