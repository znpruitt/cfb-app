...
- **Status:** Complete, **MERGED to `main`** via PR #420 (`platform/086c2-odds-polling-activation`, merge commit `262fdf0`, 2026-07-28). **DORMANT; no activation.** Second slice of PLATFORM-086C (C1 = refresh authority; C2 = polling activation). Activates the merged C1 authority in CODE ONLY — no production QStash schedule exists (`turfwar-odds-hourly` is created only by the deployment-runbook §8g operator sequence, which has NOT been executed), so `GET /api/cron/odds` is never invoked on a schedule. Independent Claude self-review + Codex reviews converged over **four remediation cycles + a clean confirming Codex round** (round 4: "No actionable correctness issues"). Full `npm test` **2435/2435**, `tsc`/`lint:all`/`build`/`git diff --check` clean; **no provider quota spent** (all fetches stubbed).
...
- **Follow-up (operator):** automatic Odds polling is **NOT yet running** — the deployment-runbook **§8g** operator sequence (provision `turfwar-odds-hourly` gates-closed → exact-authentication delivery proof → open both gates in order) is the distinct post-merge activation phase. After activation, record it (a §8g production checkpoint + a memory update), mirroring the §8f/DOCS-011 live-score closeout.
