...
**F2G status (implemented, PR open — `PLATFORM-086F2G-SYSTEM-HEALTH-UI-v1`):** `/admin/diagnostics` renders the F2F model server-side and presents as **System Health**, a current-status dashboard. It builds exactly ONE model for the **server-resolved operational season** (`resolveOperationalSeasonYear` — highest `preseason`/`season` `status.year`, else highest `league.year`, else calendar; there is NO `?year=` selection seam), performs no `/api/admin/system-health`, no client GET to `/api/admin/provider-status` or `/api/admin/usage`, and no health/severity re-derivation in React. The incremental composition (`ProviderDataStatusPanel`, `AdminUsagePanel`, `AdminStorageStatusPanel`) is **retired**.
...
| **F2E2B** — Scheduler receipt reader + delivery classifier | Code | ✅ Implemented (PR open) — the cache-only server reader over all seven receipts + schedule-slot-aware delivery classifier (`src/lib/server/schedulerDeliveryHealth.ts`); safe receipt parsing exposed on the authority. Server-only: no route, UI, provider call, scheduler mutation, settings change, or durable write |
...
| **F2G** — System Health UI | Code | ✅ Implemented (PR open) — `/admin/diagnostics` renders the F2F model as a current-status dashboard: a stoplight overview + prioritized issues + always-visible scheduler/provider/quota-storage rows (row-level forensic disclosure) + Automation safety controls; server-resolved operational season (no `?year=` seam); the three incremental panels are retired. See the composition section below |
