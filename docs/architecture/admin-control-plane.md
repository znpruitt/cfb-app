...
The eight external jobs are `live-scores`, `game-stats`, `odds`, `schedule-refresh`, `rankings`,
`team-records`, `season-transition`, and `season-rollover`. The first six are QStash-owned; the two
lifecycle jobs are Vercel Cron-owned.
