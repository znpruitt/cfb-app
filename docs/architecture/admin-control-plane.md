...
| `/admin/season` | Rollover, archive backfill, archive list | Correct destination, but duplicates rollover UI (both panels are strict/per-year since F2B; consolidation is F2H) and lacks full lifecycle state |
...
| Historical scores repair (`POST /api/admin/cache-historical-scores`) | 0 when cached; otherwise up to 2 partitions | Historical score cache only — records **no** `provider-refresh-status` attempt (known gap; see operational clarity findings) | Manual recovery | Data Maintenance |
...
- The two rollover panels (`SeasonRolloverPanel`, `RolloverPanel`) still exist on separate pages, but since F2B both consume the same strict per-year contract with identical eligibility expectations (consolidation into one surface remains F2H).
...
- Historical scores repair (`POST /api/admin/cache-historical-scores`) commits its cache write directly (`setAppState`) without recording a `provider-refresh-status` attempt, unlike the other provider writers (historical *schedule* repair converged onto the shared schedule authority and does record status). A successful repair therefore leaves provider status unchanged. The F2C action contract must disclose this, and F2C must make the repair record a scoped `provider-refresh-status` attempt — status silence is not an allowed design: `AGENTS.md`'s binding provider-status rule requires every provider refresh entry point to record a scoped attempt and outcome.
