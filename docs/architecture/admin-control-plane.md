...
| Score-attachment diagnostic (`GET /api/debug/scores-attachment`) | 1–2 CFBD score requests; when the schedule/conference caches are stale or absent, its context loader (`loadDebugSeasonContext`) forwards admin credentials and adds up to 2 schedule partitions plus 1 conferences request (≈5 total) | Score caches/status (propagates `refresh=1` upstream — **not read-only**); may also refresh schedule/conference caches and statuses via the context loader | Operator diagnostic | Data Maintenance, clearly labeled as mutating |
