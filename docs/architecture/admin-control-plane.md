...
| **F2F** — System-health read model | Code | ✅ Implemented (PR open) — one server-side view model (`src/lib/server/systemHealth.ts`) keeping scheduler delivery, execution outcome, canonical data health, automation gates, latest scoped attempts, quota, and storage distinct; stable issue codes with severity, safe static explanation, and a nullable repair destination. Consumes the F2E2B reader; server-only (no route/UI/mutation). F2G owns the UI |
