...
| `/admin/diagnostics` | Provider status + automation gates, quota, storage, and (until F2D2) the score-attachment tool | F2D1 relocated the manual provider refreshes and the team-database sync to Data Maintenance & Recovery; the mutating score tool relocates in F2D2 |
...
- ~~Diagnostics performs manual provider refreshes, team-database writes, and score-cache mutations from what presents as an observational page.~~ **Mostly resolved by F2D1**: `ProviderDataStatusPanel` no longer offers manual refreshes (System Health keeps only the global pause, dataset toggles, status, quota, and diagnostics); Odds/Rankings refreshes moved to a new Data Maintenance panel and Conferences + the relocated Team Database sync form its Reference data section — all reusing the same URL authority and truthful fallback interpretation, each with an F2C disclosure. **Remaining for F2D2**: the mutating score-attachment tool (`DiagnosticsScorePanel`) still renders on Diagnostics.
