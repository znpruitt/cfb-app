...
| `scheduler-execution-status` (key `<job>`) | latest-only durable execution receipt per scheduled cron job — the seven QStash jobs (`live-scores`/`team-records`/`game-stats`/`odds`/`schedule-refresh`/`rankings`/`usage-sample`, `source: 'qstash'`) plus the two Vercel-native lifecycle crons (`season-transition`/`season-rollover`, `source: 'vercel-cron'`), written after successful cron authentication — separate from `provider-refresh-status`, PLATFORM-086F2E1 / F2E2A / PLATFORM-118. **PLATFORM-086F2H1B additively extended the `season-transition` target's per-year entries** with `alreadyInTargetSeasonLeagues` / `removedLeagues` / `refusedLeagues` alongside the existing `targetLeagues` / `probed` / `transitionedLeagues`. The extension is BACKWARD COMPATIBLE by design: `version` stays `1`, the reader validates the three new counters as OPTIONAL and normalizes an absent one to `0` on rebuild, so a valid pre-H1B receipt keeps parsing instead of degrading its System Health row to `invalid` until the next daily run rewrites it; a PRESENT-but-invalid counter still rejects the whole record, and every new writer emits all three. |
| `provider-usage` (key `cfbd-observations`) | bounded log of raw CFBD quota observations (Item 127) — one `{ at, remaining, limit }` entry per probe, sorted by time and capped at 1,700 entries (~14 months at four samples a day, ~106 KB), trimmed on every write. Nothing is derived at write time: `limit` is recorded as CONTEXT only and is never subtracted or compared, because `used` is `limit − remaining` and a patron-tier change therefore moves it with no calls made. A quota-period boundary is a READ-time fact — the one point where `remaining` rises — not a stored classification, so arrival order cannot matter. Entries are NOT deduplicated on `at`: a QStash redelivery re-probes and stamps a fresh timestamp so it never collided, while two genuine probes inside one millisecond did and one was lost. Written ONLY by `/api/cron/usage-sample`, every 6 hours, through `withAppStateKeyTransaction`. A second opportunistic producer on the game-stats quota probe was built and removed: one durable row with two writers produced lost updates, cross-instance clock skew reading as a quota reset, and out-of-order commits, and 15-minute in-window resolution served no consumer. `/info` is unbilled, so this costs no CFBD quota. Observation-only: nothing reads it to make a decision. It exists because `/info` reports the CURRENT PERIOD only and CFBD exposes no history, so a month boundary destroys the prior month's burn permanently. |
...
Team records are a cache-only future-consumer seam rather than a public route. PLATFORM-118's
`readTeamRecordsCache(year)` reads the normalized `team-records/<year>` snapshot and never calls
CFBD. It returns creditable rows only while separately exposing provider rows whose W+L+T does not
equal games through `uncreditableTeamIds`; absence and present-but-uncreditable remain distinct, and
the reader never derives an outcome. The one writer authority, `refreshTeamRecords`, receives the
live-scores finalisation observation or the hourly job's explicit no-finalisation observation. It is
due after the six-hour floor when a final was observed, or at the independent twelve-hour cache-age
ceiling regardless of finals. The authority claims `team-records-refresh-control/<year>`, stamps
`lastProviderCallAt` immediately before `/records`, and reports a distinct
`provider-call-floor-active` suppression when a due stale cache is held by that call floor. It
rejects non-array payloads, nonempty payloads that normalize to zero rows, and zero-row replacement
of a populated cache; commits are observation-ordered and provider success is recorded only after
the durable write. The hourly `team-records` QStash receipt is separate from provider status. There
is no process memo, UI consumer, or manual route.
