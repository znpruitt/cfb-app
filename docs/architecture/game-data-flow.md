...
Canonical for: schedule → canonical games, score/odds attachment, team-record refresh flow,
game-stats ingestion/evidence flow, public cache-reader + authorized-refresh policy, provider quota
...
against the score attached to that game's disambiguated canonical key. A final requires both numeric
scores; a canceled conclusion is terminal without a score; placeholder, pending, and disrupted games
do not owe a final unless stronger conclusion evidence says they were played. One terminal row can
never cover a sibling game. Operator output retains the complete gap count and exposes at most six
sanitized, bounded game identities before routing to the existing score-recovery action.
...
Current operator and emergency controls live in [`../deployment-runbook.md`](../deployment-runbook.md) §8e; completed activation evidence lives in the linked operations archive.
