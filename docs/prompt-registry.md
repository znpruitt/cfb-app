...
### PLATFORM-086H-GAME-STATS-RECOVERY-v1

- Status: **Implemented on `platform/086h-game-stats-recovery` (2026-07-16) — pending independent review; NOT merged.** No PR opened per the prompt.
- Purpose: Close deferred PLATFORM-086A findings #5 and #1 as one cohesive game-stats correctness PR: judge weekly game-stats completeness against canonical-schedule expectations instead of "at least one usable row" (a partial week suppressed cron recovery forever), retry incomplete weeks on the existing Monday 11:00 UTC cadence with a non-destructive merge (a partial/empty recovery response can never delete prior-good rows), and make the `GameStatsCachePanel` report a valid no-op truthfully instead of "Cached 0 games". No new cadence, no cron added.
- Result:
  - **Schedule-derived expected coverage** (`src/lib/gameStats/coverage.ts`): new pure `deriveExpectedGameStatsIds(items, week, seasonType)` derives the stat-producing canonical game ids for a slate from canonical schedule rows only — never from returned provider stat rows. Exclusions require positive schedule evidence: disrupted/non-played terminal dispositions via the existing shared `expectsGameStats`; unresolved matchups via the new shared label predicate `isPlaceholderTeamLabel` (`src/lib/teamNormalization.ts` — TBD / "Winner of …" / synthetic postseason slot / invalid labels, extracted from and now consumed by `buildPlaceholderParticipant` so participant building and expected coverage share ONE definition — a resolved matchup re-enters the expected set on the next evaluation); and FCS-vs-FCS only when BOTH conferences positively classify FCS through the canonical `inferSubdivisionFromConference` (unknown classification never excludes, so a real FBS game can't silently drop out of expectations). The result also reports `hasScheduleEvidence` so "no schedule rows for this slate" is distinguishable from "nothing expected".
  - **Explicit weekly completeness contract** (`evaluateWeeklyGameStatsCompleteness`): typed states `schedule-unavailable | no-expected-games | no-usable-rows | partial | complete`. A week is complete ONLY when every expected id has a usable cached row (content-based, via the existing `usableGameStatsGameIds`); a subset is `partial` and stays recovery-eligible; unavailable schedule evidence is its own state and can never be read as complete. `providerDataDiagnostics` now builds its per-slate expected sets through the same derivation (one definition; diagnostics no longer count placeholders or FCS-vs-FCS pairings as missing coverage).
  - **Cron recovery** (`src/app/api/cron/game-stats/route.ts`): the former latest-week-only selection (`findLatestCompletedWeek`) is generalized to `findCompletedCandidateSlates` — every completed stat-producing slate is evaluated each run (schedule read through the canonical `loadCachedScheduleItems` partition fallback), a week is skipped only when the completeness contract proves it `complete` (or `no-expected-games`), and each incomplete week gets exactly ONE bounded provider call per run (no intra-run retry loops; the first hard failure aborts the remaining candidates conservatively — they stay eligible and retry on later scheduled runs). Same Monday 11:00 UTC cadence; per-week scope/attempt semantics preserved (missing-key and per-week failures still record against the exact week partition, never the year rollup). The response is now a per-week outcome list (`results[]`: `skipped-complete | skipped-no-expected-games | skipped-schedule-unavailable | committed | noop | no-change | failed | deferred`).
  - **Non-destructive merge** (`mergeWeeklyGameStats`, shared by cron and manual refresh): recovered rows merge by canonical (provider) game id — prior rows the response omits are retained; an unusable incoming row (blank team identity) never clobbers a usable prior row; a usable row repairs a previously unusable one; identical data is `changed: false`, so the caller records a truthful no-op with NO durable rewrite and no downstream invalidation. Durable write still precedes refresh-status success (commit-time ordering unchanged); `rowsCommitted` now reports rows actually added/replaced. Empty provider responses remain no-ops (prior-good retained); nonempty-zero-usable remains a `game-stats-no-usable-rows` failure.
  - **Truthful manual no-op reporting** (`src/app/api/game-stats/route.ts` + `src/components/admin/GameStatsCachePanel.tsx`): the manual refresh response `meta` now carries an explicit outcome contract (`outcome: 'committed' | 'noop'`, `noopReason: 'no-provider-rows' | 'no-new-rows'`, `rowsCommitted`, `rowsCached`; `noApplicableData` kept for compat). The panel renders wording from the new pure `describeGameStatsRefreshResult` — a valid no-op says "No game stats available yet … nothing was cached or overwritten" (or "already up to date"), a commit reports the rows actually added/updated plus total cached, and the legacy "Cached N games" wording survives only as a fallback for responses without the contract. The backfill summary now counts updated vs no-data weeks instead of calling every 200 "cached".
  - `providerDatasets.ts` game-stats descriptor strings corrected per the DOCS-009 note (current automation now includes retry/merge recovery; stale planned "PLATFORM-086C missing-week recovery" attribution removed).
- Verification: `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` suite green (1708/1708 — no pre-existing failures observed). Extended `src/lib/gameStats/__tests__/coverage.test.ts` (+18: expected-set scoping, disrupted/placeholder/FCS-vs-FCS exclusions incl. resolved-postseason inclusion and unknown-classification retention, all five completeness states, merge add/replace/retain/no-change/unusable-clobber/repair). Cron `coverage.test.ts` rewritten to the per-week contract (+7 scenarios: partial week retried with prior-good merge, empty recovery retaining rows, identical recovery as no-change without rewrite, multi-candidate run skipping the complete week and recovering two incomplete ones newest-first with exactly one call each, placeholder/canceled/FCS-vs-FCS never blocking completeness, resolved postseason recovered, unusable row never clobbering); `pause.test.ts` assertions moved to the results contract. Manual `refresh-status.test.ts` +2 (partial-response merge retention; identical-data truthful no-op without rewrite or last-success advance) and outcome-contract assertions. New `GameStatsCachePanel.test.tsx` (5 wording tests incl. the "Cached 0 games" regression). Adjacent suites green: providerDataDiagnostics, providerCacheState, providerRefreshStatus, cfbdSchedule, postseason (boundaries/classify/display/hydrate/attachment edges), schedule-eligibility, teamIdentity. Fixtures/mocks only — no live CFBD calls.
- Scope guardrails: cadence unchanged (Monday 11:00 UTC remains the only game-stats job; no new cron); no Scores/Odds/rankings/schedule automation; no diagnostics redesign (086F); no quota-policy change; no canonical-identity or ownership change; no new storage schema (same `game-stats` app-state records). Remaining campaign sequence unchanged: PLATFORM-086I → 086B → 086C → 086E1 → 086E2 → 086F.
- Review remediation (PLATFORM-086H-PARSE-VALIDITY-FIELD-MERGE-REMEDIATION-v1, separate commit on the branch pre-merge, closing the third-round adversarial findings): (1) **Parse validity, not key presence** — the normalizer's parsers are refactored to `…OrNull` variants (one parsing system; `normalizeTeam` falls back to the same zeros as before) and exported as `isParseValidCategoryValue`, covering integer categories, `thirdDownEff`-style fractions, and `possessionTime`; `TeamGameStats` gains an additive `pointsProvided` flag capturing whether the wire structurally supplied a finite `points` (the normalized 0 fallback is otherwise indistinguishable from a real shutout; absent on legacy rows → incomplete → naturally re-fetched and stamped). Persistence authority now requires ≥1 recognized PARSE-VALID category per side (a malformed-only complete-looking payload is `no-authoritative-rows` schema drift, never silently persisted zeros), and `hasCompleteStatCoverage` requires all six required categories structurally present AND parse-valid PLUS provided points on each side — so `not-a-number` values, malformed fractions/possession times, and missing/malformed points all leave the game incomplete, recovery-eligible, and analytics-ineligible, while explicit valid zeros ("0", "0-0", "0:00", 0 points) remain fully complete. (2) **Field-level merge replaces the whole-row superset rule** (`fieldMergeRows` + `buildTeamStats`, the shared normalization path also used for wire rows): a provably-later observation now merges per side — valid incoming recognized fields win per field, omitted or malformed incoming fields retain prior values, provided incoming points apply while absent/malformed incoming points retain the prior score, and every normalized metric is rebuilt from the MERGED raw maps (normalized fallback zeros are never merge inputs; nothing is synthesized) — so a stale optional category (e.g. a prior `puntReturnYards` a later response omits) can no longer block a legitimate correction to required values. Observation-window fencing is unchanged: overlapping/indeterminate windows keep the first-committed row with no field-merge, additions still union, truthful no-ops preserved. All cron/manual/degraded-502/provider-wide contracts unchanged. Lib fixtures now build FULL-SHAPE rows through `buildTeamStats` (the production path) and hand fixtures gained `pointsProvided`; the three superseded superset-contract tests were rewritten to the field-merge contract; +7 tests (six-malformed-keys never complete/authoritative/available/analytics; per-category malformed matrix; missing-vs-zero points; one-valid-one-malformed team repairable; omitted-optional-category correction applying with normalized rebuild; malformed incoming fields unable to overwrite valid priors while valid corrections apply; points corrections and retention).
- Review remediation (PLATFORM-086H-OBSERVATION-FENCING-COMPLETE-COVERAGE-MIXED-FAILURE-REMEDIATION-v2, separate commit on the branch pre-merge; scoped by the read-only PLATFORM-086H-STALE-SNAPSHOT-SPARSE-AUTHORITY-CRON-OBSERVABILITY-AUDIT-v1, closing the three second-round adversarial findings plus the cancelled-review unusable-row P2): (1) **Persistence authority vs COMPLETE coverage** — new `ANALYTICS_REQUIRED_CATEGORIES` (`netPassingYards, possessionTime, rushingYards, thirdDownEff, totalYards, turnovers` — exactly the raw-backed fields `addTeamStats` consumes, exported beside `RECOGNIZED_STAT_CATEGORIES` with a contract test; `points` is a structural wire field whose presence cannot be gated — documented limitation) and `hasCompleteStatCoverage` (usable identity + all six on EACH side). Completeness coverage (`usableGameStatsGameIds`), cache availability, and owner analytics now require COMPLETE coverage, while payload commit and merge acceptance keep the lighter persistence authority: a sparse row (e.g. `totalYards`-only, or one complete team + one sparse team) is STORED as real partial data, stays recovery-eligible, is repaired by a later complete response, and never feeds fabricated zeros into analytics; explicit all-zero complete rows remain valid coverage and analytics input. (2) **Unusable new rows never persist** — the merge's no-prior branch now requires full `isAuthoritativeGameStatsRow` (identity + content), so a stat-bearing row with a blank home/away school is dropped (`rowsDroppedUnusable`) instead of accumulating permanent non-coverage garbage; mixed payloads persist only their valid rows with truthful counts. (3) **Provider-observation window fencing** — CFBD supplies no observation timestamp (and lock/commit/request-completion order proves nothing), so each refresh captures `fetchStartedAt` before the fetch and stores it additively on the weekly record beside `fetchedAt` (no schema migration); a same-game REPLACEMENT is allowed only when the incoming observation provably STARTED after the stored record's fetch COMPLETED — overlapping or indeterminate windows keep the first-committed row for equal sets AND strict supersets alike (no superset exemption during overlap; missing/unparseable legacy timestamps are indeterminate), while ADDING an absent game id is always allowed (union preserved) and later non-overlapping fetches still correct values, enrich coverage, and repair sparse rows; an entirely-rejected payload resolves as a truthful no-op with no rewrite or last-success advance; the provider fetch stays OUTSIDE the Postgres transaction. (4) **Degraded sweeps are non-2xx** — a completed cron sweep with any target-local failure now returns 502 with a machine-readable `failedWeeks` list plus the bounded error summary (Vercel cron marks invocations failed only on non-2xx and does not auto-retry, so no duplicate provider calls; nothing consumed the former 200-body error field); 200 is reserved for clean sweeps, provider-wide aborts keep their existing status, committed recoveries remain durable in degraded runs, and the year-rollup provider-status scope stays untouched (the cache-only slate diagnostics remain the operator-visible detail). +16 tests (required-category contract; sparse stored-not-complete/available/analytics-eligible; half-complete row incomplete; complete explicit zeros valid end-to-end; sparse repair; blank-home/blank-away mixed payloads persisting only valid rows with truthful counts and no accumulation; overlap-rejected equal-set and superset replacements; provably-later corrections and enrichment; overlapping additions preserving the union; route-level manual/manual and manual/cron same-game overlap committing exactly once; sparse-only cache absent; degraded-sweep 502s with stable `failedWeeks` and durable prior successes), with the superseded 200-mixed contracts flipped and every complete-row fixture migrated to full six-category maps.
- Review remediation (PLATFORM-086H-STRUCTURAL-EQUALITY-LATEST-APPLICABLE-DIAGNOSTIC-REMEDIATION-v1, separate commit on the branch pre-merge): resolved both Codex P2s. (1) **Structural row equality independent of `jsonb` key order** — the merge's no-op detection (`rowsEqual`) now uses a minimal recursive `structurallyEqual` (no existing repo helper satisfied the contract): object-key order is ignored at every nesting level (Postgres `jsonb` does not preserve insertion order, so an identical prior row can round-trip with reordered keys — a stringify comparison misread every identical refresh of a partial week as a change, rewriting the record and advancing last-success), while array ordering, value types, and values stay significant. Identical refreshes over key-reordered durable records are truthful no-ops again (no rewrite, no last-success advance); genuinely changed values still replace. (2) **Latest-APPLICABLE diagnostic anchor** — the missing-stats warning severity now anchors on the newest completed stat slate whose expected set is NONEMPTY: a newer no-expected slate (positively classified FCS-vs-FCS-only, unverifiable-only) no longer absorbs the "latest" slot and silently downgrades the newest applicable missing slate to backfill info; older applicable slates keep their historical/backfill severity and unverifiable diagnostics stay separately visible. +7 tests (jsonb-style key-order round trip → merge no-op; reordered-key equality still detecting changed values; array order significant; manual-route identical refresh over a key-reordered durable record → truthful no-op without last-success advance; newer FCS-only and unverifiable-only slates not downgrading the applicable missing warning, which anchors on the applicable week). Recognized-category authority, superset replacement, durable locking, fairness, placeholder lifecycle, and all other contracts untouched.
- Review remediation (PLATFORM-086H-RECOGNIZED-AUTHORITY-DURABLE-ATOMICITY-FAIRNESS-REMEDIATION-v1, separate commit on the branch pre-merge; scoped by the read-only PLATFORM-086H-AUTHORITY-ATOMICITY-RECOVERY-FAIRNESS-AUDIT-v1, closing all three Codex ADVERSARIAL findings): (1) **Recognized-category authority per side** — new `RECOGNIZED_STAT_CATEGORIES` exported from `normalizers.ts` (exactly the 24 wire keys `normalizeTeam` consumes, colocated with a contract test so validation and normalization cannot drift); `isAuthoritativeGameStatsRow` now requires usable identity PLUS ≥1 RECOGNIZED category on the home side AND ≥1 on the away side — one-sided data and unknown/renamed-categories-only data are no longer authoritative (their normalized metrics are fabricated zeros), while explicit zero values of recognized categories still count and unknown categories stay visible in `raw` for debugging without establishing authority; the per-side superset replacement rule (`coversPriorCategories`) now compares RECOGNIZED sets only, so an unknown key lingering in a prior row can no longer block a legitimate replacement. `classifyGameStatsPayload` gains `no-authoritative-rows`: a nonempty payload with usable identity but zero authoritative rows is a VISIBLE target-local schema-drift failure (`game-stats-no-authoritative-rows`, 502 on the manual route) — never silently accepted as zero-filled data and never conflated with legitimately unpublished stats (an EMPTY payload remains a no-op); mixed payloads commit their authoritative rows while the merge drops the drifted ones. This supersedes the earlier identity-only-payload-as-no-op contract. (2) **Cross-instance durable atomicity** — new `withAppStateKeyLock(scope, key, fn)` in `appStateStore.ts`: on Postgres, one dedicated pooled client runs BEGIN → `pg_advisory_xact_lock(hashtextextended(scope||'/'||key, 0))` → callback reads/writes through that same client → COMMIT (rollback + release on failure; xact-scoped locks are transaction-pooling-safe and cold-key-safe, and release automatically so a failed writer cannot strand them); the file fallback runs the callback inside the existing whole-file mutex with direct store access. `withDurableGameStatsWeek` wraps it for the weekly record, and BOTH the manual route and the cron now run fresh-read → merge → write inside that durable boundary (provider fetches and status recording stay outside; commit time/seq captured inside) — the in-process `withGameStatsWeekLock` is demoted to an explicitly optimization-only role and its cross-instance claim corrected. **Verification limitation (stated, not implied covered): default CI runs the file-backed contract (serialization, disjoint-row union, cold-key safety, failure release — all tested); true cross-instance Postgres advisory-lock behavior is not provable without a live database and is not exercised in CI.** (3) **Bounded recovery fairness** — target-local validation failures (`no-usable-rows`, `no-authoritative-rows`) now record the week-scoped failure and CONTINUE to the next candidate (one bounded call per candidate per run, so a persistently bad newest slate can never starve older incomplete weeks — no cursor or persisted state needed), while provider-wide thrown failures (transport/auth/quota/durable) still abort the remaining candidates, which keep their truthful `deferred` outcome; a completed sweep returns HTTP 200 with per-week failures preserved in `results[]` plus a bounded top-level `error` summary, so a mixed run never reads as total success and non-2xx stays reserved for provider-wide aborts. +12 tests (recognized-category contract lock; per-side authority matrix incl. explicit zeros, one-sided home/away, unknown-only, mixed; one-sided rows excluded from completeness/availability/analytics and repaired by a later authoritative row; unknown prior keys not blocking recognized-superset replacement; identity-only and unknown-only payloads → `no-authoritative-rows` on classify and 502 on the manual route; mixed payload committing while dropping drift; poison newest slate failing target-locally while the older slate is recovered in the same sweep with a truthful mixed 200; provider-wide throw still deferring older candidates at 500; `withAppStateKeyLock` union/cold-key/failure-release), with the two superseded contracts (cron whole-run 502 abort; identity-only no-op) explicitly rewritten.
- Review remediation (PLATFORM-086H-CANONICAL-STAT-AUTHORITY-PLACEHOLDER-LIFECYCLE-REMEDIATION-v1, separate commit on the branch pre-merge; scoped by the read-only PLATFORM-086H-STAT-AUTHORITY-PLACEHOLDER-LIFECYCLE-SEAM-AUDIT-v1): resolved all three Codex P2s with one canonical authority contract and a bounded placeholder lifecycle. (1) **Canonical stat authority** — new exported `isAuthoritativeGameStatsRow` (usable identity + ≥1 provider-present category from the per-team `raw` maps, recorded before omitted fields normalize to zero; explicit zeros count; never a nonzero-value heuristic) is now the ONE definition shared by completeness coverage (`usableGameStatsGameIds`), cache availability (via that same function), and owner analytics (`aggregateOwnerGameStats`/`aggregateOwnerSeasonStats` gain a defensive boundary filter, because durable legacy rows predate ingestion rules) — deliberately NOT used by `classifyGameStatsPayload`, so an identity-only payload stays a merge-level no-op rather than a 502. Legacy identity-only rows (a shape pre-086H ingestion could persist from `stats: []`) therefore no longer mark a week complete, no longer read as available cache content, no longer inflate analytics, and are repaired NATURALLY: the uncovered week is re-fetched and the superset rule replaces the empty-raw row, while a repeat identity-only response is a truthful no-op and failed recovery leaves prior rows untouched — no migration, no destructive cleanup. (2) **Per-side category-set replacement** (`coversPriorCategories`) replaces the aggregate-count comparison: a usable incoming row replaces a usable prior row only when its home AND away category sets each contain every prior category (equal sets qualify, incoming values win per category; strict subsets, equal-count different sets, and complementary sets retain the prior — cached categories can never be erased behind normalized zeros; complementary maps are deliberately not field-merged into a synthetic row). (3) **Kickoff-gated placeholder recovery**: a placeholder label (TBD/TBA/winner-of/…) suppresses expectation only while the game has not provably completed — once the row's parseable `startDate` is past the ONE shared completion cutoff (`COMPLETED_GAME_MATURITY_MS`, now exported from coverage and consumed by the slate helpers — no second hard-coded policy), the game is expected by its schedule-provided numeric id, so a schedule cached before a postseason matchup resolved can no longer permanently suppress recovery (identity still comes only from the schedule row; participant display names stay stale until a schedule refresh — manual today, 086E1 later — which affects naming only, never recovery; dateless/unparseable dates keep suppressing; cache `at` timestamps are deliberately NOT used as freshness proof). An explicit `now` is threaded through `deriveExpectedGameStatsIds`/`evaluateWeeklyGameStatsCompleteness` and both cron and diagnostics pass the same instant they use for slate completion. +18 tests (authority predicate incl. explicit zeros; legacy row not covered / naturally repaired / repeat-statless no-op; equal-count different sets, complementary sets, strict subset retained; strict superset and equal sets replacing; per-side independence; analytics boundary incl. authoritative zeros and season aggregation; mature stale-placeholder expected in lib + recovered by cron + reported by diagnostics in aggregate AND partition layouts; pre-maturity and dateless placeholders suppressed; mature synthetic-id placeholder unverifiable; cache availability absent-vs-available) and fixture migrations (diagnostics/cron/insights fixtures gain provider-present `raw` maps — the insights alias-precedence fixture previously encoded the pre-authority contract).
- Review remediation (PLATFORM-086H-STAT-AUTHORITY-SCHEDULE-CACHE-PARITY-REMEDIATION-v1, separate commit on the branch pre-merge): resolved both Codex P2s. (1) **Merge identity is now separate from STAT AUTHORITY** (`mergeWeeklyGameStats` + new `statFieldsPresent`): authority is the count of provider-PRESENT stat fields, read from the normalizer's per-team `raw` category map — which records exactly the wire-supplied categories BEFORE omitted fields are normalized to zero — never a nonzero-value heuristic (an explicit "0" is present and valid). An identity-only row (valid id + team names, zero present fields) never replaces any prior row and is never persisted as new (`rowsDroppedStatless`); a row with WEAKER present-field coverage than the usable prior row retains the prior, so a partially published CFBD response can no longer regress real totals to zero-filled rows; equal-or-stronger coverage replaces exactly as before; a usable row still repairs an identity-unusable prior (that row is unreachable by owner aggregation regardless); keyless dropping and all prior merge rules unchanged. The manual route's no-change guard now covers the prior-null case (`if (!merge.changed)`), so an all-dropped payload on a cold key resolves as a truthful no-op instead of committing an empty record and advancing last-success. (2) **Shared schedule-cache view** (`providerDataDiagnostics.ts`): diagnostics now source schedule items from the SAME canonical partition-fallback loader the cron uses (`loadCachedScheduleItems`: `-all-all`, else the `-all-regular`/`-all-postseason` pair — no duplicated fallback logic), so a partition-only cache layout no longer reports "No current-season schedule" while the cron actively recovers its slates; the aggregate entry is still read separately for its refresh metadata (partial-failure flag, age) and those checks are simply skipped when only partitions exist; `deriveApplicableScoreSeasonTypes` loosened to a structural param (seasonType-only) to accept both item shapes; behavior with no supported cache is unchanged (schedule error + cron "no completed weeks"). +12 tests: identity-only rows never replace populated priors and are never persisted cold (lib + route-level cold-key no-op with no last-success advance); weaker coverage retained; explicit zeros valid and replacing at equal coverage; stronger coverage replacing; keyless accounting unchanged; diagnostics judge regular-only, postseason-only, and combined partition layouts (no false "no schedule", stat slates reported); cron recovers a partition-only layout via the same loader. Wire fixtures across cron/manual suites upgraded to stat-bearing rows (a `stats: []` row is now correctly identity-only).
- Review remediation (PLATFORM-086H-KEYLESS-ROWS-UNVERIFIABLE-DIAGNOSTICS-REMEDIATION-v1, separate commit on the branch pre-merge): resolved both Codex P2s in the recovery path. (1) **Keyless provider rows are never persisted** (`mergeWeeklyGameStats`): an incoming row whose provider game id is missing/zero/negative/non-numeric can never produce the canonical merge key, so it could never be replaced or deduplicated — every recovery run of a still-incomplete week would append another copy and inflate downstream owner aggregation (`aggregateOwnerGameStats` counts rows without validating ids). Such rows are now dropped before persistence (surfaced as `rowsDroppedKeyless` on the merge result), no fallback key is ever constructed from team names, prior-good keyed rows and the unusable-row non-clobber rule are unchanged, and a repeated malformed payload is a truthful `no-change` no-op. (2) **Unverifiable slates stay visible** (`providerDataDiagnostics.ts`): the game-stats section now carries the full `ExpectedGameStatsSlate` (not just `expectedIds`) per completed stat slate and emits a distinct warning — "N scheduled game(s) without a provider-addressable id — their stats cannot be verified or recovered automatically" — for any slate with `unverifiableCount > 0`; an unverifiable-only slate is no longer silently skipped, mixed slates report BOTH ordinary missing coverage and the unverifiable rows, unverifiable rows are still never classified as ordinary missing stats, and the cron denominator/retry behavior is untouched (no endless retries). +6 tests: merge drops keyless rows while persisting valid + prior-good rows; repeated malformed recovery never accumulates duplicates (lib + full cron path, second run `no-change`); dropped rows cannot inflate `aggregateOwnerGameStats` (gamesPlayed stays 1 across double recovery); unverifiable-only completed slate warns without the ordinary missing wording; mixed slate reports both. Existing zero-unverifiable behavior (complete / no-expected-games / full-coverage silence) covered by unchanged green suites.
- Review remediation (PLATFORM-086H-DETERMINISTIC-COVERAGE-SLATE-ELIGIBILITY-REMEDIATION-v1, separate commit on the branch pre-merge; scoped by the read-only PLATFORM-086H-GAME-STATS-CLASSIFICATION-SLATE-SEAM-AUDIT-v1): resolved all three Codex P2s by SIMPLIFYING the seam — expected coverage is now deterministic from pure schedule inputs plus the bundled static conference policy, with no identity-evidence load path. (1) **FBS-vs-FCS retention + determinism**: the resolver-backed participant test was removed (the team catalog is FBS-only, so negative resolver evidence could silently drop a real FCS opponent with missing/unrecognized conference metadata from expectations — false completeness, suppressed recovery, strictly worse than a bounded retry) and `inferSubdivisionFromConference` (which falls through to the mutable CFBD conference index that `buildScheduleFromApi` loads/resets per request) was replaced with the pure static `isPolicyFcsConference` at both call sites. A side now blocks expectation only on POSITIVE placeholder evidence via the shared `isPlaceholderTeamLabel` (patterns extended with `TBA` / "to be announced" / "to be determined", closing the prior "Home Team TBA" example positively and for every predicate consumer); pattern-valid unknown participants stay expected — the recovery-safe direction — and FCS-vs-FCS is excluded only when BOTH conferences positively classify FCS under the static policy (verified to cover all 15 current D1 FCS conferences). `GameStatsIdentityEvidence`, the `identity` parameter, and `src/lib/server/gameStatsIdentityEvidence.ts` are deleted; provider-addressable id validation and `unverifiableCount` unchanged. (2) **Slate completion split from dataset eligibility** (`providerDataDiagnostics.ts`): one private grouped whole-slate-cutoff core now backs two named exports — `deriveCompletedSlates` (ALL games; score diagnostics, so postponed/suspended/delayed slates stay visible to missing-final checks — the prior stat-filtered version silently dropped them) and `deriveCompletedStatSlates` (stat-producing games only; the cron and the game-stats diagnostics section share this exact export, preserving the identical-candidate-set guarantee without contaminating scores). Tests: lib coverage reworked (+placeholder-label matrix incl. TBA variants; FBS-vs-unknown with empty and unrecognized conference stays expected; FBS-vs-known-FCS expected; static FCS-vs-FCS excluded; derivation identical before/during/after `setConferenceClassificationRecords`/`reset` mutation), diagnostics helper tests split per consumer (disrupted-only slate completed for scores but never a stat slate; rescheduled-postponed kickoff blocks the scores view but not stat ingestion) plus a route-level postponed-only-slate regression (scores diagnostic present, game-stats absent); cron/pause/manual/merge/panel suites unchanged and green.
- Review remediation (PLATFORM-086H-CANONICAL-PARTICIPANTS-SLATE-CUTOFF-REMEDIATION-v1, separate commit on the branch pre-merge): resolved both Codex P2s against expected-coverage identity and candidate selection. (1) **Resolver-backed participant classification**: expected-coverage derivation no longer relies on string patterns alone — `deriveExpectedGameStatsIds`/`evaluateWeeklyGameStatsCompleteness` accept injected `GameStatsIdentityEvidence`, built server-side by the new `loadGameStatsIdentityEvidence` (catalog + league-agnostic alias resolver with deliberately NO observedNames seeding, consulted through the canonical `buildPlaceholderParticipant` — the exact 086G2 identity-uncertainty pattern, no new identity system). A side is a real participant only when it passes the shared pattern predicate AND either resolves canonically or its conference positively classifies FCS (a real FCS opponent is never in the FBS catalog, and FBS-vs-FCS games DO produce stats — so the FCS side is evidenced by classification while the FBS side must resolve); a pattern-passing junk label ("Home Team TBA") is now non-expected, ending its false missing/partial diagnostics and repeated cron fetches. Evidence-load failure → null → the pattern-only fallback, chosen deliberately because over-expecting retries a week while under-expecting would falsely complete it and suppress recovery; identity unavailability never suppresses recovery. Both the cron and diagnostics load the same evidence. (2) **Whole-slate completion cutoff**: the cron's per-game 6h threshold (which let an old Thursday final make a still-active split slate a candidate mid-Saturday) is replaced by the diagnostics' `deriveCompletedSlates` — now exported as the ONE shared definition (grouped per slate first, cutoff applied to the per-slate max kickoff after) and refined to group only STAT-PRODUCING games (shared `expectsGameStats`), so a disrupted game's stale or rescheduled kickoff never decides slate completion and a disrupted-only slate is never completed for either consumer. +9 tests (lib: "Home Team TBA" non-expected, one-resolved/one-unresolved non-expected, both-resolved expected, FBS-vs-FCS retained via conference evidence, FCS-vs-FCS still excluded with evidence, null-evidence fallback keeps the pattern-only expectation; cron: split slate not a candidate until its latest game passes the cutoff then recovered; diagnostics: direct shared-helper contract test — split-slate exclusion, disrupted-kickoff exclusion, newest-first) and schedule fixtures re-based on real catalog teams (made-up labels with unknown conferences are now correctly non-expected); two pre-existing diagnostics fixtures also gained provider-addressable numeric ids their non-numeric labels lost under the prior id remediation.
- Review remediation (PLATFORM-086H-CODEX-P2-COVERAGE-ATOMIC-MERGE-REMEDIATION-v1, separate commit on the branch pre-merge): resolved both Codex P2s against the completeness and retention guarantees. (1) **Provider-addressable expected ids** (`deriveExpectedGameStatsIds`): `mapCfbdScheduleGame` synthesizes fallback ids (`${week}-${home}-${away}`) when CFBD omits `game.id`, but coverage rows only ever carry positive numeric provider ids — so a synthetic/missing/zero/negative/malformed schedule id could never be covered and would leave its slate permanently incomplete (endless weekly re-fetch). Only positive-integer ids now enter `expectedIds`; otherwise-expected rows without one are counted as `unverifiableCount` on the slate contract (diagnostic visibility) and stay out of the completeness denominator, so they neither suppress completeness of the verifiable coverage nor create permanent partial coverage. Disrupted/placeholder/FCS-vs-FCS rows are classified BEFORE the id check, so a non-stat-producing row is never mislabeled "unverifiable". Identity is still never inferred from returned stat rows. (2) **Atomic per-week read→merge→write** (new `withGameStatsWeekLock` in `gameStats/cache.ts`, same promise-chain mutex shape as the provider-status `withScopeLock` / odds `withOddsTargetLock`): both the cron and the manual refresh now run the fresh prior read → merge → durable write as one critical section keyed by the canonical game-stats cache key, capturing commit time/seq inside the section — two overlapping refreshes (manual/manual or cron/manual) can no longer read the same prior record and have the later write drop the earlier one's rows; overlapping refreshes adding different games produce the union. A failed operation propagates but never poisons the chain (later refreshes proceed), the durable write remains the publication step (no separate process cache), provider-status attempt ordering is unchanged, and the lock is documented IN-PROCESS ONLY per the standing 086A limitation (cross-instance stays last-writer-wins). +9 tests: lib coverage +5 (synthetic/zero/negative/malformed/missing ids unverifiable; valid numeric accepted; disrupted synthetic-id row excluded not unverifiable; mixed rows complete on verifiable coverage; unverifiable-only slate expects nothing), new `cache.test.ts` +3 (same-week serialization, cross-week independence, failure releases the lock), cron +2 (synthetic id never causes endless recovery; overlapping cron+manual union), manual route +2 (overlapping manual/manual union; failed durable write surfaces as a failure and does not poison the next refresh); pause-test schedule seeds moved to numeric provider ids.
- Review remediation (PLATFORM-086H-GAME-STATS-NOOP-WORDING-REMEDIATION-v1, separate commit on the branch pre-merge): resolved both Codex P2s — the panel's no-op wording inferred more than the response proves. (1) **Neutral no-op wording** (`describeGameStatsRefreshResult`): the manual-refresh response carries no schedule-relative completeness evidence, so a `no-new-rows` no-op no longer claims the week is "already up to date" (a partial week can echo only its already-cached subset and stays recovery-eligible) — it now says "No new game stats were added … N cached games retained"; and a `no-provider-rows` no-op no longer claims "No game stats available" when `rowsCached > 0` — it reports the provider returned no rows with existing cached rows retained, claiming "nothing is cached yet" only at `rowsCached === 0`. (2) **Backfill accounting** (new pure `classifyBackfillResponse` + `describeBackfillSummary`): buckets branch on `noopReason`, not just `outcome`, so unchanged weeks (`no-new-rows`) are reported separately from genuine provider-empty weeks (`no-provider-rows`) and are never counted as "no stats available"; legacy/contract-less responses keep counting as updated. Panel-only — no backend recovery, merge, completeness, cron, or API change. Panel tests extended to 9 (neutral wording incl. retained-rows provider no-op and the completeness-claim regression; bucket classification incl. legacy/null bodies; summary wording incl. omitted empty categories).
