...
- Status: **Implemented — review pending — not merged.** Branch `platform/086h3-atomic-game-stats-contract-activation` from `main` @ `c8ebed4`; pushed with `preview` updated; no PR opened. Awaiting independent Codex review.
- Purpose: Atomically activate the dormant PLATFORM-086H1 contract and PLATFORM-086H2 durable merge authority as the ONE production game-stats lifecycle: canonical schedule → validated provider observations → durable merge authority → committed durable state → coverage and schedule-relative recovery → cache/read model → analytics projection → truthful availability. Governing invariant honored: **every game-stats writer routes through the durable merge authority (or the same per-partition transaction-scoped lock)** — no partial activation, no parallel writer system, no blind partition overwrite.
- Result: New `src/lib/gameStats/ingestion.ts` (validated ingestion: strict payload validation through the single contract parser, typed `invalid-payload` / `schema-drift` / `valid-empty` (expected vs contextually unexpected) / `unmatched-only` / `no-persistable-observations` / `merged` outcomes, canonical-schedule slate expectations with placeholder deferral and disrupted exclusion, matching by provider game id only — provider statistics can never create games and no team-name equality exists anywhere in the lifecycle); new `src/lib/gameStats/partitionCoverage.ts` (ONE committed-durable-state coverage model over the typed contract: per-game `satisfied`/`recoverable`/`manual-only`/`blocked`/`absent`, partition `complete`/`partial`/`absent`/`not-applicable`; analytics ELIGIBILITY is the sufficiency bar, so legacy-compatible rows stay covered — auto-refetching every legacy week would be a de-facto bulk migration, which remains a separately queued task); new `src/lib/gameStats/recovery.ts` (schedule-relative recovery planning: canonical scheduled games → durable inspection → typed coverage → bounded candidates newest-first; the weekly cron consumes exactly ONE candidate per run, satisfied slates are never refetched, placeholders defer, blocked schema rows never loop); new `src/lib/gameStats/publicProjection.ts` (public wire projection stripping `schemaVersion`/`fetchStartedAt`/`pointsProvided`; legacy rows pass through **by reference**, preserving the 086H2 byte-equivalence result). Both writers migrated: the cron (`/api/cron/game-stats`) plans recovery over committed coverage and the manual admin refresh (`/api/game-stats?bypassCache=1`) derives schedule expectations BEFORE the provider fetch (schedule unavailable → 409 before quota is spent); both route payloads through ingestion into `mergeGameStatsPartitionDurable` and map every durable outcome truthfully (`written`/`partially-merged` → success recorded only after COMMIT; `unchanged`/`stale` → no-op; `conflict` → failure with stored rows preserved; `unavailable` → failure with durable state untouched; `indeterminate` → failure stating durability is UNKNOWN, nothing published, retry safe and idempotent). The blind overwrite `setCachedGameStats` is DELETED (cache.ts is read-only; tests seed via a test-only fixture helper). Ordinary `/api/game-stats` reads are now cache-only for every caller (admin implicit fetch-on-stale removed — provider access is exclusively the explicit authorized bypass), serve the public projection, distinguish fresh/stale/miss/refresh-required, and report corrupt or unreadable durable state as typed 500s (`game-stats-durable-read-failed`, `game-stats-durable-state-invalid`) — never as absence. Owner analytics (`ownerStats.ts`) consumes `selectAnalyticsRows`/`toAnalyticsGameStats` exclusively (ineligible rows excluded instead of aggregated as fabricated zeroes; deterministic duplicate selection; output-equivalent for the validated 2021–2025 legacy inventory); the debug game-stats diagnostic aggregates through the same projection; provider-data diagnostics consume the shared partition-coverage model (message timing preserved via whole-slate completion gating). The legacy normalizer has no production importer (fixture-only). The dormant-boundary guard is replaced by `activation-guards.test.ts`: a writer-bypass scan (retired setter name, direct `game-stats` durable writes, independent `withAppStateKeyTransaction` users, resurrected normalizer imports, writer routes referencing write primitives) plus an activation-completeness guard (ingestion→merge→coverage→recovery→projection wiring asserted structurally) with scanner self-tests. End-to-end lifecycle tests (`lifecycle-activation.test.ts`, 19 scenarios) cover the full path plus: partial-batch preservation, fence-only refresh, stale late writer, disjoint concurrent writers converging, expected/unexpected empty, invalid payload/schema drift, unmatched (FCS-vs-FCS exclusion) and unresolved-identity observations, provider unavailability, failed durable write publishing nothing, indeterminate COMMIT/ROLLBACK uncertainty with safe idempotent retry (scripted fake pg pool), placeholder deferral, postseason recovery, zero provider calls from public reads, metadata-free wire output, corrupt-store failure + recovery. Legacy rows continue to serve through compatibility reads with **no migration required**. Full suite 1822/1822; `tsc` and `lint:all` clean. Deliberately NOT included: PLATFORM-086H4 diagnostics/panel presentation redesign (queued) and the legacy-row migration (queued).
