...
- Scope limit: store and route only; the applied timeline is byte-identical by construction. `getProviderRefreshSettings` throws only when the store does, and this writes through that store — so a `settings-unavailable` hold is traced only when the failure was confined to the settings read ([#619](https://github.com/znpruitt/cfb-app/issues/619)'s limit, inherited).
...
- Review / verification: both reviewers found one root — an unbounded transaction inside the job loop, ahead of an unplanned job, whose failure also downgraded an all-applied run to `partial`. Moved after planning and classification; reason union enforced at the sink. Delta +18 (5,198 → 5,216 measured), not the +23 first reported. Reading the cause off the null has no red state.
