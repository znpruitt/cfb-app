...
- Purpose: `PLATFORM-861: the FIRST year runs even past the budget` mocked `Date` but not `setTimeout`, so the venue leg's three attempts waited out `fetchUpstream`'s REAL backoff sleeps — 713–883 ms on a file whose ENTIRE budget is 30 s. The cost is TEST-HARNESS INTEGRITY: when the budget goes the FILE is cancelled mid-flight rather than an assertion failing, and every later test in it never runs.
- Scope: `api/cron/schedule-presentation/__tests__/stall.test.ts` only. No `src/`, route, budget-constant or assertion-meaning changes.
- Outcome: `setTimeout` mocked alongside `Date` and driven by the existing `tickUntilSettled`; 713–883 ms → **9.17–10.95 ms**, file 1833–1959 ms → 920–1186 ms. **The prompt's central trap was wrong and was corrected by measurement before any code was written** — it said there was "no clock to drive", while its own acceptance 1 instructs mocking `setTimeout`, which creates that clock. The real trap is the inverse: ticking advances the mocked `Date`, which in this file IS the quantity the budget assertions read. **The prompt named one test; two pay, and the unnamed one costs more** (714–904 ms) and keeps its cost by ruling, because ticks land before its 8 s admission capture. `--test-timeout` was measured to apply per FILE, not per test.
- Review / verification: both reviewers on `500c349b`, gathered before remediation and run SEQUENTIALLY with a verified-clean tree between them. `/codex:review --base 15299121` found no actionable defects and **disclosed that its runtime verification did not happen** (sandboxed `test:file`, exit 1), so it is recorded as a static-only pass; verified by exit code, transcript diff base and body. `/code-review high` returned one medium and three lows, all accepted, one **attribution corrected by measurement**: the stale tick-budget range was already stale at eight-way contention on a caller this branch never touched (ACCEPTANCE 9 reached 21, against the recorded 6–13), so the new caller raised the floor and p95, not the maximum. Cap floor 30 → 45. One remediation round.
- Controls: Acceptance 2 is **inherited and named** from the two #872 control tests rather than restated; the harness killing a file has **no constructible control and cannot get one**, so the fix is recorded as a probability reduction, not a guarantee. Acceptance 1 is checked deterministically against the defect's floor rather than inferred from a green run. Every claim carries a mutation that reddened its own named assertion. [Measurements, the sweep, the rejected alternatives and the full mutation table](campaigns/platform-875-real-time-stall-closeout.md). L2, L4 and L6 apply.
- Status: Merged [PR #878](https://github.com/znpruitt/cfb-app/pull/878), `e2b3ac90`, 2026-09-25; the merged tree hash matched the gated one. No deployment claimed — test-only, and `preview` was deliberately not taken. Residuals: the boundary test's 714–904 ms, **56 unmeasured** `Date`-only enables in `rankings/__tests__/`, and two test constants restating unexported production values.
...
- Review / verification: Both Codex and `/code-review` found only two obsolete Schedule assertions at `2f5c78c4`. Round-1 remediation `0b7318cb` updates those expectations; both confirming reviews reported no remaining issues but missed vertical clipping in the harness. Round 2 at `4d064d7e` exposed that blind spot and the missing JSDOM live-span pin. The owner authorized a bounded second, proof-only correction on 2026-09-24; production code stayed frozen in that correction. Round 3 then authorized only the 900px fixture and a template-token separator space; evidence and deferrals #868/#869 are in the closeout. At clean `0b7318cb`, full suite, required browser gate, TypeScript and pre-push `lint:all` all exited 0. Nine new browser tests; each claim has an inspected mutation failure. [Population, pixel measurements, mutation assertions and review evidence](campaigns/platform-797-header-wrap-closeout.md). L1, L4 and L6 apply.
- Status: Merged [PR #864](https://github.com/znpruitt/cfb-app/pull/864), `37354e6d`, 2026-09-24. Final correction gates at `81f5c986`; #870 deferred, DESIGN rationale resolved in `dd69113f`. No production promotion claimed.

### PLATFORM-861-ELAPSED-CAPTURE-CLAUDE-v1

- Purpose: schedule-presentation's per-year admission check reused an `elapsedMs` captured BEFORE an awaited durable read, so under a degraded store it could under-count elapsed by up to ~60s at exactly the moment it decides whether another year fits (**recorded as 15s; corrected by PLATFORM-866**, which composed the four sequential bounds one `getAppState` actually carries, and which then found even that is a FLOOR rather than a ceiling). The cost is BUDGET CORRECTNESS — the job admits a year it was designed to refuse. **An intermediate version of the #866 record claimed the under-count crossed the 300s ceiling at 60s but not at 15s; review round 2 refuted that**: the 242s figure is CFBD time only, the year's own dozen store round trips are degraded in the same scenario, so the ceiling is reachable at every figure and is a separate pre-existing residual. Measure elapsed after the await. 757a's own closing-round finding, and the gate on installing §8i.
...
- Status: **MERGED 2026-09-23.** Ships with ONE known low finding, left unfixed on purpose and tracked as
  ...
  margin erosion, no traced case breaches 300s, one-line fix. **The 15s figure is superseded: the read is at
  least ~60s (four composed 15s bounds) — see #866. Ceiling reachability under a degraded store is a separate
  pre-existing residual, not something this under-count introduced.** Runbook §8i gates installing the QStash
...
- Status: **MERGED `2b167998` ([PR #859](https://github.com/znpruitt/cfb-app/pull/859)) 2026-09-22; #827 closed. NOT LIVE until promoted.** Merged tree verified identical to the gated commit `ab60d96f` (`29500b23`). The prompt-registry conflict at merge retained both entries. Residue: [#851](https://github.com/znpruitt/cfb-app/issues/851) (movement endpoints), [#855](https://github.com/znpruitt/cfb-app/issues/855) (caption policy), [#856](https://github.com/znpruitt/cfb-app/issues/856) (palette consistency).
...
- Outcome: the read-only schedule/roster observer found 888 games, 1,776 labels and 238 distinct
  rendered schools, all 238 covered. CFBD supplied 679 abbreviations and three explicit nulls:
  Chicago State (FCS), Ohio Dominican (Division II) and Schreiner (Division III); Chicago State
  therefore returns null. `odds-team-mascots.ts` already contains all 128 FCS abbreviations, but
  merges them into a sorted alias array and discards provenance, so it cannot power this fallback.
- Review / verification: the real `selectDraftTeamInsights` / `getFBSTeams` observer stayed aligned,
  ...
  count: **15 direct, 1 indirect, 3 tests**; planning's 41 and 19 were both grep artifacts. The three
  draft consumers differ: the commissioner and spectator boards would widen through
  `selectDraftTeamInsights`; summary uses the database rows only for its display-name map; owner
  validation filters back to FBS through `getFBSTeams`. Both independent reviews found no actionable
  issue and confirmed the mutation's specific mismatch, not the artifact row count. TypeScript,
  ESLint and focused tests passed. Full suite had 5,456 passes plus three sandbox-blocked failures /
...
- **THIS BRANCH CONFLICTED WITH TWO BINDING CORE RULES IN `AGENTS.md`. RESOLVED BY OWNER AMENDMENT
  `91aa30a3`, MERGED INTO THIS BRANCH AT `6919b5fd`.** `AGENTS.md` is canonical for binding rules and
  wins over every other document, so this was a stop-and-report, not a thing to resolve in the lane.
  Measured 2026-09-20 — five symbols named in `AGENTS.md` core rules had **zero** production call sites:
  `readComposedWeekAllEntry`, `resolveChildCache`, `fetchSeasonType`, `hasRequiredSeasonTypeFailure`,
  `scheduleRefreshScope`. What the document required, before the amendment:
  ...
  **The CODE was what the ruling decided; the DOCUMENT was the thing that had gone wrong.** Both
  invariants were right about WHAT MUST NOT HAPPEN — a targeted repair must not be invisible, and a
  sibling partition's failure must not cross-contaminate another target's status — and aggregate-only
  achieves both by removing the second target rather than by scoping it. `91aa30a3` amends both bullets
  on exactly that basis: **each keeps its intent, drops the mechanism, and says not to restore the
  mechanism to satisfy it.** 085B/085C keeps uncertainty-retains-prior-good, zero-length-is-valid-absence
  and never-commit-partial-rows, and states that completeness is now validated once in the full-season
  refresh instead of per requested partition. 086A reduces to "schedule records the **`year`** scope and
  nothing else", replacing ~2,200 characters of week-partition machinery, and records that if a targeted
  repair is ever wanted again it is a NEW decision with this history attached.
  **Compliance re-verified after the merge:** `/api/schedule` records no provider-refresh status of its
  own (zero `beginProviderRefreshAttempt` / `record*` calls), and the authority records
  `yearScope(year)` and nothing else (`fullSeasonScheduleRefresh.ts:302`).
  **The five symbols still appear once each in `AGENTS.md`, and that is now HISTORY rather than a
  mandate** — the amended bullets name them to say they are dead code pending removal. A future grep
  finding them there should not read it as a violation signal, which is precisely the trap the first
  version of this entry would have set.
- **CODEX'S P2 ON TARGETED REFRESH SCOPE STAYS UNAPPLIED, DELIBERATELY, AND THAT IS NOT A SILENT
  NON-FIX.** It offered two remedies — "either reject targeted refresh shapes or preserve their
  partition scope while converging the resulting data" — and **both undo the ruling.** Rejecting
  targeted shapes breaks `?week=`/`?seasonType=` reads that the projection now serves correctly;
  preserving partition scope reintroduces the second status target whose removal is the entire point,
  since a second target is what made a sibling failure able to cross-contaminate. The finding correctly
  identified a real conflict with `AGENTS.md` as it then stood; the conflict was in the document, and
  the owner amendment is the resolution. **Recorded as accepted-diagnosis / rejected-remedy rather than
  as a fix, so a later reader does not see an unaddressed P2.**
- **FOUR ITEMS STOPPED AND REPORTED RATHER THAN DECIDED.** (1) Naming the remaining four precedences as
  ...
  centralised; the file is out of scope, and the amended 086A bullet is what now carries the "one
  target" rule that makes a second spelling of the key a drift risk rather than a style point.
  (3) **The five dead symbols and the two orphaned modules are filed as
  [#833](https://github.com/znpruitt/cfb-app/issues/833), for after this merges** —
  `src/lib/scheduleSeasonFetch.ts` (whose docstring calls itself the _"Single source of truth for the
  schedule empty-response policy, shared by the authorized `/api/schedule` route and the season-transition
  cron so the two can never drift"_ while `classifyEmptyScheduleRefresh` and
  `hasRequiredSeasonTypeFailure` have no production callers and the policy lives inline at
  `fullSeasonScheduleRefresh.ts:151-157`) and `scheduleRefreshScope` (`providerRefreshScope.ts:232`,
  whose throw message describes a path that no longer exists). **The file claiming to prevent drift is
  the copy nobody calls** — #693's "a helper nothing called", arriving by deletion of the caller rather
  than by addition. **Scope was deliberately NOT widened to reach them, including the false docstring**,
  on owner instruction: a one-line docstring fix looks free and is exactly how a "while I'm here" edit
  grows past a reviewed boundary, and the whole set is one coherent removal that belongs in its own
  slice rather than smuggled into this one. (4) `schedulePresentationJoin.ts:4` still lists "composed
  week/all responses, targeted refresh responses" among the paths it enriches; both were deleted here,
  and it goes with #833 for the same reason.
- **ONE FINDING ACCEPTED, DIAGNOSIS AND ALL, AND LEFT OPEN RATHER THAN PARTLY FIXED.** Codex P2: when
  canonical data exists ONLY in a populated legacy `-all-regular` partition, a refresh returning `[]` is
  classified by the authority against `${year}-all-all` alone, so it resolves as a genuine-absence no-op
  while the pair still serves rows. Pre-#663 the targeted path consulted those rows and rejected the `[]`
  as an unexpected empty replacement. The round-1 fix removes the related 200-with-zero-rows symptom by
  narrowing "cached and empty", but **the classification itself lives in
  `fullSeasonScheduleRefresh.ts:151-157`, out of scope.**
  **Why it was not narrowed to the reachable part:** the reachable part is the SYMPTOM, not the defect.
  Fixing what the route reports while leaving the classifier saying "genuine absence" about a season the
  store can still serve would leave the two disagreeing, with the honest-looking half in front — the
  same shape as the original #663 defect, where the visible surface was fine and the durable truth was
  not. It is **unreachable in production** (zero pair keys, measured 2026-09-19 and 2026-09-20) and
  reachable in a legacy or preview-branch store, so it is recorded open, with its reachability stated,
...
- **TWO FALSE FIGURES WERE CAUGHT BEFORE MERGE, AND THE METHOD IS THE REUSABLE PART.** (1) "1258 -> 421
  ...
  **The rule both failures point at: measure the ARTIFACT the claim is about, not the working tree
  mid-flight.** For a committed line count that is `git show <sha>:<path> | wc -l`; for a test count it is
  `grep -c '^test(' <file>` against the committed file, never a number carried in the head from an
...
- **A TEST OF MINE DEFENDED THE DEFECT IT WAS WRITTEN BESIDE — the recurring class on this project.**
  "a window request never seeds presentation or probe state" asserted the probe is NOT written. That is
  the stale gate Codex then found as a P2, and my test made it look deliberate: written in the same
  commit as the gate, from the same wrong premise, so it converted a defect into a pinned contract.
  Nothing about it looked weak — it passed, it was specific, and it had a comment explaining itself.
  **A test written alongside the code it tests inherits that code's premise, so it cannot be the thing
  that checks the premise.** It was rewritten to seed an OBSOLETE probe and assert it moves, which is a
  claim about the invariant (the probe follows the aggregate) rather than about the implementation.
- **A SECOND TEST WAS WRONG ABOUT ITS HARNESS, WHICH IS THE QUIETER FAILURE.** It omitted
  `ADMIN_API_TOKEN`, and with no token configured `resolvePlatformAdminDecision` AUTHORIZES outside
  production (`adminAuth.ts:86-90`) — so a test named for the non-admin path ran as admin and exercised
...
- Status: MERGED `9764b70f` (PR #791), 2026-09-15; #776 and #780 closed. Both reviewers gathered and remediated across two rounds.
...
- Review / verification: both reviewers, twice; three lessons worth carrying. **(1) A checked-out
  pooled client had NO `'error'` listener** — `pg-pool` removes its idle one and `pool.connect()`
  adds nothing back — so a socket reset mid-query **took the process down**. **(2) The opener is the
  one statement no server-side bound can cover, because it installs them**: measured **20,062 ms
  against a 5,000 ms** connect timeout. **(3) A naive global `query_timeout` POISONS THE POOL** —
  the timed-out client returns to idle and every later query fails — hence destroy-on-timeout and
  [#788](https://github.com/znpruitt/cfb-app/issues/788) filed, not taken. `keepAlive` was also off:
  now on, **detection, not a bound**. **See L4** — my seam-reset test was VACUOUS (5 ms never beats
  an immediately-resolving fake), caught by running the mutation. **Every fake `pg` client is now an
  `EventEmitter`; a fake lacking that surface is what let (1) ship.** `lint:all`, `tsc`, `npm test`
  each exited 0; delta **5,354 -> 5,369**, 0 fail at both ends.
- Status: MERGED `ce5e1b4d` (PR #789), 2026-09-14; #625 closed. Verified by identical tree hash with the branch tip. `preview` suspended by the prompt (no user-visible surface). **Over the 300-word line, breakdown reported at merge** — owner approved these contents explicitly, and every bullet past `Change` is measurement, named limit or open work.
- Post-merge outage, 2026-09-15: the shipped bundle held `15000set local` — the two concatenated literals lost their `;` separator — and every `app_state` read failed `42601`; owner rolled back to `c6b41378`. PR #802 makes the opener ONE template literal; `boundedBeginShape.test.ts` pins its exact SQL and shape, **source only — it cannot see the build**. Two review rounds: the first guard passed on the incident's own bytes; the second made its parser test able to fail. **#801 stays open: mechanism unestablished, sweep not done.**
...
- Scope: the route and its suites, plus comment-only corrections in three insights modules (SCOPE widened at review). Callers not passing the parameter are unaffected. Both generators' non-bypassable duplicate checks KEPT — not redundant.
- Outcome: **DECIDED — 401 when the parameter is present and the caller is not a platform admin**, from `requireAdminAuth(req)`, after the existing 404 blend-in so a passworded or unknown league stays indistinguishable.
- Evidence: **the issue's disclosure claim does not reproduce.** Both `shouldSuppressGenerator` entries carry a copy inside their own generator, so the two responses differ in `generatedAt` alone — measured, positive controls firing. #627's body says a visitor can "receive withheld editorial output"; false against this code. Its disclosure value is prospective: a future engine-only rule would have been liftable.
- Review / verification: RV6. Delta **+13, measured at both ends** (5,258 → 5,271). Three tests red against the parent; only the generator-invocation counter catches moving the guard after the build.
- Status: MERGED `d7a97917` (PR #772), 2026-09-13; #627 closed.
- Open work: **#627 makes the flag privileged and closes the BYPASS-SPECIFIC rebuild; it does NOT close the anonymous unbounded rebuild the issue was filed for.** `parseYear` has no upper bound, so `?year=<any>` mints unbounded distinct cache keys from an anonymous caller — [#770](https://github.com/znpruitt/cfb-app/issues/770). Also [#768](https://github.com/znpruitt/cfb-app/issues/768), [#769](https://github.com/znpruitt/cfb-app/issues/769), [#771](https://github.com/znpruitt/cfb-app/issues/771).
- Owed to planning at merge (planning owns `AGENTS.md`): in Season Launch invariant 4, the clause **"`bypassSuppression` is NOT admin-enforced today"**, the PLATFORM-101 instruction **"do not write new guidance that assumes the flag is privileged"**, and the INSIGHTS-025 exception sentence beginning **"`bypassSuppression` is a public query parameter on any passwordless league"** all go false. Its `unstable_cache` sentence stays true.

### PLATFORM-770-INSIGHTS-YEAR-BOUND-CLAUDE-v1

- Purpose: `?year=` on the Insights API had a floor (`n >= 2000`) and no ceiling, and the value became cache identity verbatim — the anonymous unbounded rebuild [#627](https://github.com/znpruitt/cfb-app/issues/627) was filed for and explicitly did not close ([#770](https://github.com/znpruitt/cfb-app/issues/770)).
- Scope: `api/insights/[slug]/route.ts` and a comment-only change at `loadInsights.ts`. Draft routes untouched; `bypassSuppression` untouched.
- Outcome: **DECIDED — a caller-supplied year is accepted iff `MIN_SEASON_YEAR <= n <= maxCreatableSeasonYear(now)` (`currentYear + 1`) OR `n === resolveLeagueOperatingYear(league)`, else 400 `{ error, field: 'year' }`** matching `/api/rankings`. The range is derived from the registry's own creation cap and reuses `maxCreatableSeasonYear` rather than recopying it. **The disjunct is load-bearing**: a range alone makes refusing a league its own operating year unlikely, the disjunct makes it structurally impossible. Bounded at the route, which is the only place that can tell a caller-supplied year from a server-derived one.
- Evidence: **the prompt's SECOND CONSUMER section did not reproduce, and it was the section added
  to correct an earlier miss.** `loadRecapContextForSeasonScope` gates on
  `isWeeklyRecapActiveSeason` — `status.year === seasonYear`, an EQUALITY check — so an absurd year
  returns `null` before `loadRecapContext` runs and the `year < seasonYear` archive filter at
  `loadRecapContext.ts:44` is never reached. Measured with the control that makes it non-vacuous:
  legit year `absent` (entered the loader), both absurd years `inactive` (stopped at the gate). Even
  if reached, `tsc`'s archives are 2018 and 2021–2025, all already below 2026, so the amplification
  factor would be **1×**. **The REAL second consumer is canonical standings** —
  `resolveStandingsYear` lets an explicit override win unconditionally, so each distinct year minted
  TWO cache identities, and the standings entry carries `revalidate: false` (tag-only, no time
  expiry; its year tag can never fire, it survives on `standingsSlugTag` alone). Rebuild cost
  measured at **112.1 ms** against a **203.2 ms** populated 900-game build — not the ~157 ms the
  prompt carried, because the year-scoped reads all miss while the league-wide fixed cost does not.
  **Reachability, measured on the production replica: all three leagues (`tsc`, `test`, `pruitt`)
  carry a password**, so `leagueAuth` condition 1 is satisfied by none and the live vector today is
  any password-holding member, not an anonymous caller.
- Review / verification: **both reviewers gathered against `32b226e1` before any remediation.**
  Codex: no findings. `/code-review high`: two low findings, adjudicated separately rather than
  applied wholesale. **(1) ACCEPTED and fixed** — emptiness was decided on the trimmed value and
  validity on the untrimmed one, so `?year=%20` was absent (200) while `?year=%202026` was a 400
  that `Number.parseInt` had served before this slice; now trimmed once, both decisions read the
  same value, pinned by a test mutation-proven to be the only one that goes red when the trim is
  removed. **(2) ACCEPTED AS COMMENT ERROR, NOT BEHAVIOUR** — see Open work. Delta **+10, measured
  at both ends** (5,271 → 5,281). **5 of 10 red against `main`, every one on the defect's own
  assertion** (`expected 400, actual 200`); the 4 green are the controls and no-regression pins — a
  legit year on the same fixture still builds, an omitted year still resolves the operating season,
  next season still serves, and #627's refusal still precedes the bound. The generator-invocation
  counter is what makes "no rebuild" an observation rather than an inference from a status code.
- Status: MERGED `31fc892a` (PR #775), 2026-09-13; #770 closed. **Flipped twice** — the first flip (`3c375b89`) was silently reverted when PR #777 merged a branch carrying the older line. Clean `ort` merge, no conflict.
- Open work: **REVIEW FINDING 2, adjudicated as pre-existing and left in place.** A legacy record
  whose operating year is BELOW `MIN_SEASON_YEAR` — `status: { state: 'season', year: 1999 }` with a
  later `league.year` — is accepted by the route (the disjunct admits a league its own year) and
  then discarded by `loadInsights.ts`'s floor in favour of `league.year`, while `loadWeeklyRecap`
  still scopes to 1999: one response, two seasons. **MEASURED as reachable with NO `?year=` at all**
  — `resolveLeagueOperatingYear` returns the same sub-floor value — so the bound neither causes nor
  worsens it, and the reviewer said so. Not fixed inside this scope: raising the floor would refuse
  such a league its own feed on BOTH callers, which is the failure the disjunct exists to prevent,
  and narrowing the disjunct would contradict the owner's explicit ruling. What WAS wrong and is
  fixed: the new `loadInsights.ts` comment claimed the route is the authority for caller-supplied
  years without naming this exception. Not reachable in production — no league carries a sub-2000
  status year.
- Open work: **echo-versus-derive CLOSED** — bounding caps per-league key cardinality at 28 values
  (2000..`currentYear+1`), ≤168 entries across three leagues and two caches, self-retiring; a derive
  layer would buy a constant factor and cost a seam. **`/api/history/[slug]/[year]` is the same
  defect and is [#774](https://github.com/znpruitt/cfb-app/issues/774)** — same
  `isAuthorizedForLeague` gate, `year < 2000` floor with no ceiling, and `getSeasonArchive` caches a
  genuine `null` under `revalidate: false`. Three corrections to the prompt's own class survey, none
  swept in: a **ninth** parser hides under `parseSeasonParam`
  (`api/debug/insights/[slug]/suppression/route.ts:78`, admin-gated) where a `parseYear` grep cannot
  see it; `api/debug/schedule-eligibility` is the eighth copy and has **no floor either**,
  contradicting "every one `n >= 2000`"; and **`draft/[slug]/[year]` GET is NOT admin-gated** — it
  uses `isAuthorizedForLeague`, the same gate as Insights, so "the other seven are draft routes,
  admin-gated" was generalized from the POST. The draft GET mints no cache entry (a durable-store
  read returning 404), which is why it is reported rather than fixed here.
- Owed to planning at merge (planning owns `AGENTS.md`): in Season Launch invariant 4, the clause recording #770 as the open gap — **"What #627 did NOT close: an anonymous `?year=` is unbounded (`n >= 2000`, no ceiling) and is echoed into the cache key, so it remains a guaranteed miss and a full rebuild per distinct value, with the guard never consulted — #770"** — goes false on merge. Suggested replacement: **"`?year=` is bounded at the route to `[MIN_SEASON_YEAR, currentYear + 1]` or the league's own operating year, and a rejected value returns 400 rather than falling back — #770. The same shape is still open on `/api/history/[slug]/[year]` (#774)."**

### PLATFORM-774-HISTORY-YEAR-BOUND-CLAUDE-v1

- Purpose: `GET /api/history/[slug]/[year]` and the RSC page `/league/[slug]/history/[year]` each parsed the year with `Number()` behind a floor and no ceiling, so every distinct value minted a `revalidate: false` archive cache entry ([#774](https://github.com/znpruitt/cfb-app/issues/774)). Governing precedent: [#770](https://github.com/znpruitt/cfb-app/issues/770), merged `31fc892a`.
- Scope: the route, the page, and one new exported resolver in `src/lib/seasonArchive.ts`. `getSeasonArchive` itself untouched; no change to the archive WRITE path, to what a legitimate archive returns, or to the other year parsers.
- Outcome: **DECIDED — a caller-supplied year is accepted iff it is an integer and `MIN_SEASON_YEAR
  <= n <= resolveLeagueOperatingYear(league)`, OR `n` appears in `listSeasonArchives(slug)`; else
  400 `{ error, field: 'year' }` at the route and `notFound()` on the page.** **NOT #770's
  `currentYear + 1` ceiling** — an archive is the record of a season that FINISHED, so that ceiling
  admits at least one year no league can ever hold. **NOT the archive list alone either**, and the
  gap-year case is what rules it out: `tsc` genuinely has holes at 2019 and 2020, and the page has a
  designed "No archived data found for the {year} season" empty state for exactly that — bounding on
  the list would convert a designed surface into a `notFound()` and answer "no archive" with "bad
  year". The disjunct makes refusing a held archive structurally impossible rather than merely
  unlikely, and costs no new cache identity: `listSeasonArchives` is keyed `['season-archive-years',
  slug]` — slug alone, confirmed by reading the constructor — is `React.cache`-wrapped, is already
  read by seven league pages, and runs only after the range has rejected. **Integers are the larger
  half**: a range alone leaves the accepted set DENSE, so no enumeration bounds it.
- Evidence: **THIS IS THE UNCLOSED HALF OF A HAZARD THE CODEBASE HAD ALREADY DIAGNOSED, not a new
  finding, and that is the most useful sentence this branch leaves.** `rolloverTargeting.ts:83`
  names `2026.5` by name and says an unusable year would let `saveSeasonArchive` "mint a permanent,
  TTL-less archive" under such a key; PLATFORM-086F2H1R4 hardened the WRITER against exactly this
  value. The READ path never got the same guard, so `getSeasonArchive` would mint the entry the
  writer had been forbidden to mint — the precedent for the fix was already in the repo.
  **Reproduced live** (`NEXT_PRIVATE_DEBUG_CACHE=1`, seeded passwordless league): 11 API requests
  produced **8 distinct entries on disk**, including `archive:<slug>:2026.5`, `2026.6`,
  `2026.0000001`, `20000000000` and `2016` (from `0x7E0`); the page independently minted `2029.25`
  and `2029.75`. **Post-fix the same probes produce 4** — `2018`, the in-range gap year `2019`,
  `2026`, and exactly one slug-keyed year-list entry — with zero dense entries and the page 404ing.
  **The prompt's "its only tag fires once a season" is wrong and was corrected**: every entry
  carries TWO tags. The per-year tag is DEAD for a junk year (no writer can emit
  `archive:<slug>:2026.5`); the live one is the slug tag, which is what sweeps them, and it fires
  once a league per season. `invalidateSeasonArchive`'s caller set was re-derived by rename
  mutation, not counted: renaming it yields exactly one `tsc` error (inside `saveSeasonArchive`),
  and renaming `saveSeasonArchive` yields four importers of which **one is production** —
  `api/cron/season-rollover/route.ts`, inside its `eligible` branch. **Production values, read-only
  replica, 2026-09-13:** `tsc` archives 2018/2021–2025 with operating year 2026; `test` and `pruitt`
  hold none, operating 2025 and 2026. **Severity: all three leagues carry a `passwordHash`**, so the
  live vector is a password-holding member, not an anonymous caller.
- Review / verification: `npm run lint:all` **exit 0**, `npx tsc --noEmit` **exit 0**, `npm test`
  **exit 0** — each its own command, each its own real exit code. Delta **+33, measured at both
  ends** (5,281 → 5,314). **The first baseline run exited 1 with `fail 0, cancelled 1`** —
  `OverviewScoreboardGrid.browser.test.tsx` hit its 30 s file timeout under full-suite load; it
  passes alone in 2.1 s and the rerun was clean, so it is a resource flake and the known-failure set
  stays EMPTY. Mutations, each reddening only the tests that own its half: restoring the pre-#774
  parser at both callers turns **4 of 10 route tests** and **4 of 8 page tests** red, the sharpest
  on `expected 400, actual 200` against a **planted archive at store key `2026.5`** — a fixture only
  the defect could ever reach; dropping the integer test reddens 4 resolver tests, dropping the
  disjunct 2, widening the ceiling by one year 3. All 33 pass under `test:clock-shift -- 800`, which
  is measured rather than asserted: the bound reads no clock, because its ceiling is league-relative
  rather than `currentYear + 1`.
...
- Open work: **RESIDUE, two debug routes still reach this cache unbounded — review finding, and its
  shape needed correcting.** `api/debug/archive-integrity/route.ts:329` and
  `api/debug/archive-audit/route.ts:455` both parse the year with a `< 2000` floor and no ceiling
  and call `getSeasonArchive`, and neither validates `leagueSlug` against the registry. **They are
  #770's shape, NOT #774's** — measured: `Number.parseInt('2026.5', 10)` is 2026, `'2e10'` is 2,
  `'0x7E0'` is 0, so the accepted set is unbounded but COUNTABLE and the dense half does not apply.
  **The unvalidated slug is the worse and genuinely novel dimension**:
  `?leagueSlug=anything&year=999999999` mints an entry tagged `archive:anything`, and since no
  rollover will ever run for a league that does not exist, that tag can never fire — permanent in
  the strict sense, where #774's junk entries were swept once a season. Both are
  `requireAdminAuth`-gated, and the prompt's scope excludes the other year parsers, so reported
  rather than fixed. Named here because the bullets above enumerate `draft/[slug]/[year]`,
  `schedule-eligibility` and `parseSeasonParam` while omitting the only two that mint an ARCHIVE
  entry.
...
### PLATFORM-778-ARCHIVE-SLUG-VALIDATION-CLAUDE-v1

- Change: two admin debug routes read archives under an unchecked query-string `leagueSlug`, minting entries tagged `archive:<slug>` that no rollover can fire. **The refusal now lives in the AUTHORITY, covering both readers, returning `null`/`[]`** — decided by the sibling, not the call-site ratio: `listSeasonArchives` mints its own cache identity under the same tag, so guarding `getSeasonArchive` alone leaves it open for the next caller. A **malformed** registry throws; **missing** is absence. Routes keep `404 league-not-found`.
- Scope limit: widened at the receipt gate to `listSeasonArchives` and `listAppStateKeys`' seam. No member-facing surface shares the hole — 8 route-param sites probed against a non-degenerate registry, zero mints, positive control. `admin/[slug]/preseason/owners` is safe **by reading, not observation** (probe got a 307); `recap/loadRecapContext.ts` calls `getLeague` nowhere, safe by three unrelated accidents.
- Evidence: reproduced live (`NEXT_PRIVATE_DEBUG_CACHE=1`), absent after, real-slug control still minting. **See L4** — the guard made four store-failure tests vacuous: readers mutated to swallow every failure passed 24/24, and removing the guard alone reddened exactly those four. **See L1** — "the registry read is free" held only for RSC renders; `React.cache` does not dedupe in Route Handlers (in-process RSC control: page 1, route 3, `/api/insights` at 7 archives 3 -> **14**). Production build unmeasured — `next start` forces `NODE_ENV=production`, refusing the file store.
- Historical corrections: `revalidate: false` is a one-year TTL, not "no time expiry"; the call-site split is 8/3/4, not 11/4.
...
- Status: MERGED `99a41fbe` (PR #783), 2026-09-14; #778 closed. **PROMOTED 2026-09-14** in `c6b4137` — live.
...
- **Retry decision, TAKEN not inherited: body failures get NO retry branch.** Once the kind is honest `isRetryableError` already decides correctly — `timeout`/`network` retry, `parse` does not, because re-requesting an unparseable payload spends quota to fail identically. Cost: `maxAttempts` is unchanged, so no ceiling moves; a body failure consumes the EXISTING budget instead of being single-shot, worst case `maxAttempts - 1` extra billed calls — +2 at the seven 3-attempt sites, **+0 on every automatic and metered path**, all of which are `maxAttempts: 1` by construction. A future body-failure retry branch means the classification above it is wrong.
- **THE ODDS HALF IS NOT IN THIS BRANCH.** `oddsRefreshExecutor` is byte-identical to `main`, its
  suite is removed, and the 15th call site's body read is still unbounded — tracked as
  [#759](https://github.com/znpruitt/cfb-app/issues/759). It came out under a stopping rule agreed
  IN ADVANCE (owner, before the final round): one last review, and anything further in the odds seam
  meant the odds half shipped separately while the clean parts merged regardless. The final round
  found FOUR odds-seam findings across both reviewers, including an unguarded durable write on a
  path that was write-free on `main` (a failed `setLatestKnownOddsUsage` would have turned a
  truthful `provider-fetch-failed` into `unexpected-error`, against this prompt's own CARRIES
  best-effort invariant) and usage headers persisting across attempts so an early attempt's balance
  could be recorded as authoritative after later billed attempts. The rule fired and was honoured
  rather than argued around — the escape hatch pre-committed is what PLATFORM-098 lacked.
- **The model is the branch's most valuable output and it survives in [#759](https://github.com/znpruitt/cfb-app/issues/759), not here.** Three attempts at bounding the odds body all failed for one reason worth keeping: a deadline armed by the CALLER cannot see the retry loop's attempt boundaries, so it either spans attempts it does not belong to — measured, `remainingMs=-34`, a complete 200 discarded as `provider-fetch-failed` — or grants a fresh budget per phase. Rounds 1 and 2 prescribed OPPOSITE placements for the same durable write, which is the PLATFORM-092 signal that the placement was never the variable; `captureOddsUsageSnapshot` takes `Headers`, not a `Response`, so usage has no ordering dependency on the body at all. The fix belongs inside the loop.
- **`fetchUpstreamResponse`'s docblock now describes the real state**, including that its sole caller DOES read a body with no deadline, naming #759. An earlier revision asserted there were no such callers; that stopped being true the moment the odds attempt was reverted, and an aspirational claim beside a live violation is PLATFORM-086F2J's "asserting narrowness is not enforcing it" in different clothes.
- Consumers changed by the changed `kind`: **none BRANCH on it — one does, and it is inside the edited module.** `isRetryableError` (`fetchUpstream.ts`) is the only behavioural branch in the repo. Six sites read the kind into recorded evidence: `refreshAuthority` and `fullSeasonScheduleFetch` via `classifyUpstreamFault` (durable receipt label), `scores/route.ts:86` template-building `` `cfbd-${kind}` `` into `CfbdFallbackReason` — a DURABLE value that changes from `cfbd-parse` to `cfbd-timeout`/`cfbd-network` — plus two `logDebug` details. Client-facing HTTP status is unchanged everywhere: every site derives `details.status ?? 502` and neither kind carries a status.
- Scope: EXTENDED by owner ruling to close [#632](https://github.com/znpruitt/cfb-app/issues/632).
  The prompt's SCOPE excluded "the timeout VALUES at the ten call sites" on the premise the items
  were separable; this fix is what couples them. Under the old semantics a site's total ceiling was
  effectively unbounded (headers bounded, body free), so preserving 12s would have silently moved it
  to 12s against PLATFORM-115's measured 8.2 s / 16.0 s / 21.5 s completions. Ten CFBD sites moved
  to `CFBD_PEAK_LATENCY_TIMEOUT_MS` — one existing constant, no new value. Spend does not rise
  (`maxAttempts` untouched; per #632's measurement an aborted call bills as much as a completed one,
  so a slow body succeeding in one attempt costs 1 where 12s would have aborted and retried for 3).
  Worst case becomes ~120.9 s at the five 3-attempt sites, replacing an UNBOUNDED worst case; no
  `maxDuration` envelope was added, which would have been the new number the ruling excluded —
  recorded in [#757](https://github.com/znpruitt/cfb-app/issues/757).
- **Both "ten call site" sets are ten and they are NOT the same ten**, enumerated rather than count-matched: `scripts/recover-game-stats.ts:862` is in this set and not #632's (whose stated limit was "counted by value across `src/`"); `oddsRefreshExecutor` is in #632's and not this one (different provider — "do not sweep it in on pattern-match alone"), and with the odds half reverted it keeps `main`'s literal, with [#756](https://github.com/znpruitt/cfb-app/issues/756) owning its measured band.
- Historical corrections: the prompt's "23 call sites across 12 files" is **14 in `src/` across 12 files, 15 with the script**; its base SHA `54fa6d7f` was not a stale `main` but the CODEX lane's head (`origin/main` was `87212f61`); and its stated central risk — "a changed `kind` is a behaviour change at every consumer that branches on it" — is inverted, since zero consumers branch. My own receipt headline ("zero consumers branch on it") then understated its own table, which had already recorded `isRetryableError`.
- Review / verification: THREE rounds, both reviewers each time, 17 findings. Applied: the round-1
  deadline/guard/timer findings; the round-2 HIGH via reconstruction rather than patching.
  Escalated: the `timeoutMs` semantic change, which became the #632 ruling. Declined with reasons: a
  pre-existing undrained-body finding; a wall-time finding whose "serialized by pacing" premise is
  wrong (`applyPacing` chains waits, not requests) — residue to #757. Reverted: everything in the
  odds seam. **Two of my own tests were caught VACUOUS by mutation before shipping** — a timer
  assertion filtering `process._getActiveHandles()`, which reports `[]` for an armed timer on Node
  22, and a usage assertion that held whether or not the request recorded anything. Nine mutations,
  each read for WHICH assertion fired; the surviving suite's multi-attempt test fires under M7 while
  all eight single-attempt tests stay green, which is the property the first two rounds lacked. `npm
  test` 5,238/5,238 exit 0 (+9 from 5,229); `tsc`/`lint:all` exit 0; clock-shift +400 d green. See
  L3, L4, L6.
- Status: Merged (PR [#762](https://github.com/znpruitt/cfb-app/pull/762), `57fdbd82`, 2026-09-13) from `claude/662-upstream-body-deadline`. [#632](https://github.com/znpruitt/cfb-app/issues/632) closed with it. **#662 is NOT fully discharged:** the 15th call site — the odds provider body read — is still unbounded and is [#759](https://github.com/znpruitt/cfb-app/issues/759), which carries the model three rounds produced.
...
- Scope limit: `gameWeek.ts`, `GameWeekPanel.tsx`; the component joined a selector-only scope at review so awaiting cannot leak partial scores Matchups suppresses. NOT #740's `isPlannedGame` fail-open, NOT #728's `contextSlot`.
- Evidence: 6,410 built FBS games over seven cached seasons held no persisted awaiting-with-score row, and none pairing absent kickoff confidence with missing score evidence, so the score-nulling and display-confidence remediations are **prospective guards, not production repairs**. Three evidence-beats-gate cases and both final review additions mutation-proven, unaffected controls green; the synthetic disruption test disclaims production coverage. See L1, L4.
- Review / verification: converged, no credible in-scope P0/P1/P2. Focused 74/74 (+13); `tsc`/`lint:all` 0; `npm test` 5,170 of 5,172 — the standing Item 137 `odds/writer-convergence` pair.
- Status: Merged (PR [#741](https://github.com/znpruitt/cfb-app/pull/741), `ff0c5e38`, 2026-09-11). Stopped PR [#736](https://github.com/znpruitt/cfb-app/pull/736) closed as superseded, the v1 review history.
