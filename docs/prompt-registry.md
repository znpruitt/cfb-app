...
- Verification: `git diff --check` clean; `npm run lint:all` clean; `npx tsc --noEmit` clean. New `src/lib/__tests__/seasonArchive.test.ts` (10 tests — key/tag isolation across slug+year, read/write round-trip shape, cross-league/cross-year isolation, sorted year list) passes; standings + insights consumer tests (`selectors-leagueStandings`, `loadInsights`, `insights-context-aliases`) and rollover/history tests (`seasonRollover-aliases`, `selectors-historySelectors`, `leagueRecords`, `historyOverview`) all green.
