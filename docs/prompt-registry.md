...
- Status: Implemented on `claude/861-elapsed-capture` (PR #865); pre-merge closeout. No deployment claimed. §8i's gate is discharged but **re-tied to PROMOTION rather than merge** — stricter than the owner's proposed wording, flagged as a deviation, because `upsert --apply` hits production and auto-promotion is off. `docs/next-tasks.md:290` still says §8i is gated on this issue; planning's file, a post-merge flip.
