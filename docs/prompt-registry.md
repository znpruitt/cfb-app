...
- Remediation additions (review cycles): centralized the hydration issue constant with its classifier (`ODDS_HYDRATION_ISSUE` co-located with `isLiveOddsIssue` in `cfbScheduleAppHelpers.ts`, matched by EXACT VALUE not a prefix); freshness-aware `mergeFresherOddsUsage` (`apiUsage.ts`) applied by BOTH `useOddsHydration` and `useAdminOddsUsage`, so the two concurrent writers of shared `oddsUsage` converge on the newest `capturedAt` (a null/older reading never clobbers a newer one); one shared response decoder `applyOddsResponse` (`src/lib/oddsClientPayload.ts`) used by the hydration hook AND the manual seam; and a `scheduleGeneration` monotonic counter (`CFBScheduleApp`, bumped on every `games` rebuild) threaded into the hydration deps so a with-games in-place schedule reload (which does not toggle `scheduleLoaded`) still re-hydrates against the new canonical keys.
- Review: Codex round 1 clean; then the owner ran Claude `/code-review` (xhigh) → 4 low findings (issue-string prefix fragility, two-writer `oddsUsage` race, duplicated response decode, decouple test-coverage gap) — ALL FIXED; then a cycle-2 confirming Codex round found one credible P2 (a with-games in-place schedule reload did not re-hydrate odds — pre-existing, but the C3 docs overstated reload behavior) — FIXED via `scheduleGeneration`. The stale PLATFORM-086C2 activation documentation was corrected in the same effort (C2 is ACTIVE in production; §8g executed).
- Verification: +22 focused tests total — the 10 original (9 `useOddsHydration` cases + 1 `GameScoreboard` far-future render) plus 12 remediation tests (freshness-merge + shared-applier units, exact-value issue classification, admin-usage freshness, decouple regression ×2, and schedule-rebuild re-hydration); full `npm test` 2454/2454; `npx tsc --noEmit`, `npm run lint:all`, `npm run lint:markdown`, `npm run build`, `git diff --check` all clean. All fetches stubbed; no provider or production request.
- Status: **implemented + review-converged (Codex r1 clean → Claude `/code-review` 4 findings fixed → Codex cycle-2 1 P2 fixed → confirming round), full gate green; opened as PR #421 to `main`, awaiting owner merge.** A bounded client-display follow-up to PLATFORM-086C2's production rollout — it does NOT reopen or alter C2 activation (the runbook §8g operator sequence, now executed, is a separate step). NEXT after merge: PLATFORM-086E1 (weekly schedule refresh).
