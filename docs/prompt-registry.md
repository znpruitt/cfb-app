...
- Status: Implemented on `claude/875-real-time-stall`; pre-merge closeout. No deployment claimed — test-only, and `preview` was deliberately not taken. Residuals: the boundary test's 714–904 ms, **56 unmeasured** `Date`-only enables in `rankings/__tests__/`, and two test constants restating unexported production values.
