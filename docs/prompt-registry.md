...
### PLATFORM-086H3B-REVISION-STATUS-AUTHORITY-v1

- Status: **Implemented — review pending — NOT merged — production behavior unchanged (dormant).** Branch `platform/086h3b-revision-status-authority` from `main` @ `2793a6f`; branch pushed; no PR opened. Second prerequisite (B) of the PLATFORM-086H3 decomposition. Prerequisite A merged via **PR #398** (`platform/086h3a-app-state-multi-key-transaction`, merge commit `2793a6f`); B builds on that merged multi-key transaction primitive and was reconstructed from `main` — the single-branch monolith (`platform/086h3-atomic-game-stats-contract-activation`, HEAD `e1a9593`) remains a frozen read-only salvage reference and was NOT cherry-picked. **H3 production activation has NOT occurred**; C–E remain pending; H4 (diagnostics + panel wording) and the legacy-row migration remain deferred.
- Purpose: Implement the frozen contract §5/§7/§14/§17 revision layer — lineage-aware revision authority, transactional refresh-status chronology, admin-only operator repair, and the durable activation-control fence — all production-DORMANT: no production route/cron arms/activates the fence, calls the revisioned writer, or allocates a revision; the legacy writer stays behavior-equivalent while the fence is `legacy`.
- Result:
  - New `src/lib/gameStats/revisionStamp.ts` — the NON-dormant lineage-aware commit-stamp primitive `{ lineage, revision }`: opaque lineage, positive-safe-integer revision, same-lineage-only comparison (different lineages never numerically ordered), never emitted to the wire. Kept scanned by the dormant-boundary guard (pure, imports nothing dormant).
  - New `src/lib/gameStats/revisionAuthority.ts` (dormant) — the revision ledger (`game-stats-revision`, schema-versioned, exact partition identity, active lineage, last committed revision, initialization mode/timestamp, optional repair audit ref) and the frozen §5 state machine: genuinely-new / recognized-legacy mint lineage 1; healthy initialized allocates `ledger.revision + 1` (E(P) only — status is never the ordinary authority); recoverable same-lineage partial history reconstructs above the highest surviving evidence (one-time bootstrap consults the status stamp under `E(P) → S(P)`); lineage conflict, ambiguous history, and suspected evidence loss BLOCK with stable typed codes (`revision-lineage-conflict` / `revision-history-ambiguous` / `revision-evidence-loss-suspected`), writing no evidence and preserving all durable state. A missing revision field ALONE never proves a scope new. An operator-repaired ledger (`initializedFrom: 'repair'`) continues past an absent partition.
  - `src/lib/gameStats/durableMerge.ts` (dormant) — new `mergeGameStatsPartitionRevisioned` co-commits the stamped evidence partition and its ledger row in ONE transaction (rollback discards both), attaches the commit stamp ONLY after confirmed COMMIT, checks the activation fence (refuses unless `armed`/`active`), and surfaces revision blocks as typed `unavailable` reasons. The legacy H2 `mergeGameStatsPartitionDurable` is untouched.
  - New `src/lib/gameStats/activationControl.ts` (dormant) — the durable fence (`game-stats-activation-control/global`): states `legacy | armed | active | read-only-safe`; absent → `legacy`, malformed → `read-only-safe`; `active` only from `armed`; return to `legacy` permanently forbidden once evidence has existed; the fenced legacy setter (`writeLegacyGameStatsPartition`) acquires the activation lock and writes only in `legacy` (behavior-equivalent to `setCachedGameStats`). No B path transitions to `armed`/`active`.
  - `src/lib/server/providerRefreshStatus.ts` — new dormant game-stats chronology (generic helpers UNCHANGED, so unrelated datasets never regress): durable per-scope attempt ordinal + token; transactional read-modify-write begin/success/no-op/failure returning typed persistence results; committed-evidence advances ONLY via a valid same-lineage commit stamp (lower skipped, equal idempotent only when metadata agrees, divergent equal a typed conflict, foreign lineage rejected); attempt vs committed-evidence chronology kept separate; composite `{ begin, terminal, complete }` never claims status recorded when persistence failed.
  - New `src/lib/gameStats/revisionRepair.ts` + `src/app/api/admin/game-stats-revision/route.ts` — the frozen §14 admin-only inspect/repair surface: safe structured inspection + an expected-current-state digest; repair requires platform-admin auth, defaults to dry-run, requires exact partition + digest CAS, acquires `E(P) → S(P) → C(P)`, refuses on state change / unexpired recovery claim / floor below surviving same-lineage evidence / malformed evidence / missing acknowledgement of lineage conflict or evidence loss; three bounded actions (rebuild ledger from surviving same-lineage evidence; adopt an attested restored lineage+floor; establish a new lineage after acknowledged evidence loss, preserving the historical lineage in the audit trail); never alters game-stat rows or fabricates provider evidence; appends an audit record (actor, time, reason, before digest, action, safe after state).
  - Lock graph: `game-stats` (E) < `game-stats-activation-control` (fence) < `game-stats-recovery` < `game-stats-revision` (ledger; co-serialized under E, never separately locked) < `provider-refresh-status` (S) < `recovery-disposition` (C). Ordinary allocation E(P) only; bootstrap `E(P) → S(P)`; the fenced legacy write `E → activation-control`; repair `E(P) → S(P) → C(P)`; all monotonic under the merged A comparator (no reverse acquisition).
  - Dormancy: the recursive dormant-boundary guard is EXTENDED — production may not import `revisionAuthority`/`activationControl`/`revisionRepair` (added to the forbidden-module set in every import form) nor reference the lifecycle symbols (`mergeGameStatsPartitionRevisioned`, `allocateGameStatsCommitStamp`, `setActivationState`, `writeLegacyGameStatsPartition`, `repairRevisionState`, `inspectRevisionState`) or the game-stats chronology symbols (forbidden everywhere except their `providerRefreshStatus.ts` definition home). The admin repair route + `revisionRepair.ts` are the ONE sanctioned production connection (excluded homes, §15). The legacy writer remains the production writer in `legacy`.
- Verification: focused revision/status/activation/repair/transaction/H2/dormant-boundary suites green (57 new B tests + existing suites); `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` green. No PR opened; `/verify` NOT run; `preview` NOT updated. New runbook `docs/revision-repair-runbook.md` documents inspection, refusal states, repair preconditions, activation states, deployment/rollback, and the direct-database-editing prohibition.
- Folded PLATFORM-086H3B-ACTIVATION-DORMANCY-REMEDIATION-v1 (two confirmed high findings; still no PR, no `/verify`): (1) the LIVE production legacy writer `setCachedGameStats` is now routed through the durable fence — `cache.ts` roots the write at E(P), acquires the activation-control lock (forward `E → activation-control`), re-reads the activation record + the durable global witness + this partition's revision ledger UNDER the lock, and writes ONLY when the fence resolves to `legacy` with no surviving revision history (behavior-equivalent to the prior blind write in `legacy`; throws `GameStatsFenceError` otherwise). `activationControl.ts` became a LIVE fence-primitive module (no `cache` import → no cycle); the guard reclassifies it from a fully-excluded dormant home to a MODULE-scanned forbidden-symbol home (`setActivationState`/`markRevisionedEvidenceCommitted`/`schemaVersion` defined there; `writeLegacyGameStatsPartition` is no longer a forbidden symbol — it is the live writer). (2) The revisioned writer now validates activation INSIDE the commit transaction (a pre-transaction read is an optimization only) and requires EXACTLY `active` — `armed` prepares deployment but no longer authorizes evidence commits; `read-only-safe` fences both writers. A durable global write-once witness (`game-stats-activation-control/revisioned-evidence-witness`) is set ATOMICALLY with the first revisioned commit, forbids the legacy writer and any automatic return to `legacy` even if the activation record is lost, and is consulted when the record is absent (absent → `legacy` only when no witness/ledger/stamp survives; malformed → `read-only-safe`, never auto-normalized). APPLIED repair is now DORMANT through the live admin route: `apply: true` is refused with `revision-repair-application-not-active` (writes nothing — no stamp, ledger, status, or audit); inspection + dry-run planning remain. The public `/api/game-stats` route emits an explicit allowlist envelope that strips the internal `commitStamp` (byte-compatible with legacy data). Deferred to later bounded passes (documented in the prompt): valid-ledger restoration vs newer status history, malformed-evidence/CAS-digest completeness, success-without-stamp, revision/attempt exhaustion, repair error/audit truthfulness, and full contract/runbook reconciliation. Verification: focused activation/legacy-writer/revisioned-writer/repair-route/transaction/H2/dormant-boundary suites green; `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` 1882/1882. No PR; `/verify` NOT run; `preview` NOT updated.
- Folded PLATFORM-086H3B-RESTORATION-STATUS-REMEDIATION-v1 (three confirmed findings; still no PR, no `/verify`): (1) **Restored-behind ledger vs newer status.** Every revision allocation — ORDINARY and bootstrap — now consults the committed refresh-status stamp under `S(P)` (lock sequence `E(P) → S(P)`, or `E(P) → activation-control → S(P)` under the revisioned writer). Committed status is a mandatory restoration/high-water WITNESS (never the allocator): on the same lineage, status equal/behind/absent allocates from the ledger, status AHEAD of the surviving partition blocks `revision-evidence-loss-suspected`, a foreign lineage blocks `revision-lineage-conflict`, a malformed revision-era status blocks `revision-history-ambiguous`; a legacy/unrelated provider-status record (no `lastCommittedStamp`) never falsely blocks. A restored-behind ledger can therefore never reuse a stamp already represented in status; blocked states write nothing. (2) **Mandatory commit stamp on success.** `recordGameStatsRefreshSuccess` requires a valid CONFIRMED commit stamp at the TS boundary (non-optional) and at runtime; a missing/malformed/unsafe stamp returns the stable typed refusal `game-stats-success-commit-stamp-required`, persisting nothing, not clearing `lastError`, not marking the attempt succeeded, and never `complete: true`. The explicit no-op API remains the only stamp-free successful terminal. (3) **Safe-integer exhaustion.** Revision allocation refuses `revision-counter-exhausted` before any `+1` at `Number.MAX_SAFE_INTEGER` (healthy, reconstruction, repair-continue); the attempt ordinal refuses `refresh-attempt-ordinal-exhausted` at the bound and `refresh-attempt-ordinal-malformed` for a present-but-invalid ordinal (never resets to 1, distinguishing malformed from absent); administrator repair floors that cannot advance (nonpositive / unsafe / `MAX_SAFE_INTEGER`) are refused `revision-repair-floor-not-advanceable` during planning AND transactional apply validation. Applied repair remains dormant through the live route (only the planner's floor validation was hardened). Deferred (documented): repair CAS digest completeness, malformed-evidence certification, raw-error redaction, audit availability, and full contract/runbook reconciliation. Verification: focused revision-authority/restoration/refresh-status/activation/revisioned-writer/repair-planning/transaction/H2/dormant-boundary suites green; `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` 1894/1894. No PR; `/verify` NOT run; `preview` NOT updated.
- Folded PLATFORM-086H3B-REPAIR-SAFETY-DOCS-REMEDIATION-v1 (repair-safety, error-truthfulness, and docs reconciliation; still no PR, no `/verify`): (1) **Malformed evidence is a hard repair refusal.** `planRepair` certifies evidence FIRST for every action — a malformed weekly envelope, invalid/inconsistent partition identity, malformed commit stamp, revision-era rows without a valid stamp, or unsafe revision refuses `revision-repair-evidence-malformed`, which NO acknowledgement (lineage-conflict / evidence-loss) can override; repair still never rewrites/normalizes/fabricates game-stat rows. (2) **Versioned complete-state CAS digest.** The prior count/stamp digest is replaced by a versioned (`gsr-state-v1:`) canonical (recursively key-sorted, array-order + values significant) SHA-256 over the COMPLETE inspected durable state — the full weekly envelope + every game row + metadata + commit stamp + ledger + committed status + activation record + irreversible witness + recovery-disposition/claim (token/owner/expiry) + audit + identity — hashed, never embedded; recomputed transactionally under `E → S → C` and refused as `revision-repair-state-changed` on any change. (3) **Error redaction + typed audit availability.** Raw storage/SQL/path/stack errors are logged server-side only and never returned — inspection failures return `revision-repair-inspection-unavailable`, repair failures `revision-repair-planning-unavailable`; `readRevisionAuditTrail` returns typed `available` / `absent` / `unavailable` (a failed or malformed read is never collapsed to `[]`), and the admin route preserves the distinction (503 on inspection-unavailable). Applied repair stays dormant through the live route. (4) **Contract/runbook reconciliation.** `docs/ai/platform-086h3-contract.md` and `docs/revision-repair-runbook.md` now record the implemented lock graph, the `recovery-disposition` single-ownership scope (D must adopt, not fork), `lastCommittedStamp` (not `lastSuccessRevision`), `active`-only revisioned writes, applied-repair dormancy, malformed-evidence hard refusal, complete-state CAS, and explicit audit availability. Governance: B remains **implemented, review-remediation in progress, NOT merged, NOT `/verify`-confirmed, production lifecycle inactive**. Deferred: nothing further from the original prompt beyond routine full contract reconciliation at merge. Verification: focused repair/digest/malformed/audit/error-contract/admin-route/revision-authority/chronology/activation/transaction/H2/dormant-boundary suites green; `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` green. No PR; `/verify` NOT run; `preview` NOT updated.
- Folded PLATFORM-086H3B-ACTIVATION-STATE-CORRUPTION-REMEDIATION-v1 (one confirmed high finding: present-but-malformed control rows treated as absence, and completed activation states returning to `legacy`; still no PR, no `/verify`): (1) **Presence ≠ value.** Durable control reads now distinguish `row absent` / `row present with valid value` / `row present with invalid-or-null value` via a `ControlRead` (`toControlRead`) built from the raw transaction record BEFORE decoding — the `?.value ?? null` / `validateActivationRecord(stored) ?? defaultActivationRecord(...)` fallbacks that collapsed a present JSON-`null`/malformed row into absence are removed. A PRESENT-but-invalid activation or witness value fails safe (activation → `read-only-safe`; witness present valid OR malformed → history survives), and a PRESENT-but-invalid revision-ledger row (incl. JSON `null`) is a revision-era marker that blocks allocation as `revision-history-ambiguous` (presence, not value, decides — even alongside a valid partition stamp). Malformed control rows are never auto-normalized (byte-identical after refusal). (2) **Forward-only activation graph.** `classifyActivationTransition` now permits ONLY `legacy→armed`, `armed→active`, `armed→read-only-safe`, `active→read-only-safe`, plus a safe idempotent same-state; ALL `→legacy` transitions are removed (`legacy→armed` is irreversible even before evidence; `read-only-safe` is terminal), and an idempotent same-state `legacy` refuses when a surviving witness proves history. `setActivationState` reads presence-aware, refuses a present-malformed record (and an absent record whose witness survives) with the new typed `activation-state-malformed`, and never normalizes. The legacy writer (`cache.ts`) and revisioned writer (`durableMerge.ts`) both pass presence-aware reads to the gates. Deferred (documented, unchanged this pass): repair high-water lowering, repair evidence-certification completeness, repair CAS activation-control serialization, tokenless terminal status / ordinal-zero, global activation-lock cross-partition serialization, malformed audit-entry validation, dormant-boundary sanctioned-route bypass. Verification: focused activation-control/legacy-writer/revisioned-writer/revision-authority/transaction/public-projection/H2/dormant-boundary suites green; `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` 1907/1907. No PR; `/verify` NOT run; `preview` NOT updated.
- Folded PLATFORM-086H3B-REPAIR-HIGH-WATER-REMEDIATION-v1 (one confirmed high finding: repair could propose lowering a valid surviving same-lineage ledger and thereby permit reuse of already-committed revision numbers; still no PR, no `/verify`): the repair high-water now derives from EVERY valid durable witness — `highestSurvivingSameLineage` and a new structured `assessSurvivingHistory` (none / one-lineage / conflicting-lineages / malformed, with an evidence-loss flag) include the VALID revision ledger (previously omitted — the exact bug: partition `L/5` + ledger `L/10`, `adopt-lineage L floor 5` was accepted and rewrote the ledger down to `L/5`). Revisions are compared only within a lineage. `adopt-lineage` refuses a floor below the full same-lineage high-water (`revision-repair-floor-below-surviving-history`); `rebuild-ledger` rebuilds only UP to the high-water and refuses a ledger/status AHEAD of the surviving partition (`revision-repair-ledger-ahead-of-evidence`) or conflicting witness lineages (`revision-repair-lineage-conflict`); `establish-new-lineage` mints a genuinely different lineage (never numerically comparing across lineages) and preserves the abandoned lineage + high-water in the audit. Floor semantics are documented as "last committed revision" (next allocator issues `floor + 1`), so every accepted plan leaves the next allocation strictly above all surviving same-lineage history — proven by a non-reuse integration test (adopt at the high-water, then a revisioned allocation issues `high-water + 1`). Truthful planning output adds a safe `survivingHighWater` (lineage + high-water + witness sources) to results and audit; no raw rows/storage exposed. Applied repair stays dormant through the live route; the deferred medium findings (game-row certification, repair CAS activation-lock serialization, tokenless terminal status / ordinal-zero, activation-lock cross-partition serialization, malformed audit-entry validation, dormant-boundary sanctioned-route bypass) are untouched. Verification: focused repair-planning/revision-authority/revisioned-merge/activation/chronology/transaction/route-dormancy/H2/dormant-boundary suites green; `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` 1914/1914. No PR; `/verify` NOT run; `preview` NOT updated.
- Folded PLATFORM-086H3B-REPAIR-STRUCTURE-CAS-AUDIT-REMEDIATION-v1 (three confirmed medium repair findings; still no PR, no `/verify`): (1) **Full evidence certification.** `certifyEvidence` is replaced by a bounded runtime classifier `classifyRepairEvidence` (absent / recognized-legacy / valid-revision-era / malformed) behaviorally aligned to the revision authority (reuses `classifyPartitionStamp`) that certifies the complete envelope — exact identity, `fetchedAt` under the production contract, `games` array with EVERY row passing a bounded durable game-stat contract (`isSafeRepairGameRow`: game identity + participant identity + well-formed stat containers), and a valid `commitStamp` if present. Malformed timestamps/rows (or a mixed valid/invalid `games` array) are a hard `revision-repair-evidence-malformed` refusal, checked FIRST for every action, that no acknowledgement can override; repair still never rewrites/normalizes/fabricates rows. (2) **CAS serialization.** The repair lock chain is now `E(P) → activation-control → S(P) → C(P)` — the activation-control lock is acquired BEFORE reading the activation record + irreversible witness (both bound by the versioned complete-state digest), so a concurrent activation transition or another partition's first revisioned commit cannot mutate those CAS inputs inside the window; a completed change is caught as `revision-repair-state-changed`. Monotonic under A's comparator (`game-stats` < `game-stats-activation-control` < `provider-refresh-status` < `recovery-disposition`); inspection takes the same locks. (3) **Audit validation.** Audit entries gain `schemaVersion: 1`; a per-entry validator + field allowlist (`validateAuditEntry`) rebuilds a clean entry and rejects missing/invalid fields OR any unapproved extra field (no arbitrary stored content reaches responses; never a raw cast). `readRevisionAuditTrail` returns `available` only when the dataset is a valid array in which EVERY entry validates (order preserved), else `unavailable` (non-array, any malformed/unapproved entry, or read failure) — never silently dropped; `absent` only for a truly missing dataset. A malformed audit history is never trusted or appended to: planning and apply refuse `revision-repair-audit-unavailable`. Applied repair stays dormant through the live route; error redaction preserved. Deferred (unchanged): tokenless terminal status / ordinal-zero, global activation-lock cross-partition serialization, dormant-boundary sanctioned-route bypass. Governance: B remains implemented, review-remediation in progress, NOT merged, NOT `/verify`-confirmed, production lifecycle inactive. Verification: focused repair-evidence/CAS/audit/admin-route/revision-authority/activation/revisioned-writer/transaction/chronology/H2/dormant-boundary suites green; `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` 1921/1921. No PR; `/verify` NOT run; `preview` NOT updated.
- Folded PLATFORM-086H3B-STATUS-ATTEMPT-OWNERSHIP-REMEDIATION-v1 (one confirmed refresh-attempt ownership + ordinal-validity finding; still no PR, no `/verify`): **Tokenless terminal status mutations could overwrite another attempt's chronology, and stored ordinal `0` was treated as valid history.** (1) **Mandatory, runtime-validated attempt handle on every terminal.** `GameStatsRefreshSuccess/Noop/Failure.attempt` is now REQUIRED at the type boundary and re-validated at runtime (`validateGameStatsAttemptHandle`) before any storage read/write: non-empty token, POSITIVE safe-integer ordinal, non-empty `startedAt`, and the exact mutation scope. A missing handle → stable `game-stats-refresh-attempt-required`; a structurally invalid or misrouted (scope-mismatched) handle → `game-stats-refresh-attempt-malformed`. Both persist nothing and never claim `complete`. The `no attempt supplied → treat as latest` behavior is removed — `isLatestGameStatsAttempt` returns `true` only for an owning handle, never for a missing one. (2) **Ordinal AND token jointly establish ownership.** A normal persisted attempt owns the chronology only when BOTH its token AND ordinal match the stored latest (a matching token with a stale/unknown ordinal, or a matching ordinal with a wrong token, is `skipped-older` and writes nothing). Success still requires BOTH a valid confirmed commit stamp AND a valid matching handle; no-op and failure remain stamp-free but still require a valid matching handle. The failed-begin exception is preserved ONLY through the explicit handle whose begin persistence failed (ordinal strictly above the stored ordinal) — never recreated from a missing/tokenless argument and never able to overwrite a later successfully persisted attempt. Committed-evidence chronology stays separate from attempt chronology; a stale attempt cannot change `latestAttemptOutcome`/`lastError`/timestamps or advance `lastCommittedStamp`. (3) **Ordinal `0` is malformed history, not absence.** A valid stored ordinal is a positive safe integer; `0`/negative/fractional/unsafe/non-number stored ordinals are `refresh-attempt-ordinal-malformed` (`classifyStoredOrdinal` — refused, durable record preserved for inspection/repair, never restarted at 1, persists nothing across a simulated restart). Only genuine absence begins at `1`; `Number.MAX_SAFE_INTEGER` stays `refresh-attempt-ordinal-exhausted`; the four states (absent / malformed / exhausted / valid) stay distinct. No production caller exists (dormant; C/D/E wire the terminals), so no caller had to be rewired; generic non-game-stats status writers are untouched. Deferred (unchanged): global activation-lock cross-partition serialization, dormant-boundary sanctioned-route bypass. Governance: B remains implemented, review-remediation in progress, NOT merged, NOT `/verify`-confirmed, production lifecycle inactive. Verification: focused game-stats-chronology/status-publication/revisioned-writer/revision-authority/activation/repair/transaction/H2/dormant-boundary suites green; `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` 1930/1930. No PR; `/verify` NOT run; `preview` NOT updated.
- Folded PLATFORM-086H3B-ACTIVATION-FENCE-CONCURRENCY-REMEDIATION-v1 (the confirmed activation-fence concurrency finding; still no PR, no `/verify`): **Unrelated partition writers serialized on one exclusive GLOBAL activation lock.** Closed via a BOUNDED extension of prerequisite A's transaction lock primitive (A's comparator/poisoning/uncertainty contracts unchanged). (1) **Shared/exclusive secondary locks.** `AppStateKeyTxn` gains `lockKeyShared` (SHARED) alongside `lockKey` (now explicitly EXCLUSIVE); the primary root stays EXCLUSIVE. The strongest held mode per lock identity is tracked, so the same-transaction matrix is `shared→shared`/`exclusive→shared`/`exclusive→exclusive` idempotent, `shared→exclusive` REFUSED (`AppStateTxnLockUpgradeError`, fail-fast, poisons the transaction under A's required-lock contract). Lock MODE never alters tuple ordering, advisory-lock identity, persisted identity, or the deadlock comparator. (2) **PostgreSQL.** Exclusive keeps `pg_advisory_xact_lock`; shared uses `pg_advisory_xact_lock_shared` on the SAME canonical key, same client, transaction-scoped; rollback/cleanup/poisoning/uncertain-COMMIT unchanged. (3) **File fallback.** The exclusive promise chain is replaced by a FAIR in-process reader/writer lock (FIFO with reader coalescing): concurrent shared holders proceed, an exclusive holder is sole owner, a queued exclusive blocks later shared arrivals (no writer starvation), every holder releases on success/refusal/callback/cleanup failure, and fully-idle lock-map entries are GC'd (no leak). (4) **Rewired fence.** Legacy writer (`cache.ts`) and revisioned writer (`durableMerge.ts`) acquire activation-control via `lockKeyShared` (`E(P) exclusive → activation-control shared [→ S(P) exclusive]`), so unrelated partitions commit concurrently; activation transitions (root at activation-control) and repair CAS (`lockKey`) stay EXCLUSIVE and drain/exclude in-flight writers (`E(P) exclusive → activation-control EXCLUSIVE → S(P) → C(P)`). (5) **Deterministic witness.** The revision-history witness value is reduced to the canonical `{ everExisted: true }` and the revisioned writer no longer syncs the shared-locked activation-record flag (a write-under-shared-lock) — its `revisionedEvidenceEverExisted` mirror is a non-authoritative optimization transitions re-sync; so concurrent first commits under the shared fence cannot produce conflicting witness content. Deferred (unchanged): dormant-boundary sanctioned-route bypass (guard NOT modified). Governance: B remains implemented, review-remediation in progress, NOT merged, NOT `/verify`-confirmed, production lifecycle inactive. Verification: focused app-state-transaction (mixed-mode), activation-fence-concurrency, activation, revisioned-writer, repair, chronology, H2, public-projection, and dormant-boundary suites green; `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` 1948/1948. No PR; `/verify` NOT run; `preview` NOT updated.
- Folded PLATFORM-086H3B-DORMANT-BOUNDARY-GUARD-REMEDIATION-v1 (the final dormant-boundary guard finding; still no PR, no `/verify`): **The admin revision route was excluded from dormancy scanning, so forbidden lifecycle capabilities could hide behind aliased imports, barrels, or re-export chains while the guard stayed green.** (1) **Narrow inspection facade.** New `src/lib/gameStats/revisionRepairInspection.ts` re-exports ONLY the safe reads/validation (`inspectRevisionState`, `readRevisionAuditTrail`, `isCfbdSeasonType`) and a locally-defined DRY-RUN-only `planRevisionRepair` that forces `dryRun: true`; it imports the applied-repair function internally but NEVER re-exports it. The admin route is rewired to import its revision capabilities exclusively through this facade (no direct `revisionRepair` import), so `repairRevisionState` is not a route-reachable binding at all; behavior is unchanged (GET inspect/audit, POST dry-run, `apply:true` → `revision-repair-application-not-active`). (2) **Route is scanned, not excluded.** Removed the route from `EXCLUDED_FILES`; the route + facade are SANCTIONED-SURFACE (walked, exempt only from the coarse regex symbol/module scan that cannot tell an approved inspection import from a forbidden one or a type-only import from a runtime one). (3) **Parser-backed capability allowlist.** A new TypeScript-compiler-API resolver (`dormantBoundaryParser.ts`) enforces an exact route import allowlist (module specifier + symbol names) and verifies the facade's export surface, RESOLVING re-export/alias/`export *`/multi-hop/mixed-barrel chains to each binding's terminal definition. Fail-closed: an unresolved local import, a namespace/default/`require`/dynamic-`import` form, an unapproved local module, or a terminal that is not on the allowlist is a violation; type-only imports are treated separately (no runtime capability). The route cannot acquire applied repair, revisioned writes, activation transitions, status/chronology publication, recovery, or generic app-state mutation by any form — including a forbidden capability renamed through a barrel (verified terminal, not the name). `activationControl.ts` stays a legitimate live module; only its transition/writer capabilities are barred from the route. (4) **Coverage preserved.** The existing global regex module/symbol/chronology scan for every other production file is unchanged. Fixtures cover every prohibited path (direct/aliased/namespace/default/dynamic/`require`, one-hop `export *`, named re-export, multi-hop barrel, mixed safe/forbidden barrel, renamed-through-barrel, applied-repair-not-importable, chronology-mutation, activation-transition, facade-export-expansion, the concrete Codex barrel regression) plus approved paths (real route + facade, approved named imports, type-only, external packages). Governance: B remains implemented, review-remediation in progress, NOT merged, NOT `/verify`-confirmed, production lifecycle inactive. Verification: focused dormant-boundary, admin-route, revision-repair, activation, chronology, transaction, H2, and public-projection suites green; `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` 1955/1955. No PR; `/verify` NOT run; `preview` NOT updated.
- Folded PLATFORM-086H3B-REPAIR-PRESENCE-H1-AUDIT-REMEDIATION-v1 (three confirmed repair findings; still no PR, no `/verify`): **(a) repair inspection/CAS collapsed present JSON-null durable rows into absence; (b) repair evidence certification accepted rows outside the actual H1 durable contract; (c) nested audit ledger/commit-stamp objects were not reconstructed through an exact allowlist.** (1) **Presence-aware durable reads.** A `DurableRead` (`{present:false} | {present:true;value}`) is built from `AppStateRecord | null` BEFORE `.value` for every repair-state row (partition, ledger, status, activation, witness, recovery, audit) — no `?.value ?? null` collapse. Row presence is bound in the versioned CAS digest, so absent / present-null / present-valid / present-malformed all hash DIFFERENTLY and an absent↔present-null change is caught as `revision-repair-state-changed`. A present-null partition is malformed (not absent); a present-invalid ledger (incl. JSON-null) is an ambiguous marker that blocks new-lineage init; a present-null audit row is `unavailable`. (2) **Evidence certified via the H1 contract.** The repair-specific `isSafeRepairGameRow`/`isValidEnvelopeFetchedAt` are deleted; each row is now classified by H1's `classifyGameStatsRow` (no second contract) — repair accepts only `legacy-compatible`/`legacy-statless`/`v2-complete`/`v2-sparse` and refuses everything H1 withholds (`non-persistable-*`, `unsupported-version`, `legacy-normalized-mismatch`) or rejects (`unaddressable`/`unusable-identity`/`malformed-v2`/`legacy-malformed`) as `revision-repair-evidence-malformed`. Added: canonical round-tripping `fetchedAt` (not merely `Date.parse`-finite), per-row `week`/`seasonType` partition consistency, `withheld` evidence class. Parity tests assert repair and H1 classify the same sample identically; repair never rewrites/fabricates evidence. (3) **Exact-allowlist nested audit reconstruction.** Every nested audit object (ledger, both commit stamps, action, after-state, surviving-high-water) is rebuilt field-by-field against an exact allowlist — no raw stored object retained by reference; a commit stamp is exactly `{lineage, revision}` (extra fields rejected). Any unexpected/malformed field, top-level OR nested, makes the WHOLE dataset `unavailable` (never silently stripped); `readRevisionAuditTrail` and the route return `unavailable`, so arbitrary stored content (e.g. a nested secret) can never reach the response, and apply refuses to append to such history. Deferred (untouched): fabricated failed-begin attempt handles; dormant-boundary laundering; final doc closeout. Applied repair stays dormant through the live route; the repair CAS lock chain and shared/exclusive writer concurrency are unchanged. Governance: B remains implemented, review-remediation in progress, NOT merged, NOT `/verify`-confirmed, production lifecycle inactive. Verification: focused H1-evidence/revision-repair/repair-CAS/repair-audit/admin-route/revision-authority/activation/mixed-mode-transaction/chronology/H2/public-projection/dormant-boundary suites green; `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` 1968/1968. No PR; `/verify` NOT run; `preview` NOT updated.
- Folded PLATFORM-086H3B-FAILED-BEGIN-PROVENANCE-REMEDIATION-v1 (the confirmed failed-begin attempt-provenance finding; still no PR, no `/verify`): **A caller could fabricate a structurally valid handle (`persistence: 'failed'`, higher ordinal) and claim the failed-begin exception without having received that handle from `beginGameStatsRefreshAttempt`.** Fixed by making failed-begin authority RUNTIME-OPAQUE. (1) **Opaque provenance registry.** A module-private `WeakMap<GameStatsRefreshAttempt, snapshot>` is populated ONLY when a begin's own durable write fails; `authenticFailedBegin` requires the EXACT object identity in the registry AND field agreement (attemptId/ordinal/scopeKey/startedAt) with the immutable snapshot — structural `persistence: 'failed'` alone never authorizes. A fabricated, `{...spread}`, `JSON`-round-tripped, reconstructed, proxied, or field-mutated handle loses identity or agreement → `game-stats-failed-begin-handle-invalid`; the WeakMap is weakly held (no strong-ref growth) and process-local (a restart, or the test-only `__resetGameStatsProvenanceForTests`, invalidates a nonpersisted handle). No public brand/field/token is added; the handle type is unchanged. (2) **Failure-terminal only.** A handle claiming `persistence: 'failed'` on SUCCESS or NO-OP is refused `game-stats-failed-begin-terminal-not-allowed` before any write (committed evidence never advances); only `recordGameStatsRefreshFailure` may consume an authentic failed-begin handle, and only when its ordinal strictly exceeds the stored ordinal. A `persistence: 'failed'` handle NEVER owns normally (`ownsNormally` excludes it), so its authority is confined to the one-shot exception. (3) **One-shot with bounded retry + supersession.** Provenance is consumed only after CONFIRMED persistence — a failed terminal write keeps it retry-eligible; a superseded (later persisted attempt) or tampered handle is released and returns `skipped-older`/invalid nonmutatingly and can never overwrite the later attempt; a repeated use after consumption writes nothing. Normal token+ordinal ownership, positive-only ordinals, tokenless refusal, the success commit-stamp requirement, and every prior PLATFORM-086H3B guarantee are unchanged; both new refusals keep the composite `complete: false`. The previous repair-presence/H1/nested-audit remediation is preserved unchanged; the dormant-boundary alias/wrapper laundering finding stays deferred. Governance: B remains implemented, review-remediation in progress, NOT merged, NOT `/verify`-confirmed, production lifecycle inactive. Verification: focused game-stats-chronology/status-publication/revision-authority/revisioned-writer/activation/repair/transaction/H2/public-projection/dormant-boundary suites green; `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` 1976/1976. No PR; `/verify` NOT run; `preview` NOT updated.
- Folded PLATFORM-086H3B-DORMANT-BOUNDARY-LAUNDERING-REMEDIATION-v1 (the final dormant-boundary finding; still no PR, no `/verify`): **The admin-route boundary could be bypassed through local side-effect imports and locally defined aliases/wrappers whose approved export names conceal forbidden runtime capabilities.** Closed architecturally + in the parser. (1) **Mutation-free planner.** New `src/lib/gameStats/revisionRepairPlanning.ts` owns all pure + read-only repair logic (types, digest, H1/audit classifiers, `planRepair`, high-water, `shapeLoadedState`, `getAppState`-based `loadPlanningState`/`inspectRevisionState`/`readRevisionAuditTrail`, mutation-free `planRevisionRepair` dry-run, `isCfbdSeasonType`). It imports `getAppState` only — no `withAppStateKeyTransaction`, no `setAppState`, no `repairRevisionState`. `revisionRepair.ts` is slimmed to the applied service (transactional `repairRevisionState` + `loadStateTxn`) that CONSUMES the planner (direction service→planner) and re-exports the read-only surface for existing consumers. The facade (`revisionRepairInspection.ts`) now imports EXCLUSIVELY from the planner, so it has NO direct/transitive runtime dependency on the applied-repair service or an app-state mutation primitive. Both dry-run paths (`planRevisionRepair` read-only; `repairRevisionState(dryRun:true)` transactional) reuse the same pure `planRepair` + `shapeLoadedState`, so digests are identical and the applied CAS re-validates under the full `E → activation-control → S → C` locks. (2) **Parser hardening.** `dormantBoundaryParser.ts` now REJECTS local side-effect imports (`import './x'`) and import-equals (`import x = require('./y')`) in the route + facade graph, and TRACES the guarded facade graph's exported function IMPLEMENTATIONS — resolving local aliases (direct/chained/destructured), wrappers (declarations/arrows/expressions, direct/returned calls, namespace member access), and one-or-more local helper hops — to the runtime capabilities they use. An approved export NAME can no longer conceal a forbidden terminal (`repairRevisionState`, `withAppStateKeyTransaction`, `setAppState`, and the other mutation owners); analysis is bounded to the route/facade/planner and fails closed on unresolved/computed access; type-only imports stay separate; external packages are outside analysis. Fixtures cover every prohibited form (side-effect, import-equals, direct/chained/destructured alias, direct/arrow/expression wrapper, one/multi helper indirection, namespace member wrapper, unresolved computed, mixed safe/forbidden, barrel-concealed applied-repair) plus the safe mutation-free-planner wrapper (allowed) and real-module closure assertions. Route runtime behavior unchanged (GET inspect/audit, POST dry-run, `apply:true` → `revision-repair-application-not-active`, no mutation on refusal, error redaction). Governance: B remains implemented, review-remediation in progress, NOT merged, NOT `/verify`-confirmed, production lifecycle inactive. Verification: focused dormant-boundary-parser/admin-route/revision-repair-planning/revision-repair-service/activation/chronology/transaction/H2/public-projection suites green; `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` 1986/1986. No PR; `/verify` NOT run; `preview` NOT updated.
- Folded PLATFORM-086H3B-DURABLE-STATE-CLASSIFICATION-REMEDIATION-v1 (the durable-state classification cluster; still no PR, no `/verify`): **(1) present-null refresh status could be mistaken for absence and restart revision history; (2) malformed ledger fields were silently normalized; (3) repair preserved raw presence in the digest but lost it during policy classification; (4) audit ledger reconstruction trusted the permissive validator.** Closed with SHARED classifiers (no parallel/weaker validators). (1) **Shared status classifier.** New `durableState.ts` holds the presence-aware primitives (`DurableRead`/`toDurableRead`/`presentValue`/`isPlainObject`/`isCanonicalTimestamp`), consumed by allocation + repair. `classifyRevisionStatus` (absent / recognized-legacy / valid-revisioned / malformed) in `revisionAuthority.ts` is used by BOTH `allocateGameStatsCommitStamp` and repair `shapeLoadedState` — a present JSON-null / primitive / array / unrecognized-object / malformed committed-stamp / malformed attempt-chronology status is MALFORMED (never `?.value ?? null` collapse); the live allocator returns `revision-history-ambiguous` with no mutation instead of minting a new lineage / revision 1. Recognized-legacy is only a self-describing provider-status record (never broadened). (2) **Strict ledger validation.** `validateLedgerRecord` now enforces an EXACT key allowlist, a canonical round-tripping `initializedAt` (never normalized to `""`), and a non-empty-string `repairAuditRef` — a noncanonical/object `initializedAt`, malformed lineage/revision, malformed `repairAuditRef`, or any extra field makes the ledger a malformed marker (refused, never normalized); a present malformed live ledger blocks allocation before mutation. (3) **Presence-aware repair policy.** `LoadedState` gains a typed `classifications` field (activation/witness/status/recovery/ledger via the shared validators — a present JSON-null is malformed); the raw presence-aware values still bind the CAS digest unchanged. `planRepair` refuses `revision-repair-durable-state-malformed` when ANY control/status/witness/disposition/ledger row is malformed, checked BEFORE acknowledgements (no ack waives it), and the applied service independently reclassifies the locked reread — an unchanged CAS digest never certifies malformed durable state. (4) **Audit inherits it.** `rebuildAuditLedger` uses the tightened `validateLedgerRecord`, so `initializedAt: {}` → audit `unavailable` (not a reconstructed `initializedAt: ""`). Regressions: allocator present-null/primitive/array/unrecognized + malformed-ledger (no mutation), the `classifyRevisionStatus`/`validateLedgerRecord` matrices, and a repair present-null/malformed activation/witness/status/recovery matrix (distinct digest + classification, planning + application refuse, no ledger/status/activation/evidence/audit/witness/recovery mutation, ack cannot waive, unchanged-digest cannot authorize). Valid absent + valid initialized states retain existing behavior. Deferred (unchanged): parser-laundering namespace-destructuring/computed member forms, failed-begin retry bounds, broad registry reconciliation. Governance: B remains implemented, review-remediation in progress, NOT merged, NOT `/verify`-confirmed, production lifecycle inactive. Verification: focused revision-authority/restoration-status/strict-ledger/audit/repair-loading/repair-planning/applied-CAS/activation-witness-recovery/revisioned-merge/chronology/public-projection/H2 suites green; `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` 1995/1995. No PR; `/verify` NOT run; `preview` NOT updated.
- Folded PLATFORM-086H3B-DORMANT-PARSER-COMPUTED-LAUNDERING-REMEDIATION-v1 (the remaining medium dormant-boundary parser finding; still no PR, no `/verify`): **The parser-backed capability guard did not detect namespace DESTRUCTURING or literal/unresolved computed-member laundering, so an approved export name could reach applied repair through `const { repairRevisionState: apply } = rr` or `rr['repairRevisionState']`.** Closed by extending ONLY the parser (`dormantBoundaryParser.ts`); no production repair/revision/status/activation/storage behavior changed, and durable-state classification + the shared/exclusive lock substrate were untouched. (1) **Namespace-origin resolution.** New `moduleNamespaceOrigin` resolves an identifier to a namespace value (a LOCAL guarded-graph module or an EXTERNAL package), following module-level `const alias = ns` chains; `checkNamespaceMember` resolves a member of a local namespace to its terminal (forbidden export → violation) and FAILS CLOSED on a non-static computed index into a local namespace. (2) **Destructuring.** Both module-level `collectForbiddenCapabilities` and in-function-scope tracing now bind `const { member: x } = ns` / `const { member } = ns` / `const { member = d } = ns` to the underlying `ns.member`, so a laundered binding — even one created INSIDE a function body and used via a later alias or helper hop — resolves to the capability. (3) **Literal computed access.** `analyzeImplementation`/`analyzeInitializer` resolve `ns['member']` and ns[`member`] (no-substitution template) to the export, trace aliases of them (`const apply = ns['member']`), and resolve namespace aliases plus element access (`const service = ns; service['member']`, chained). (4) **Fail-closed + safe.** Unresolved computed access to a local guarded namespace (`ns[op]`, `ns[fn()]`, ns[`p${x}`]) is a violation; ordinary computed/destructured access on non-namespace locals and external namespaces stays permitted, type-only namespace imports carry no capability. Bounded to the route/facade/planner local graph (no whole-program call graph); a namespace-derived name shadowed across nested scopes conservatively resolves to the capability (fail-safe). Fixtures (dormant-boundary suite, +6 tests) cover the two exact regressions, all twelve reqd forms (direct/renamed/default/chained/helper-indirection destructuring; literal/template/aliased/namespace-alias/chained-namespace-alias computed access; unresolved identifier/call/template computed access), the approved-name-cannot-conceal proof, and safe local-object destructuring + local/external computed access + type-only namespace import; every prior dormant-boundary case (direct/named/namespace/default/side-effect/import-equals/require/dynamic/barrel/renamed-re-export/alias/wrapper/helper) and the REAL route/facade/planner closure remain green and mutation-free. Route behavior unchanged (GET inspect/audit, POST dry-run, `apply:true` → `revision-repair-application-not-active`). Deferred (unchanged): failed-begin retry bounds, broad prompt-registry reconciliation. Governance: B remains implemented, review-remediation in progress, NOT merged, NOT `/verify`-confirmed, production lifecycle inactive. Verification: focused dormant-boundary (33/33, +6), revision-repair-planning, and admin-route suites green; `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` 2001/2001. No PR; `/verify` NOT run; `preview` NOT updated.
- Folded PLATFORM-086H3B-DORMANT-PARSER-LEXICAL-SCOPE-REMEDIATION-v1 (two valid parser false negatives from the prior remediation; still no PR, no `/verify`): **(1) the flat function-wide binding map lost provenance under sibling/nested shadowing — a later `{ const apply = safe }` block erased an earlier `{ const { repairRevisionState: apply } = rr }` block's forbidden binding; (2) destructuring DEFAULT initializers (`const { missing = fallback } = table`) were never analyzed, so a fallback reaching applied repair passed.** Closed by extending ONLY the parser (`dormantBoundaryParser.ts`); no production repair/revision/status/activation/storage behavior changed, durable-state classification and the shared/exclusive lock substrate untouched. (1) **Lexical binding resolution.** The flat map is replaced by a `LexScope` chain — function scope (params + `var` + top-level body decls), block scope (`const`/`let`/`function`/`class`), catch-clause scope, and for/for-in/for-of loop scope — each owning only its DIRECT declarations, predeclared before references are analyzed (order-independent). A reference resolves to the NEAREST applicable declaration (`lookup` distinguishes a plain local shadow from an unbound name → module-level fallback); imported namespace provenance is preserved at module scope unless a nearer lexical binding shadows it. So a safe inner/sibling binding hides a forbidden one only inside its own scope, leaving the scope restores the outer binding, and an unrelated sibling/later declaration never changes an earlier scope's resolution. Ordinary legal shadowing of a genuinely safe binding is not a violation. (2) **Destructuring defaults as runtime code.** Every `BindingElement.initializer` is analyzed as a conditionally-executable runtime expression — regardless of whether the destructured source is a guarded local namespace, an ordinary local object, an external namespace, a call result, or another destructured binding — recursively through nested object patterns, array patterns, and function/arrow parameter patterns; a default that creates an alias later invoked (chained variable, returned/helper function, approved wrapper) is caught because the default expression itself reaches the capability. Bounded: lexical bindings + the module-level graph only (no whole-program control-flow); a namespace-derived name shadowed across nested scopes conservatively resolves to the capability (fail-safe). Fixtures (dormant-boundary suite, +5 tests) cover the two exact regressions and: sibling shadows (both orders), safe-inner-over-forbidden-outer + forbidden-inner-over-safe-outer, nested-block-exit restoration, helper-local + catch-binding shadowing, same-name across separate functions; local/external/nested/array/function-param/arrow-param/chained/helper destructuring-default fallbacks; plus safe fixtures (parameter shadowing a namespace, benign numeric default, unused safe fallback, ordinary local + external destructuring, same-name safe bindings across functions and sibling blocks). Every previously closed parser case (namespace destructuring, renamed destructuring, literal/aliased computed access, namespace aliases, unresolved-computed fail-closed, direct/side-effect/import-equals/require/dynamic imports, barrels/re-exports, wrappers/helper indirection) and the REAL route/facade/planner closure remain green and mutation-free; capability approval still depends on resolved runtime provenance, never symbol/export names. Route behavior unchanged (GET inspect/audit, POST dry-run, `apply:true` → `revision-repair-application-not-active`). Deferred (unchanged): failed-begin retry bounds, broad prompt-registry reconciliation. Governance: B remains implemented, review-remediation in progress, NOT merged, NOT `/verify`-confirmed, production lifecycle inactive. Verification: focused dormant-boundary (38/38, +5) and admin-route/planning suites green; `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` 2006/2006. No PR; `/verify` NOT run; `preview` NOT updated.
- Folded PLATFORM-086H3B-DORMANT-PARSER-VAR-SCOPE-REMEDIATION-v1 (a narrowly-bounded `var`-hoisting regression introduced by the lexical rewrite; still no PR, no `/verify`): **the lexical rewrite scoped `var` declarations in loop headers and unbraced/nested control-flow bodies to the loop/block instead of the function, so a function-scoped `var` initialized from a guarded namespace (`for (var service = rr; …)`, `if (enabled) var service = rr`) lost its provenance and a facade reaching applied repair after the construct passed.** Closed by extending ONLY the parser (`dormantBoundaryParser.ts`); no production behavior changed, durable-state classification and the shared/exclusive lock substrate untouched. New `hoistVars` precollects a function body's `var` bindings BEFORE references are resolved, recursing through every nested statement form (blocks, if/else and other unbraced bodies, for/for-in/for-of headers, while/do-while bodies, switch clauses, try/catch/finally, labeled statements) and registering each in the nearest `isFunction` scope, but STOPPING at nested function/accessor boundaries (their `var`s stay isolated). Declaration provenance is preserved (initialized `var` (re)binds, incl. destructuring `var { m: x } = ns`; a bare re-declaration never clears an existing parameter/earlier binding). `predeclare` and the for/for-in/for-of case now register ONLY block-scoped `let`/`const` locally — a `var` in a loop header is no longer loop-scoped — while lexical shadowing is preserved (a nearer parameter/block binding shadows the function `var` inside its scope; leaving it restores the `var`; siblings do not erase it; binding-element defaults are still analyzed as runtime code). Fixtures (dormant-boundary suite, +4 tests) cover the two exact regressions (for-header, unbraced `if`) and while/do-while/switch/try/catch/for-in-body/for-of-body/nested-block/labeled/C-style-destructuring/ordinary-destructured `var` forms (all rejected), plus controls proving `let`/`const` stay block/loop-scoped, a benign function-scoped `var` is not flagged by name, a nearer block binding shadows a function `var`, and a nested function's `var` does not leak outward. Every previously closed parser case (lexical sibling/nested scoping, destructuring defaults, namespace destructuring/aliases, literal/unresolved computed access, direct/aliased/side-effect/import-equals/require/dynamic imports, barrels/re-exports, wrappers/helper indirection, type-only + external handling, approved-name laundering) and the REAL route/facade/planner closure remain green and mutation-free; capability approval still depends on resolved runtime provenance, never symbol/export names. Route behavior unchanged (GET inspect/audit, POST dry-run, `apply:true` → `revision-repair-application-not-active`). Deferred (unchanged): failed-begin retry bounds, broad prompt-registry reconciliation. Governance: B remains implemented, review-remediation in progress, NOT merged, NOT `/verify`-confirmed, production lifecycle inactive. Verification: focused dormant-boundary (42/42, +4) and admin-route/planning suites green; `git diff --check`, `npx tsc --noEmit`, `npm run lint:all` clean; full `npm test` 2010/2010. No PR; `/verify` NOT run; `preview` NOT updated.

### PLATFORM-086H3A-APP-STATE-MULTI-KEY-TRANSACTION-v1

- Status: **Merged via PR #398 (`platform/086h3a-app-state-multi-key-transaction`, merge commit `2793a6f`).** Previously "implemented — Codex reviewed clean — `/verify` passed — awaiting merge"; that closeout landed as PR #398. Branch was from `main` @ `c8ebed4`. First prerequisite (A) of the PLATFORM-086H3 decomposition — the single-branch monolith (`platform/086h3-atomic-game-stats-contract-activation`, HEAD `e1a9593`) is frozen as a read-only salvage reference and was NOT cherry-picked; this PR was reconstructed from `main`. Includes the folded PLATFORM-086H3A-LOCK-ORDER-ENFORCEMENT-REMEDIATION-v1, PLATFORM-086H3A-LOCK-IDENTITY-OVERLAP-REMEDIATION-v1, PLATFORM-086H3A-LOCK-FAILURE-POISON-REMEDIATION-v1, PLATFORM-086H3A-DUAL-ERROR-SHAPING-REMEDIATION-v1, and PLATFORM-086H3A-CLEANUP-BEFORE-SHAPING-REMEDIATION-v1 (the lock-order, lock-identity, lock-failure, dual-error, and cleanup-ordering notes below are the historical folded-remediation record — the final reviewed contract and verification outcome are summarized in the closeout paragraph immediately after this Status line).
- Final reviewed scope (verified, awaiting merge): a generic multi-key app-state transaction capability in `src/lib/server/appStateStore.ts` — `AppStateKeyTxn` gains `readKey`/`writeKey`/`lockKey` over the existing single-key `read`/`write` (all current callers preserved). PostgreSQL runs ONE client and ONE `BEGIN`/`COMMIT` for the whole transaction (every lock/read/write on that client; conservative uncertain-COMMIT containment; uncertain clients destroyed). The file fallback stages all writes and commits them in ONE atomic replacement (all-or-nothing; a failed replacement leaves the prior file intact) — it is in-process development/test support only, and PostgreSQL is the production correctness boundary. Secondary locking is EXPLICIT (`lockKey`); lock identity is an INJECTIVE `(scope, key)` tuple (`JSON.stringify([scope, key])`, identical on both backends; persisted ROW keys unchanged); acquisition is ENFORCED monotonic (fail-fast `AppStateTxnLockOrderError`, reacquisition idempotent), and overlapping `lockKey` requests are SERIALIZED so state advances only after successful acquisition. A failed required-lock acquisition POISONS the transaction (noncommittable even if the promise was un-awaited/caught/discarded); callback, lock, and cleanup failures are all truthfully represented (`AppStateTxnCallbackLockError` — callback value verbatim as `cause`, typed lock failure as `lockFailure`; wrapped in `AppStateTxnCleanupError` on rollback failure), and the combined error is CONSTRUCTED only after complete backend cleanup (rollback + client containment; every file-lock slot release). **Codex review is clean and `/verify` passed.** Verification: full suite `1817/1817`; focused transaction + dormant-boundary suites `91/91`; `tsc`, `lint:all`, and `git diff --check` clean; production `/api/game-stats` HTTP responses were byte-identical to `main` across cache hit, corrupt durable store, stale read, parameter validation, cache miss, and unauthorized-refresh probes — after establishing `.env.local`/build-cache parity between the two servers (an initial comparison used unequal `.env.local` state, a methodology artifact, not a product defect; environments and caches were normalized and the full probe matrix reran clean). No live provider or database was contacted; no game-stats lifecycle was activated (the transaction primitive has no production route caller — only the dormant `durableMerge.ts` imports it). The transaction primitive is DORMANT: `src/lib/server/appStateStore.ts` is the only production file changed vs `main`, and production behavior is unchanged.
