...
- Status: **Implemented — review pending — not merged.** Branch `platform/086h3a-app-state-multi-key-transaction` from `main` @ `c8ebed4`; branch pushed; no PR opened. First prerequisite (A) of the PLATFORM-086H3 decomposition — the single-branch monolith (`platform/086h3-atomic-game-stats-contract-activation`, HEAD `e1a9593`) is frozen as a read-only salvage reference and was NOT cherry-picked; this PR was reconstructed from `main`. Includes the folded PLATFORM-086H3A-LOCK-ORDER-ENFORCEMENT-REMEDIATION-v1, PLATFORM-086H3A-LOCK-IDENTITY-OVERLAP-REMEDIATION-v1, and PLATFORM-086H3A-LOCK-FAILURE-POISON-REMEDIATION-v1 (see the lock-order, lock-identity, and lock-failure notes below).
