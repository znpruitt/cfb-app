...
### PLATFORM-086H3-ATOMIC-CONTRACT-ACTIVATION-RECOVERY-INTEGRATION-v1

- Status: **Implemented — review pending — not merged.** Branch `platform/086h3-atomic-game-stats-contract-activation` from `main` @ `c8ebed4`; pushed with `preview` updated; no PR opened. Includes the folded PLATFORM-086H3-CANONICAL-COVERAGE-RECOVERY-REMEDIATION-v1 (Codex first round), PLATFORM-086H3-IDENTITY-RECOVERY-CLAIM-PUBLIC-CONTRACT-REMEDIATION-v1 (second round), PLATFORM-086H3-STATUS-RECOVERY-PUBLIC-CLOSURE-REMEDIATION-v1 (third round), PLATFORM-086H3-STATUS-REVISION-PUBLIC-PROJECTION-CLOSURE-REMEDIATION-v1 (fourth round), and PLATFORM-086H3-REVISION-ATOMICITY-PUBLIC-DUPLICATE-GUARD-REMEDIATION-v1 (fifth round). Awaiting the delta-focused Codex re-review.
- Purpose: Atomically activate the PLATFORM-086H1 contract and PLATFORM-086H2 durable merge authority as the ONE production game-stats lifecycle: canonical schedule → validated provider observations → durable merge authority → committed durable state → coverage and schedule-relative recovery → cache/read model → analytics projection → truthful availability. Governing invariant honored: **every game-stats writer routes through the durable merge authority (or the same per-partition transaction-scoped lock)** — no partial activation, no parallel writer system, no blind partition overwrite.
- Result: **Canonical attachment.** `src/lib/gameStats/ingestion.ts` derives slate expectations that RETAIN canonical participants: each scheduled game carries both participants' `teamIdentity.ts` resolution (canonical identity key with the central normalization for registry-unknown labels — no raw string equality anywhere), FBS/FCS classification (resolver identity first, canonical-schedule conference policy fallback), neutral-site semantics, status, and addressability. A provider game id alone NEVER authorizes persistence: an observation merges only when its participants resolve and AGREE with the canonical participants in schedule orientation (documented exception: neutral-site games accept the identity-preserving reversed orientation). Typed attachment states — `matched` / `participant-mismatch` / `unresolved-participant` / `excluded-classification` / `placeholder-deferred` / `unscheduled-id` — are never collapsed; FCS-vs-FCS games are excluded even when scheduled (FBS-vs-FBS, FBS-vs-FCS, FCS-vs-FBS persist); a NUMERIC postseason id with unresolved placeholder participants defers (typed, never ordinary absence). Payload validation stays on the single strict parser (`invalid-payload`/`schema-drift`/`valid-empty`/`no-persistable-observations`). **Committed-state coverage & publication.** `partitionCoverage.ts` is the ONE coverage evaluation (per-game `satisfied`/`recoverable`/`manual-only`/`blocked`/`absent`; typed top-level `complete`/`partial`/`blocked`/`manual-only`/`absent`/`not-applicable` — a blocked-only partition is BLOCKED, never recoverable absence) consumed by the cron, manual refresh, ordinary reads, recovery planning, provider diagnostics, and the provider cache-state probe (the legacy `usableGameStatsGameIds` usability shell is deleted). New `refreshPublication.ts` is the ONE post-merge path both writers use: merge COMMIT → durable REREAD → committed-row classification → coverage → status/HTTP — success (recorded `partial` whenever committed coverage is not complete) publishes only after that sequence; `unchanged`/`stale` resolve by committed coverage (complete → truthful no-op; incomplete → `game-stats-stale-insufficient`/`game-stats-unchanged-insufficient` failures, so no-write outcomes never clear a meaningful failure); a contextually UNEXPECTED empty response is a stable failure (`game-stats-empty-unexpected`), never "no applicable data"; `conflict`/`unavailable` fail with durable state preserved; `indeterminate` publishes no success and no post-write coverage; a post-commit reread failure is `game-stats-postcommit-reread-failed` (the merge may have committed — never reported as available). The AST guard enforces that success/no-op publication for game-stats exists ONLY in this module. **Bounded cross-run recovery.** New `recoveryDisposition.ts` persists a per-partition disposition (attempt count, typed last reason, deterministic backoff tiers 30m→7d cap, next-eligible time, terminal manual-action state, meaningful-progress reset) in its own operational scope; recovery planning selects the newest ELIGIBLE candidate, so a backed-off/terminal newer partition rotates selection to older eligible ones — no tight indefinite retry, no newest-slate starvation, still at most one slate provider request per cron run. **Writers & reads.** The manual refresh rejects invalid `seasonType` values (never coerced) and requires ≥1 canonically identified, classification-eligible, addressable-or-pending scheduled target BEFORE any provider access (`game-stats-no-canonical-targets`/`game-stats-schedule-unavailable` 409s); ordinary reads stay provider-free and wire-compatible while adding a coverage-derived `meta.availability` summary (complete/partial/blocked/absent/manual-only/not-applicable × fresh/stale, plus typed corrupt/malformed/read-failure 500s distinct from absence). The blind overwrite `setCachedGameStats` remains DELETED; `publicProjection.ts` strips `schemaVersion`/`fetchStartedAt`/`pointsProvided` (legacy rows by reference — byte-equivalent). **Consumers.** Owner analytics stays on `selectAnalyticsRows` exclusively; the archive-integrity score comparison now consumes the analytics projection (a v2 row with `pointsProvided: false` or compatibility-defaulted points is never a real score); provider diagnostics and cache-state report through the shared coverage/classification. **Guards.** `activation-guards.test.ts` is AST-based (TypeScript compiler API): it catches aliased/renamed/namespace-member mutation calls, same-file wrapper functions, const-derived `game-stats` scope strings, independent `withAppStateKeyTransaction` users, mutation-primitive imports in the writer routes, publication outside the finalize path, resurrected legacy-normalizer imports, and DISCONNECTED lifecycle wiring (dead merge/coverage/projection call sites) — with fixture self-tests for each bypass class and a documented statement of what the boundary structurally guarantees. Deterministic tests cover canonical-attachment regressions, manual pre-fetch target validation (zero provider calls for every invalid target), the full merge-outcome/status matrix, committed-coverage e2e flows (partial commit → partial status → partial public availability), and repeated-run quota regressions for every unresolved outcome (failure once → backoff → zero provider calls → no failure clearing → older-candidate rotation). Full suite 1875/1875 at that round; `tsc` and `lint:all` clean. Legacy rows continue to serve through compatibility reads with **no migration required**. Deliberately NOT included: PLATFORM-086H4 diagnostics/panel presentation redesign (queued) and the legacy-row migration (queued).
- Second remediation (IDENTITY-RECOVERY-CLAIM-PUBLIC-CONTRACT): **Authoritative identity only** — the normalized-text identity fallback is deleted: a participant is identified exactly when the central resolver resolves it through the team-catalog/alias authority (`resolution.status === 'resolved'` + canonical identity key); registry-unknown labels keep an EMPTY key, defer their game (typed `unresolvedIds`), and can never match — normalization-collision regressions (`A-B` vs `AB`, punctuation/whitespace/case) prove distinct labels never merge. Classification is an explicit ALLOWLIST (`FBS|FBS`, `FBS|FCS`, `FCS|FBS`): any UNKNOWN side defers (typed `classificationUnknownIds`; unknown is never FBS, never FCS, never persistable); conference-policy fallback applies only to catalogued identities. Manual target validation requires an authoritative, classification-eligible target before ANY provider access (zero-fetch tests for unresolved/unknown/collision/FCS-only/placeholder-only partitions). **Atomic fenced recovery claims** — `recoveryDisposition.ts` now runs claim acquisition inside the per-key transaction lock on the recovery-metadata key (never held across provider access): fenced `attemptToken` + lease (`RECOVERY_CLAIM_LEASE_MS`), atomic attempt increments, token-CONDITIONAL finalization (late failure cannot overwrite newer success and vice versa; stale tokens no-op; expired claims reclaim with a NEW token, history preserved, lease-bounded retries), safe retirement (satisfied → cleared; blocked/manual-only → terminal `manual-action`), and claim/finalization persistence failures stay visible (no claim → no provider request; finalization failure surfaces on the result). Meaningful progress derives ONLY from committed-COVERAGE fingerprint change or canonical-SCHEDULE fingerprint change (`computeCoverageFingerprint` — fences excluded; `computeScheduleExpectationFingerprint` — placeholder/identity/classification/addressability changes only): fence-only refreshes with unchanged gaps escalate backoff instead of pinning the newest partition at the 30-minute tier. **Orchestration boundary** — new `refreshOrchestration.ts` is the ONE writer entry (`runScheduledGameStatsRefresh`/`runManualGameStatsRefresh`): plan → claim (candidates newest-first, racing runs rotate to older eligible partitions; overlapping-cron barrier tests prove single-fetch-per-partition) → fetch (route-supplied thunk) → ingest → committed-state finalize → conditional claim finalize; manual refreshes participate in the same lifecycle with documented override semantics (skip backoff, fence out in-flight scheduled claims, typed dispositions recorded identically). Writer routes are thin shells importing no mutation/status/disposition/coverage/raw-row machinery. **Commit-order truth** — the merge authority stamps `commit: {committedAt, commitSeq}` IMMEDIATELY after COMMIT; publication forwards that stamp (never regenerates it), so an older commit publishing after a newer one can never roll back last-success metadata (stalled-finalizer ordering regression). **Public contract** — `readAvailability.ts` validates the COMPLETE weekly envelope (shape, partition identity vs request, `fetchedAt`, `games` array) before serving (typed `game-stats-durable-state-invalid` failures); `buildPublicWeeklyGameStats` publishes only H1-approved rows: legacy by reference (byte-equivalent), v2 stripped, `schemaVersion: 3`/malformed-version rows WITHHELD with typed counts (never laundered into legacy-looking data), structurally malformed rows withheld — `meta.availability`, `meta.withheld`, and the served `games` always agree. **Provider cache-state** — `evaluateYearGameStatsAvailability` uses the same schedule-relative committed coverage (one eligible row unrelated to the schedule is never availability; no-schedule-with-rows → `unknown`, never proven absent). **Mixed-payload observability** — parse/attachment diagnostics ride the publication (`attempt: {parseFailures, attachment, degraded}`); a successful commit with payload degradation records PARTIAL success and keeps the diagnostics (never erased). **Archive integrity** — `scoreEvidence.ts` + route-level regressions (strict v2 evidence, `pointsProvided: false` never a real score, unsupported schema/malformed/conflicting-duplicate exclusion, deterministic duplicates). **Guards** — module-OWNERSHIP boundaries over resolved imports (evidence mutation → merge authority only; recovery mutation → disposition module, callable only from orchestration; success/noop publication → finalize path only; failure/attempt recording → finalize + orchestration; coverage evaluation → shared consumers, never routes; raw durable rows → lifecycle internals + documented compatibility readers; orchestration entries → writer routes only) plus the corrected AST pass (aliased/namespace lock calls, chained scope constants, wrapper scope arguments at any position, aliased publication imports) with self-tests per bypass class; the header documents exactly what is structurally guaranteed and that cross-file wrapper laundering/reachability are NOT proven by scanning — they are prevented by the import boundaries instead. Full suite 1916/1916 at that round; `tsc` and `lint:all` clean.
- Third remediation (STATUS-RECOVERY-PUBLIC-CLOSURE): **Durable atomic refresh-status ordering** — the process-local commit sequence is retired as ordering authority: the merge authority allocates a durable partition `commitRevision` (`(prior ?? 0) + 1`) INSIDE its advisory-locked evidence transaction (globally comparable across processes, instances, and restarts; internal metadata, stripped from the public wire), passes it through ingestion → finalization → `recordProviderRefreshSuccess`, and the success compare-and-write now runs inside ONE per-scope `withAppStateKeyTransaction` (read → revision compare → write → COMMIT; never a separate getAppState/setAppState pair). Ordering rules: newer revision wins; an equal revision is an idempotent duplicate/retry; a stored status without a valid revision (legacy row or malformed value) yields to the first revision-carrying commit; datasets without revisions keep the legacy committedAt/seq path; attempt CHRONOLOGY stays separate from committed-success ordering. Regressions cover the stale-publisher race (an older revision publishing LAST cannot overwrite newer metadata), duplicate/malformed rules, and restart-independent ordering with cleared module state. **Post-claim authoritative revalidation** — the pre-claim recovery plan is never trusted after claim acquisition: `claimAndRevalidateNextCandidate` rereads the canonical schedule expectation AND committed durable coverage after each claim, releases now-satisfied/ineligible candidates with a token-conditional `satisfied` finalization (zero provider calls; a stale validation cannot clear a replacement claim), rotates to the next eligible candidate, and keeps the one-fetch-per-run bound; stale-plan race and rotation regressions prove a partition another writer filled can never trigger a duplicate fetch. **Strict public contract** — the weekly envelope's `fetchedAt` uses the shared strict RFC 3339/calendar parser (Feb-30-style `Date.parse` normalization rejected); public rows use an EXPLICIT H1 allowlist (`legacy-compatible`, `v2-complete`, `v2-sparse`) with typed withheld buckets for defective legacy states, non-persistable v2 states, unsupported/malformed schema (never laundered), structurally malformed rows (classification precedes any field access — no projection exceptions), rows whose own week/seasonType contradict the requested partition, and conflicting duplicates (the deterministic duplicate policy runs BEFORE the wire: identical copies collapse, divergent copies are entirely withheld). In-repo consumer review: the only `/api/game-stats` consumers are the admin bypass flows (`GameStatsCachePanel`, `manualRefresh`), which check `res.ok` and display counts without absence semantics — the 200-with-`meta.availability`/`meta.withheld` contract is safe for them (H4 owns panel wording). **Complete degradation observability** — `attempt.degraded` now covers every provider-boundary bucket (parse failures, participant mismatches, unresolved participants, excluded classifications, unscheduled ids, placeholder deferrals, merge-skipped non-persistable rows, no-persistable outcomes) with typed counts carried through publication into the cron and manual responses; committed availability is never downgraded by extraneous degradation and the degraded attempt is never hidden (per-bucket matrix regressions). **Re-export-safe boundaries** — guarded capabilities cannot be re-exported by ANYONE, owners included: named/aliased re-exports, `export *`, namespace re-exports, barrels, local `export {}` of guarded bindings, and exported aliases (`export const tx = withAppStateKeyTransaction`) all fail the guard, with self-tests per laundering shape; the status ledger joined the transaction-lock owner allowlist. **Visible recovery-metadata failures** — manual provider-failure responses carry the recovery outcome with the stable `game-stats-recovery-metadata-failure` code and a disposition-persistence-uncertainty marker while preserving the provider error separately; scheduled results surface retirement/finalization failures (`recoveryFailures` + stable code) and stale-claim releases (`staleClaims`) on every result kind; failure-injection regressions assert provider-call counts, evidence immutability, and both error surfaces. Full suite 1931/1931; `tsc` and `lint:all` clean.
- Fourth remediation (STATUS-REVISION-PUBLIC-PROJECTION-CLOSURE): **Durable revision LEDGER (non-resettable ordering)** — commit-revision allocation no longer trusts the partition row alone: a per-scope ledger (`'game-stats-revision'`, key `year:week:seasonType`) is read inside the merge authority's advisory-locked evidence transaction, and the next revision is the FLOOR over every valid durable source (ledger revision, partition `commitRevision`, status `lastSuccessRevision`) + 1 — malformed values contribute nothing, so a partition restored from backup (or corrupted) without its revision can never reset ordering below status history; the ledger row co-commits ATOMICALLY with the partition through new secondary-key transaction access (`AppStateKeyTxn.readKey`/`writeKey` — same client, same transaction, no additional advisory lock; documented as serialized by the primary key's lock). Regressions: restored-partition floor (status remembers 10 → next commit is 11), malformed partition revision (ledger keeps the floor), restart/multi-instance ordering with zero shared process state. **Every status mutation is one durable transaction** — begin/success/noop/failure all run through a single per-scope transactional helper (read → normalize → compare → write → COMMIT) returning a TYPED persistence result (`persisted`/`idempotent`/`skipped-older`/`skipped`/`failed` — a swallowed status write no longer exists); success ordering compares ONLY valid incoming commit revisions (newer wins, equal is idempotent, an invalid stored revision yields to revision-carrying evidence; legacy datasets keep the committedAt/commitSeq path); attempt chronology stays separate from committed-success chronology (`lastSuccessRevision`); per-attempt diagnostics persist durably in the status row (`lastAttemptDiagnostics`), and a publication whose EVIDENCE committed but whose status write FAILED records `partial-success` with the typed `statusPublication` surfaced on cron and manual responses (dead best-effort read/write helpers deleted). **Explicit public construction** — the public v2 wire is BUILT from explicit allowlists (`PUBLIC_TEAM_NUMERIC_FIELDS`, explicit team/row/envelope constructors), never by spreading a persisted object and deleting known-internal names: unknown persisted fields (recovery bookkeeping, `commitRevision`, anything future) are dropped by construction; strict optional partition fields treat a MALFORMED own `year`/`week`/`seasonType` as a mismatch, never as absent. **Public duplicates through the H1 authority** — `selectAnalyticsRows` (the same selection coverage/analytics/archive-integrity use) decides duplicate publication: an eligible v2 copy SUPERSEDES its equivalent compatible legacy copy (a coverage-satisfied pair never serves an empty `games` array), analytics conflicts withhold every copy, and no-eligible groups (v2-sparse duplicates) publish only when their public projections are indistinguishable — cross-format regression matrix included (defective/unsupported copies never join the decision). **Complete degradation taxonomy** — `canonical-unresolved` and `classification-unknown` are DISTINCT typed attachment states and counts (no longer folded into placeholder deferral), and all-matched-but-statless payloads carry `noPersistableObservations`; every bucket flips `degraded`, rides the response `attempt`, and persists durably in status. **Structured post-claim errors** — post-claim revalidation failures throw `GameStatsRecoveryRevalidationError` (stable `game-stats-recovery-metadata-failure` code; primary stage `schedule-context`/`durable-reread`; every secondary recovery-metadata failure; `providerAccessOccurred: false`; lease-bounded truth), shaped through the cron route as a 500 carrying BOTH causes; dual-failure regressions (primary reread failure + release-finalization failure) run at the library level and through the actual cron route with zero provider fetches asserted, using new deterministic seam grace counts (`afterReads`/`afterWrites`) so the claim commits while its release fails. **Guard closure** — generic raw `getAppState('game-stats')` reads are restricted to documented owners (cache reader, diagnostic route); alias/wrapper laundering is detected by fixed-point binding propagation plus a pure-forwarder rule (single-statement re-exported wrappers of guarded capabilities fail; multi-statement domain APIs pass); merge/ingestion/finalization capabilities gained import-OWNERSHIP rules (durable merge names importable only by ingestion; ingestion entry only by orchestration; finalize only by orchestration). The scripted fake-pg harness now applies MULTI-row transactional commits atomically (the merge's partition + ledger co-commit), matching real Postgres. Full suite 1952/1952; `tsc` and `lint:all` clean.
- Fifth remediation (REVISION-ATOMICITY-PUBLIC-DUPLICATE-GUARD): **Atomic file-fallback multi-key transactions** — the dev/test file backend no longer writes each `write`/`writeKey` immediately: `withAppStateKeyTransaction` now STAGES every write against one loaded snapshot (reads are staged-first for read-your-writes; untouched keys come from the snapshot), and on a successful callback commits ALL staged keys in ONE atomic file replacement under the whole-file write lock (the live file is reread inside that lock and only the staged keys overlaid, so concurrent unrelated-key writers are never clobbered). A callback throw discards every staged write; a failed replacement leaves the prior file intact and surfaces a typed `AppStateTxnFinalizeError` with `writeAttempted:false` (the atomic-rename contract proves nothing became durable). Evidence + revision-ledger co-commit therefore roll back together on the file backend exactly as on Postgres; regressions cover two/three-key commit, second-write and final-replacement rollback, malformed staged value, read-your-writes, and retry-after-failure. `AppStateKeyTxn` gains `lockKey` (Postgres: a second `pg_advisory_xact_lock` on the same client; file: the target key's chain slot held until the transaction settles) for the one-time revision bootstrap's status consultation, with a documented acyclic partition → status ordering. **Enduring revision-ledger authority** — the per-scope ledger is now a schema-versioned record (`{schemaVersion:1, revision, initializedFrom:'new'|'legacy'|'bootstrap', initializedAt}`) and, once initialized, is the SOLE ordinary allocator (`ledger.revision + 1`) — ordinary allocation never reconstructs ordering from mutable partition/status rows. A one-time BOOTSTRAP (no valid ledger) classifies the scope under a locked status-floor read: provably NEW (no partition, no status marker, no ledger marker) → 1; a RECOGNIZED pre-revision legacy partition (explicit shape: legacy-only rows, no `commitRevision`) → 1 (`initializedFrom:'legacy'`); any valid surviving floor (partition revision or status `lastSuccessRevision`, the latter read under that key's `lockKey` so a concurrent publisher cannot race the floor) → max(floors)+1; otherwise (revision-era markers survive but no usable source) a typed `revision-history-ambiguous` refusal that makes NO evidence write, allocates NO revision, preserves data, and surfaces `game-stats-revision-history-ambiguous` — never a silent reset to 1. Regressions: new/legacy/valid-ledger/missing-partition-revision/malformed-partition/missing-ledger-with-status/valid-partition-no-ledger/malformed-ledger-with-status/ambiguous/all-malformed/conflicting-floors/locked-bootstrap-race/restart floors. **Composite begin/terminal status publication** — `statusPublication` is now `{begin, terminal, complete}`: the begin marker result carries through orchestration and route shaping (never discarded in config/provider/failure branches), a terminal transaction records its attempt identity + outcome + diagnostics truthfully even when its BEGIN never persisted (chronology-guarded so a begin row is not a precondition, while a genuinely stale attempt still cannot overwrite a newer one — strict id match unless the begin failed, then strictly-newer `startedAt`), and `complete:false` after a begin failure means route output never implies a fully-recorded lifecycle. Begin/terminal failure matrix + route regressions included. **Always-new public construction** — the by-reference envelope fast path is removed: EVERY public level (envelope, game, team, `raw`, numeric fields) is built from an explicit allowlist and no persisted object is returned by reference; `team.raw` is restricted to the recognized contract categories (`RECOGNIZED_GAME_STAT_CATEGORIES`), dropping unknown/future/internal categories (e.g. the unmodeled `completionAttempts`) by construction. **Public-visible duplicate selector** — a purpose-specific selector replaces analytics-equality for the wire: it applies H1 format precedence (eligible v2 supersedes an equivalent legacy copy) then compares the ACTUAL PUBLIC PROJECTIONS of the winning class, so two copies that agree for analytics but differ in any public-only field (return statistics) are a public CONFLICT and every copy is withheld; array order never picks a winner, `conflictingDuplicates` counts conflicted GAME IDS, and a coverage-satisfied equivalent legacy+v2 pair still yields one deterministic game (never `games:[]`). Contamination (envelope/game/team/raw/nested), non-string-raw, order-independence, three-copy, and return-stat-conflict regressions included. **Sanitized recovery revalidation errors** — `GameStatsRecoveryRevalidationError` separates an internal raw cause (logged server-side only) from STAGE-SPECIFIC stable public codes (`game-stats-revalidation-schedule-context-failed`/`-durable-reread-failed`/`-coverage-failed`) and safe summaries; its `toPublic()` projection emits the recovery-metadata code (`game-stats-recovery-metadata-failure`) ONLY when a recovery-metadata operation actually failed (a primary reread failure whose claim release succeeded carries just the stage code + zero-fetch metadata), and every secondary failure is projected through `toPublicRecoveryMetadataFailure` (stable operation, code, safe summary, `dispositionPersistence:'uncertain'`) — no SQL, paths, tokens, rows, or stack traces reach the wire; the cron/manual routes serialize only these safe projections. Redaction, primary-only, and dual-failure regressions assert the wire carries no injected SQL/path/token/secret. **Guard closure (realistic)** — exported forwarding-wrapper detection now flags any function that returns a guarded call forwarding its OWN parameters (or a rest spread) through `await`/parentheses/`as`, an aliased or namespace callee, default/rest params, and non-first argument positions, while a genuine domain API that constructs its own scope/key (module constant + internally-derived local) is intentionally allowed; call-site callee unwrapping, alias-chain propagation into the guarded-binding set, a multi-file resolution test (owner alias, barrel, downstream importer, extra-statement wrapper), and a bounded-claims header (static imports/aliases/wrappers/exports only — no runtime/dynamic-dispatch claims) are added. Formatting fixed (`providerRefreshStatus.ts`). Full suite 1986/1986; `tsc` and `lint:all` clean (verified by exit status).
