...
- Purpose: two admin debug routes read season archives under an unchecked query-string `leagueSlug`, minting entries tagged `archive:<slug>` that no rollover can ever fire ([#778](https://github.com/znpruitt/cfb-app/issues/778)). Precedent [#774](https://github.com/znpruitt/cfb-app/issues/774). Forensic detail in the PR and the prompt.
- Scope: the two routes, `src/lib/seasonArchive.ts` (**both readers** — widened at the receipt gate), `listAppStateKeys`' test seam, their suites.
- Outcome: **the refusal lives in the AUTHORITY, covering both readers, returning `null`/`[]`.** The sibling carried it, not the call-site ratio: `listSeasonArchives` mints its own cache identity under the same tag, so guarding `getSeasonArchive` alone leaves it open for the next caller. `null` is the contract honoured, not a refusal in disguise. It sits in the outer `React.cache` wrapper; inside `unstable_cache` the entry is already minted. A **malformed** registry throws; **missing** is absence. Routes keep `404 league-not-found`. Three prompt claims corrected: `revalidate: false` is a one-year TTL; the split is 8/3/4, not 11/4; no member-facing surface shares the hole.
- Review / verification: RV1; lint/tsc/test each exit 0, delta **+13 at both ends** (5,328 -> 5,341). Reproduced live and absent after, with a real-slug control that still mints. **Both reviewers gathered against `31698c92`**; six findings, all accepted, and the two sharpest are this entry's L1 and L4. **L4:** the guard made four pre-existing store-failure tests **vacuous** — readers mutated to swallow every failure passed 24/24, and removing the guard alone reddened exactly those four; fixed by extending the scoped read-failure seam to `listAppStateKeys`. **L1:** "the registry read is free" held only for RSC renders — `React.cache` does not dedupe in Route Handlers, measured against an in-process RSC control (page 1, route 3, `/api/insights` at 7 archives 3 -> **14**).
- Status: PENDING OWNER RULING on that amplification; otherwise merge-ready, branched `fec5842b`.
- Open work: the amplification is **unresolved and not swept in** — every contained fix left scope. RESIDUE: already-minted entries are neither enumerable nor purgeable; an absent registry makes archives unreadable (pinned, unreachable in production).
- Owed to planning at merge: Season Launch invariant 4's "Still open: two admin debug routes cache archives under an UNVALIDATED `leagueSlug`" goes false. Replacement: **"Both archive readers refuse a slug no league holds, returning `null`/`[]` before any entry is minted, and the debug routes answer `404 league-not-found` — #778. The refusal is the AUTHORITY's, which is where this differs from #770/#774: only a caller can tell a client-supplied year from a server-derived one, whereas no legitimate server-derived slug names a league that does not exist."** At merge verify BOTH directions here and in `docs/next-tasks.md` — pure appends merge cleanly, which is the hazard.
