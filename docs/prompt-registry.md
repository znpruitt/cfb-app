...
- Follow-ups: Operator can now dry-run against a read-only connection, then `--apply` against a writable primary. Unblocks the PLATFORM-081 production `--apply` operator step. (Operator cleanup since completed: removed `aliases:test:2025`/`aliases:test:2026`/`aliases:tsc:2025`; confirmation dry-run found zero remaining legacy league-scoped alias keys.)
...
- Deletion status: **Deferred to a documented manual operator step — NOT automated.** This session cannot touch production data, so the deliverable is tooling (dry-run by default, `--apply` gated) plus a runbook step; an operator runs `npm run cleanup:legacy-aliases` against prod, reviews the dry-run, then re-runs with `--apply`.
...
- Follow-ups: Production `--apply` run is an operator action (runbook step above). **Dry-run hotfix → PLATFORM-081b** (the dry run hit a read-only-connection DDL failure in production; fixed so it inspects without writes). Next queued: `STANDINGS-PRESEASON-STATE` docs-contradiction verification; deferred product decisions (CSV current-season guard, owner-identity mapping, whether to schedule PLATFORM-040).
