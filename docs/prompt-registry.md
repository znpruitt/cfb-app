...
- Status: **Implemented dormant on branch `platform/086h3c5-numeric-participant-validation` from `main` @ `b301774` (impl `d95de9e` + this docs closeout); Codex review of the code/test diff clean on the FIRST pass (no actionable regressions); a final review of the complete code+tests+docs diff gates the PR. Merge + post-merge status flip pending.** Gates: focused schedule suites 82/82 and dormant game-stats suites 179/179 (dormant-boundary guard UNMODIFIED and green); `tsc` / `lint:all` / `git diff --check` clean; full `npm test` 2030/2030; `npm run build` clean. Local-only runtime probes (file store; no provider or production contact): an old-shaped seeded schedule cache serves with NO fabricated ids, a new-shaped cache serves ids verbatim, and all seven schedule/game-stats probes are **byte-identical to `main`**.
