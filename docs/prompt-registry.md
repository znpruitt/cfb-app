...
- Verification: `git diff --check` clean; `npm run lint:all` clean; `npx tsc --noEmit` clean. New `src/lib/__tests__/seasonArchive.test.ts` (15 tests — key/tag isolation across slug+year, read/write round-trip shape, cross-league/cross-year isolation, sorted year list, genuine empty list, plus read-failure propagation + post-recovery success for both readers) passes; standings + insights + rollover/history consumer tests (`selectors-leagueStandings`, `loadInsights`, `insights-context-aliases`, `seasonRollover-aliases`, `selectors-historySelectors`, `leagueRecords`, `historyOverview` — 168 total) all green.
