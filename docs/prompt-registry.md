...
- Outcome — THE DETECTOR IS THE CACHE'S OWN WRITE RECORD. `unstable_cache` publishes one of two
  ways: with a work store it defers into `pendingRevalidates[invocationKey]`
  (`unstable-cache.js:211`), and without one it `await`s `cacheNewResult` INLINE (`:249`) and never
  touches that map. The route brackets the cached read and reads whichever signal exists, with
  `generatedAt` — stamped by every snapshot constructor from a `currentDate` that is deliberately not
  part of the cache key — separating "this request computed it" from "an existing entry was
  returned". **The prompt's compute-counter mechanism was refuted**: it must live inside
  `leagueStandings.ts`, and it is PER-INSTANCE while the data cache is shared across Fluid instances.
- Outcome — FOUR verdicts, not the three the receipt named, and the extra one is not decoration.
  `bypassed` exists because `workStore.isDraftMode` gates the read AND the publication
  (`unstable-cache.js:143` and `:204`), so a draft-mode request recomputes and stores nothing — it is
  neither a hit nor a miss. `unavailable` checks `globalThis.__incrementalCache` as well as the work
  store, because `unstable_cache` falls back to it and `base-server.js:852` sets it process-wide.
  `matches` is `null`, never `true`, whenever an EMPTY result cannot be read as agreement
  (`blockedBy`: `cached-side-not-a-snapshot`, `shared-archive-cache`, `empty-population`) — and a
  blocker gates absence only, so an observed difference reports `false` under all of them.
- Outcome — `computeCanonicalStandingsUncached` exported for one caller, pinning `statusOverride` to
  `undefined`. The `leagueStatusOverride` bypass was rejected on correctness: it cannot express
  "whatever the registry says", so it cannot reproduce a status-absent league — the shape
  `/api/admin/leagues` creates by default. An import-graph test fails if anything outside the route's
  directory names the symbol, or if the defining module aliases it.
- Findings recorded. **Comparison is per OWNER, not per team** — `CanonicalStandings` carries no
  team-to-owner map; `roster` lives in `LiveDerivation` and never reaches the snapshot. **No `ties`
  column is possible**: `deriveStandings` drops final ties (`standings.ts:219-224`), and the only tie
  count in the system is the hardcoded `ties: 0` that `toHistoryStandingsRows`
  (`standingsHistory.ts:83-88`) writes onto archive rows — so archive-sourced rows carry an
  undeclared fabricated `ties` and live-sourced rows carry no such key.
- **Named blind spot, enumerated in the payload as `sharedNestedCaches`.** "Fresh" means uncached at
  the canonical-standings layer; `seasonArchive.ts` has two `unstable_cache` families that BOTH sides
  read through, and the archive-YEARS one is read on **every** season compute via `resolveSeason` —
  not only on archive-sourced leagues. A test fails if that file gains a third cache site, so the
  enumeration cannot go stale. Every other canonical input is a direct `getAppState`.
- Cost, measured on the read-only replica 2026-09-17: a 2026 rebuild walks **3,679** cached schedule
  items (347,718 bytes) against a **138**-item catalog and a 138-row owners CSV; ≈9 store reads per
  compute pass, so ≈10-11 and one build on a hit, ≈19-20 and two on a miss. **Nothing rate-limits
  it** — no `/api/debug/*` route has a limit or a `maxDuration` — so the admin gate is the only
  control. Year and slug are both bounded before any build: #770/#774 for the year, and #778's
  unknown-slug ruling for the slug, which `getCanonicalStandings` does not enforce itself (it
  computes an empty snapshot and publishes it under a year-long entry keyed on the caller's string).
- **#771 is untouched.** `isAuthorizedAdminRequest` returns `!isProductionRuntime()` when
  `ADMIN_API_TOKEN` is unset, so the 401 test pins `NODE_ENV='production'` and says in-file that it
  therefore does not exercise #771's condition. Planning verified `ADMIN_API_TOKEN` is set for
  Production and Preview on 2026-09-17 — **a measurement of current state, not a property: the
  exposure returns if the variable is removed.**
- Corrections to the prompt's fact table: `revalidate: false` is a 365-day ceiling, not "never"
  (`unstable-cache.js` maps it to `CACHE_ONE_YEAR`, `constants.js:263`); eleven `/api/debug/*` routes
  existed, not nine; `origin/main` was `c9cb3195`, not the `ca55da85` the prompt named.
- **Review: two remediation rounds, and the second was owner-approved under AGENTS.md step 6.** Both
  reviewers, both rounds, same commit each time, gathered before any change; Codex verified by exit
  code and by a `git diff` against the merge-base in the transcript on every run. Round 0: 13
  findings, all confirmed, none refuted. Round 1: 7 findings, all confirmed, **three of them defects
  round 1 itself introduced** — including a HIGH where widening the cache check to the global without
  widening the publication signal made the route report "nothing was stored, no snapshot exists" for
  a request that had durably written two entries.
- **What the two rounds actually taught, because it is the reusable part.** All 20 findings were one
  class: the route asserting something about state that was not true. The recurring surface was
  hand-written English claims in the payload — `restsOn`, `caveat`, `excluded`, `freshSideBypasses`,
  `comparisonReach` — which no test checked and three of which shipped false. In two cases **this
  ledger stated the fact correctly while the shipped string stated it backwards.** Round 2 deleted
  that surface rather than correcting it again: `blockedBy` is an enum produced by the same predicate
  that gates `matches`, so the two cannot disagree, and `sharedNestedCaches` is an enumerated fact a
  test pins. The generalisable rule: **a claim a reader will act on needs a test or a derivation, and
  prose beside code is neither.**
- One surviving mutation is documented in the code as documented-not-tested: dropping
...
- **Round-3 confirming pass: `/code-review` 6 findings, Codex 3. One REFUTATION, four new.**
- **The refutation, recorded with its evidence so no later round re-derives it — planning has filed
  it as refuted on [#819](https://github.com/znpruitt/cfb-app/issues/819).** The finding that
  `classifyCacheRead` branches on `workStoreCachePresent` where Next branches on the work-store
  ...
  return fires before the discriminator is consulted. Verified independently.
- **Round 4, owner-authorized and final on this class.** Three fixes, all one shape — the detector
  describing more than it observes — plus two proof-surface items that rode along.
- **The millisecond verdict, reported three times across both reviewers, is closed at its reachable
  cause.** `bypassed` was derived by ELIMINATION (no publication and the stamp matches), so a
  snapshot warmed in this request's millisecond flipped a genuine hit into "recomputed and published
  nothing". It is now gated on the observed `isDraftMode` flag, which gates `unstable_cache`'s read
  and publication alike (`:143`, `:204`) and is the only state that can produce it.
  **THE RESIDUALS ARE NAMED RATHER THAN IMPLIED AWAY**, and reported per request as
  `provenanceRestsOnTimestamp`: the inline branch keeps no publication record, so the stamp is its
  only signal; and a stale entry served with a background revalidation queues a publication, so
  `miss` and `hit`-with-revalidation are separated by the stamp alone. Both need a same-millisecond
  coincidence; neither is removable without a per-execution identity `unstable_cache` does not
  expose.
- **A BLIND SPOT IS NOT A BLOCKER, and collapsing the two would have been the wrong fix.**
  `resolveSeason` and `resolveOffseason` both read `listSeasonArchives` through its own tag-only
  cache, so a stale years list sends BOTH sides down the live branch and they agree for that reason —
  the case planning rated highest. But the rows on that path are still independently re-derived from
  six direct `getAppState` reads, so blocking `matches` outright would answer `null` for essentially
  every real request and detect nothing at all. The total block stays scoped to
  `source === 'archive'`, where the ROWS come through the shared cache; the narrower exposure is
  named in `blindSpots`, per lifecycle, so a reader can tell **"the two sides agree" from "the two
  sides read the same possibly-stale input"**. Preseason carries none — `resolvePreseason` never
  calls `listSeasonArchives`.
- **The publication summary counts from entry.** `resolveStandingsYear` consults
  `listSeasonArchives` on an offseason league and can queue a publication before the bracket opens. I
  recorded that ordering as protection against a false POSITIVE without noticing it left a false
  NEGATIVE in the summary. The verdict still reads the narrow bracket; the summary reads from an
  entry snapshot taken before year resolution.
- **Two proof-surface items, both the same keyed-on-a-rendering-detail mistake.** The import-graph
  guard walked `src/` only while five scripts import from `../src` — including
  `recover-game-stats.ts`, the one place CLAUDE.md says holds the production WRITE credential, so an
...
- Verification: `tsc --noEmit`, `lint:all`, `npm run build` and `npm test` each exited 0 on branch
  ...
  named assertion was proven — including a round-2 mutation that restores the round-1 HIGH, and a
  round-4 mutation that restores the elimination rule round 4 removed. Two round-4 mutations
  initially SURVIVED against a vacuous publication test; the fixture was rebuilt on an offseason
  league with no archives so the pre-bracket write is the request's only publication, and both then
  reddened. `origin/main` moved to `057cca04` mid-review and was merged in; the review base was
  re-derived and verified an ancestor rather than reused.
...
- **THE GATE IS UN-RUN. Fail-closed on preview is not the gate.** The route is deployed on preview
  ...
  authenticated preview call would answer about preview's snapshot. **The gate stays un-run until an
  authenticated call happens post-merge** — that is the step that decides whether this route can tell
  a warm snapshot from one it just created, and it has not happened. No amount of green here
  substitutes for it.
- Prediction on record for that run: `tsc`/2026 should report `hit` with an empty delta, because
  `/api/cron/live-scores` calls `invalidateAndWarmStandingsForYear` after every committed score write
  (`route.ts:458,:591`) and republishes in the same request — **so a clean `tsc` result confirms the
  warmer runs and says nothing about whether the cache can go stale.** `test`/2025 is the informative
  target: status `season` 2025, no archive under `standings-archive:test`, inputs untouched since
  July. Both are `blindSpots`-bearing season leagues, so read that field beside `matches`.
...
- Evidence: **the prompt's SECOND CONSUMER section did not reproduce, and it was the section added to correct an earlier miss.** `loadRecapContextForSeasonScope` gates on `isWeeklyRecapActiveSeason` — `status.year === seasonYear`, an EQUALITY check — so an absurd year returns `null` before `loadRecapContext` runs and the `year < seasonYear` archive filter at `loadRecapContext.ts:44` is never reached. Measured with the control that makes it non-vacuous: legit year `absent` (entered the loader), both absurd years `inactive` (stopped at the gate). Even if reached, `tsc`'s archives are 2018 and 2021–2025, all already below 2026, so the amplification factor would be **1×**. **The REAL second consumer is canonical standings** — `resolveStandingsYear` lets an explicit override win unconditionally, so each distinct year minted TWO cache identities, and the standings entry carries `revalidate: false` (tag-only, no time expiry; its year tag can never fire, it survives on `standingsSlugTag` alone). Rebuild cost measured at **112.1 ms** against a **203.2 ms** populated 900-game build — not the ~157 ms the prompt carried, because the year-scoped reads all miss while the league-wide fixed cost does not. **Reachability, measured on the production replica: all three leagues (`tsc`, `test`, `pruitt`) carry a password**, so `leagueAuth` condition 1 is satisfied by none and the live vector today is any password-holding member, not an anonymous caller.
- Review / verification: **both reviewers gathered against `32b226e1` before any remediation.** Codex: no findings. `/code-review high`: two low findings, adjudicated separately rather than applied wholesale. **(1) ACCEPTED and fixed** — emptiness was decided on the trimmed value and validity on the untrimmed one, so `?year=%20` was absent (200) while `?year=%202026` was a 400 that `Number.parseInt` had served before this slice; now trimmed once, both decisions read the same value, pinned by a test mutation-proven to be the only one that goes red when the trim is removed. **(2) ACCEPTED AS COMMENT ERROR, NOT BEHAVIOUR** — see Open work. Delta **+10, measured at both ends** (5,271 → 5,281). **5 of 10 red against `main`, every one on the defect's own assertion** (`expected 400, actual 200`); the 4 green are the controls and no-regression pins — a legit year on the same fixture still builds, an omitted year still resolves the operating season, next season still serves, and #627's refusal still precedes the bound. The generator-invocation counter is what makes "no rebuild" an observation rather than an inference from a status code.
...
- Open work: **REVIEW FINDING 2, adjudicated as pre-existing and left in place.** A legacy record whose operating year is BELOW `MIN_SEASON_YEAR` — `status: { state: 'season', year: 1999 }` with a later `league.year` — is accepted by the route (the disjunct admits a league its own year) and then discarded by `loadInsights.ts`'s floor in favour of `league.year`, while `loadWeeklyRecap` still scopes to 1999: one response, two seasons. **MEASURED as reachable with NO `?year=` at all** — `resolveLeagueOperatingYear` returns the same sub-floor value — so the bound neither causes nor worsens it, and the reviewer said so. Not fixed inside this scope: raising the floor would refuse such a league its own feed on BOTH callers, which is the failure the disjunct exists to prevent, and narrowing the disjunct would contradict the owner's explicit ruling. What WAS wrong and is fixed: the new `loadInsights.ts` comment claimed the route is the authority for caller-supplied years without naming this exception. Not reachable in production — no league carries a sub-2000 status year.
- Open work: **echo-versus-derive CLOSED** — bounding caps per-league key cardinality at 28 values (2000..`currentYear+1`), ≤168 entries across three leagues and two caches, self-retiring; a derive layer would buy a constant factor and cost a seam. **`/api/history/[slug]/[year]` is the same defect and is [#774](https://github.com/znpruitt/cfb-app/issues/774)** — same `isAuthorizedForLeague` gate, `year < 2000` floor with no ceiling, and `getSeasonArchive` caches a genuine `null` under `revalidate: false`. Three corrections to the prompt's own class survey, none swept in: a **ninth** parser hides under `parseSeasonParam` (`api/debug/insights/[slug]/suppression/route.ts:78`, admin-gated) where a `parseYear` grep cannot see it; `api/debug/schedule-eligibility` is the eighth copy and has **no floor either**, contradicting "every one `n >= 2000`"; and **`draft/[slug]/[year]` GET is NOT admin-gated** — it uses `isAuthorizedForLeague`, the same gate as Insights, so "the other seven are draft routes, admin-gated" was generalized from the POST. The draft GET mints no cache entry (a durable-store read returning 404), which is why it is reported rather than fixed here.
...
- Outcome: **DECIDED — a caller-supplied year is accepted iff it is an integer and `MIN_SEASON_YEAR <= n <= resolveLeagueOperatingYear(league)`, OR `n` appears in `listSeasonArchives(slug)`; else 400 `{ error, field: 'year' }` at the route and `notFound()` on the page.** **NOT #770's `currentYear + 1` ceiling** — an archive is the record of a season that FINISHED, so that ceiling admits at least one year no league can ever hold. **NOT the archive list alone either**, and the gap-year case is what rules it out: `tsc` genuinely has holes at 2019 and 2020, and the page has a designed "No archived data found for the {year} season" empty state for exactly that — bounding on the list would convert a designed surface into a `notFound()` and answer "no archive" with "bad year". The disjunct makes refusing a held archive structurally impossible rather than merely unlikely, and costs no new cache identity: `listSeasonArchives` is keyed `['season-archive-years', slug]` — slug alone, confirmed by reading the constructor — is `React.cache`-wrapped, is already read by seven league pages, and runs only after the range has rejected. **Integers are the larger half**: a range alone leaves the accepted set DENSE, so no enumeration bounds it.
- Evidence: **THIS IS THE UNCLOSED HALF OF A HAZARD THE CODEBASE HAD ALREADY DIAGNOSED, not a new finding, and that is the most useful sentence this branch leaves.** `rolloverTargeting.ts:83` names `2026.5` by name and says an unusable year would let `saveSeasonArchive` "mint a permanent, TTL-less archive" under such a key; PLATFORM-086F2H1R4 hardened the WRITER against exactly this value. The READ path never got the same guard, so `getSeasonArchive` would mint the entry the writer had been forbidden to mint — the precedent for the fix was already in the repo. **Reproduced live** (`NEXT_PRIVATE_DEBUG_CACHE=1`, seeded passwordless league): 11 API requests produced **8 distinct entries on disk**, including `archive:<slug>:2026.5`, `2026.6`, `2026.0000001`, `20000000000` and `2016` (from `0x7E0`); the page independently minted `2029.25` and `2029.75`. **Post-fix the same probes produce 4** — `2018`, the in-range gap year `2019`, `2026`, and exactly one slug-keyed year-list entry — with zero dense entries and the page 404ing. **The prompt's "its only tag fires once a season" is wrong and was corrected**: every entry carries TWO tags. The per-year tag is DEAD for a junk year (no writer can emit `archive:<slug>:2026.5`); the live one is the slug tag, which is what sweeps them, and it fires once a league per season. `invalidateSeasonArchive`'s caller set was re-derived by rename mutation, not counted: renaming it yields exactly one `tsc` error (inside `saveSeasonArchive`), and renaming `saveSeasonArchive` yields four importers of which **one is production** — `api/cron/season-rollover/route.ts`, inside its `eligible` branch. **Production values, read-only replica, 2026-09-13:** `tsc` archives 2018/2021–2025 with operating year 2026; `test` and `pruitt` hold none, operating 2025 and 2026. **Severity: all three leagues carry a `passwordHash`**, so the live vector is a password-holding member, not an anonymous caller.
- Review / verification: `npm run lint:all` **exit 0**, `npx tsc --noEmit` **exit 0**, `npm test` **exit 0** — each its own command, each its own real exit code. Delta **+33, measured at both ends** (5,281 → 5,314). **The first baseline run exited 1 with `fail 0, cancelled 1`** — `OverviewScoreboardGrid.browser.test.tsx` hit its 30 s file timeout under full-suite load; it passes alone in 2.1 s and the rerun was clean, so it is a resource flake and the known-failure set stays EMPTY. Mutations, each reddening only the tests that own its half: restoring the pre-#774 parser at both callers turns **4 of 10 route tests** and **4 of 8 page tests** red, the sharpest on `expected 400, actual 200` against a **planted archive at store key `2026.5`** — a fixture only the defect could ever reach; dropping the integer test reddens 4 resolver tests, dropping the disjunct 2, widening the ceiling by one year 3. All 33 pass under `test:clock-shift -- 800`, which is measured rather than asserted: the bound reads no clock, because its ceiling is league-relative rather than `currentYear + 1`.
...
- **THE ODDS HALF IS NOT IN THIS BRANCH.** `oddsRefreshExecutor` is byte-identical to `main`, its suite is removed, and the 15th call site's body read is still unbounded — tracked as [#759](https://github.com/znpruitt/cfb-app/issues/759). It came out under a stopping rule agreed IN ADVANCE (owner, before the final round): one last review, and anything further in the odds seam meant the odds half shipped separately while the clean parts merged regardless. The final round found FOUR odds-seam findings across both reviewers, including an unguarded durable write on a path that was write-free on `main` (a failed `setLatestKnownOddsUsage` would have turned a truthful `provider-fetch-failed` into `unexpected-error`, against this prompt's own CARRIES best-effort invariant) and usage headers persisting across attempts so an early attempt's balance could be recorded as authoritative after later billed attempts. The rule fired and was honoured rather than argued around — the escape hatch pre-committed is what PLATFORM-098 lacked.
...
- Scope: EXTENDED by owner ruling to close [#632](https://github.com/znpruitt/cfb-app/issues/632). The prompt's SCOPE excluded "the timeout VALUES at the ten call sites" on the premise the items were separable; this fix is what couples them. Under the old semantics a site's total ceiling was effectively unbounded (headers bounded, body free), so preserving 12s would have silently moved it to 12s against PLATFORM-115's measured 8.2 s / 16.0 s / 21.5 s completions. Ten CFBD sites moved to `CFBD_PEAK_LATENCY_TIMEOUT_MS` — one existing constant, no new value. Spend does not rise (`maxAttempts` untouched; per #632's measurement an aborted call bills as much as a completed one, so a slow body succeeding in one attempt costs 1 where 12s would have aborted and retried for 3). Worst case becomes ~120.9 s at the five 3-attempt sites, replacing an UNBOUNDED worst case; no `maxDuration` envelope was added, which would have been the new number the ruling excluded — recorded in [#757](https://github.com/znpruitt/cfb-app/issues/757).
...
- Review / verification: THREE rounds, both reviewers each time, 17 findings. Applied: the round-1 deadline/guard/timer findings; the round-2 HIGH via reconstruction rather than patching. Escalated: the `timeoutMs` semantic change, which became the #632 ruling. Declined with reasons: a pre-existing undrained-body finding; a wall-time finding whose "serialized by pacing" premise is wrong (`applyPacing` chains waits, not requests) — residue to #757. Reverted: everything in the odds seam. **Two of my own tests were caught VACUOUS by mutation before shipping** — a timer assertion filtering `process._getActiveHandles()`, which reports `[]` for an armed timer on Node 22, and a usage assertion that held whether or not the request recorded anything. Nine mutations, each read for WHICH assertion fired; the surviving suite's multi-attempt test fires under M7 while all eight single-attempt tests stay green, which is the property the first two rounds lacked. `npm test` 5,238/5,238 exit 0 (+9 from 5,229); `tsc`/`lint:all` exit 0; clock-shift +400 d green. See L3, L4, L6.
