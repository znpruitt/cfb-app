...
- Purpose: `api/debug/archive-audit` and `api/debug/archive-integrity` read season archives under a `leagueSlug` taken from the query string and never checked, so a slug naming no league minted a cache entry tagged `archive:<slug>` — a tag no rollover can ever fire ([#778](https://github.com/znpruitt/cfb-app/issues/778)). Governing precedent: [#774](https://github.com/znpruitt/cfb-app/issues/774), merged `4c939fbf`; the issue was found by that lane while adjudicating a review finding whose framing it corrected.
- Scope: the two routes, `src/lib/seasonArchive.ts` (**both `getSeasonArchive` AND `listSeasonArchives`** — added at the receipt gate), and their suites. NOT the year bound, NOT the other debug routes' year parsers, NOT the archive write path.
- Outcome: **DECIDED — the refusal lives in the AUTHORITY, covering both readers, and returns `null`/`[]` rather than throwing.** **The sibling is what carried the decision, not the call-site ratio the prompt opened with.** `listSeasonArchives` mints a SEPARATE cache identity (`['season-archive-years', slug]`, tagged `archive:<slug>`, same `revalidate: false`), so a guard in `getSeasonArchive` alone would leave it open for the next caller — an argument the prompt did not make and the ratio could not reach. **`null` is the contract being honoured, not a refusal smuggled into a return value**: `null` is already this function's answer for "this league has no archive", and a league that does not exist genuinely has none, so all eight previously-safe callers keep their failure mode exactly. **The guard sits in the outer `React.cache` wrapper, never inside the `unstable_cache` callback** — inside, the entry would already have been minted before the callback ran, which is the thing being closed. The two routes keep an explicit `404 league-not-found` matching `debug/insights-career-diagnostic`: the authority stops the mint, the route says why, and mutation confirms those are separable (removing only the route guard still blocks the mint and loses only the reason).
- Evidence: **Reproduced live** (`NEXT_PRIVATE_DEBUG_CACHE=1`, seeded league): each route answered `404 No archive found` for a nonexistent slug **and left an on-disk entry anyway**, tagged `["archive:zzz-no-such-league-778","archive:zzz-no-such-league-778:2020"]` and `["archive:qqq-no-such-league-778","archive:qqq-no-such-league-778:2019"]`, body the literal `null`. Post-fix both answer `404 league-not-found` and **mint nothing**, while a real slug still mints `["archive:seed778","archive:seed778:2024"]` — so the absence is a measurement that looked, not one that failed to look. Control throughout: `insights-career-diagnostic` with a nonexistent slug minted nothing before the fix either. **The tag's unreachability was re-derived, not inherited**: `revalidateTag` with an `archive:` argument exists at exactly two lines (`seasonArchive.ts:323-324`, inside `invalidateSeasonArchive`), whose one `src/` caller is `saveSeasonArchive`, whose one non-test caller is `api/cron/season-rollover/route.ts:293`, which iterates the league REGISTRY; `scripts/` has none. **`getLeague`'s cost was measured, not assumed**: instrumenting `readLeagueRegistry` and rendering a history page that issues six `getLeague` calls (its own, one per archive year, one for the year list) produced **ONE registry store read**.
- Evidence: **the call-site population the prompt supplied was wrong and was re-measured.** 15 non-test call sites split **8 route-param / 3 query-string / 4 library**, not the prompt's "eleven RSC / four query-string" — `/api/history/[slug]/[year]` takes a dynamic route SEGMENT, not a query value. So **two of three** query-string callers forgot to validate, not two of four. All 8 route-param sites were probed with a bogus slug **against a registry containing a real league** (the first sweep ran against an EMPTY registry, where every slug 404s trivially — a degenerate population, re-run) and minted **zero** entries, with a positive control proving the same harness records entries for the real slug on the same page.
- Evidence: **site 7 (`admin/[slug]/preseason/owners:46`) is safe BY READING, not by observation** — the probe got a `307` from middleware and the page body never executed. Its guard is `getLeague` at line 19 and `notFound()` at line 21, unambiguous on the page, but it is the one cell in that sweep with a different warrant and is recorded as such rather than counted with the rest.
- Evidence: **`recap/loadRecapContext.ts:45` is safe by three unrelated accidents, not by a check**, and this is the sentence most worth keeping. It calls `getLeague` NOWHERE. An unknown slug fails to reach `listSeasonArchives` only because `readConfirmedRosterInputs` returns an empty owners CSV and the function bails first; its two consumers then gate independently (`api/insights/[slug]/route.ts:162` behind `league &&`, `league/[slug]/insights/page.tsx:26` early return). Three independent paths, none of which exists to protect this, and **nothing would fail if any one changed**. After #778 the authority covers it, which is a second reason the refusal belongs there.
- Evidence: **NOT an instance of a class, unlike #770 and #774** — and the whole population was classified, not sampled. `src/` contains exactly **four** cross-request cache constructions (`unstable_cache(` in three files; zero `'use cache'` directives), all four examined: the two archive readers carry a slug tag and nothing else; `dataCachedCanonicalStandings` is `revalidate: false` but also carries `ALL_STANDINGS_TAG`, a shared global tag that reclaims any entry regardless of slug (probed: a bogus slug on the standings page mints nothing, with a real-slug control that does); `dataCachedRawInsights` has a genuine TTL. Owner ruling: do not widen to either.
- Evidence: **`revalidate: false` is NOT "no time expiry", and the issue said it was.** `node_modules/next/dist/server/web/spec-extension/unstable-cache.js:27` reads `typeof revalidate !== 'number' ? CACHE_ONE_YEAR : revalidate`, and the minted entries carry `"revalidate":31536000` on disk. The accurate claim is **no tag-driven reclamation, and a one-year floor** — past every operational horizon, but not unbounded. Corrected in the prompt and in #778's body at the receipt gate.
- Review / verification: `npm run lint:all` **exit 0**, `npx tsc --noEmit` **exit 0**, `npm test` **exit 0** — each its own command, each its own real exit code. Delta **+12, measured at both ends** (5,328 → 5,340), zero failures at either end; the known-failure set stays EMPTY. Mutations, each reddening only what it owns: **removing both authority guards turns exactly the 3 refusal tests red while both controls stay green** — the controls staying green is what proves the red is the guard and not a broken fixture — and the failures name the guard's own assertions (`an archive sitting at the store key must NOT be served under a slug no league holds`). **Removing only the two route guards reddens the 2 route-refusal tests on `expected 'league-not-found', actual 'No archive found for leagueSlug=audit-absent year=2024'`** — which also demonstrates the layering: with the route guard gone the authority still blocks the mint, and only the reason is lost. Every refusal fixture **plants a real archive at the store key under the unknown slug**, so the route would answer 200 without the guard and a status-code assertion genuinely discriminates.
- Review / verification: **the test bill was quoted at the receipt gate as two files and was actually three** — `seasonArchive.test.ts` and `seasonArchiveYearBound.test.ts` were predicted (9 failures, exactly as forecast), and `recap/__tests__/weeklyRecap.test.ts` was not (6 further failures, found only by the full-suite run). Fitting that the miss was the recap loader — the one site whose production path this closeout describes as protected by accident. All three were fixed by **planting registry entries, never by weakening the guard**: a suite that could read archives for leagues that do not exist was testing a state production cannot reach.
- Status: PENDING REVIEW at `a4d53217`, branched at `fec5842b`.
- Open work: **RESIDUE, entries already minted are not reclaimable and this slice does not claim otherwise.** Existing junk entries are neither enumerable nor purgeable short of a global cache purge; the one-year TTL is their only floor. This stops new ones. It does not clean old ones, and nothing here closes the exposure retroactively.
- Open work: **RESIDUE, an ABSENT or MALFORMED registry makes every archive unreadable**, pinned by a test rather than fixed. `readLeagueRegistry` classifies both as non-`ok` and `getLeagues` flattens them to `[]`, so no slug is held. Nothing in production reaches it — every page and the rollover cron resolve a league from the same registry first, so a store with archives and no usable registry has no surface that could ask for one — but the flattening is now load-bearing for archive reads in a way it was not before, and a future change to it has to come past that assertion.
- Open work: **RESIDUE, both routes echo the raw `leagueSlug` into their 404 body**, including the new one (`{ error: 'league-not-found', leagueSlug }`, matching the sibling exactly). Harmless — `requireAdminAuth`-gated, JSON content type — and noted because it is reflected unvalidated input and the classification is what surfaced it, not a gate.
- Owed to planning at merge (planning owns `AGENTS.md`): in Season Launch invariant 4, the sentence **"Still open: two admin debug routes cache archives under an UNVALIDATED `leagueSlug`, and those entries carry a tag no rollover can ever fire — #778."** goes false. Suggested replacement: **"`getSeasonArchive` and `listSeasonArchives` both refuse a slug no league holds, returning `null`/`[]` before any cache entry is minted, and the two admin debug routes answer `404 league-not-found` — #778. The refusal is the AUTHORITY's rather than each caller's, which is the one place this invariant's shape differs from #770/#774: a year splits into client-supplied and server-derived and only a caller can tell which, whereas no legitimate server-derived slug names a league that does not exist."**
- Open work: **AT MERGE, VERIFY BOTH DIRECTIONS IN THIS FILE AND IN `docs/next-tasks.md`, not just that these rows landed.** These edits are pure appends and will merge cleanly, which is the hazard rather than the reassurance — PR #777 silently reverted a true correction through exactly such a merge. Read the merged file and confirm the pre-existing rows survived alongside the new ones, with a control proving the check can report a genuine absence.
