...
- Change: `?bypassSuppression=1` on `/api/insights/[slug]` now requires a platform admin ([#627](https://github.com/znpruitt/cfb-app/issues/627), audit finding S1, prerequisite 6 of 6 in [#610](https://github.com/znpruitt/cfb-app/issues/610)). It was gated only by `isAuthorizedForLeague`, which admits ANY caller on a passwordless league. **DECIDED: an unauthorized request carrying the parameter gets 401**, from `requireAdminAuth(req)` — not an ignored parameter and not a 404. 401 costs no legitimate caller anything, because nothing in the app sets the flag; ignoring it would serve an expired-session admin a normal feed that looks like a successful bypass. The guard sits AFTER the existing 404 blend-in, so an unauthorized caller on a passworded or unknown league still gets 404 and cannot distinguish the two.
- Scope limit: the route and its suites only. A caller not passing the parameter is served identically, admin or not — pinned by test. `ADMIN_API_TOKEN` gained no new dependency beyond what `requireAdminAuth` already had. **`generators/membership.ts`'s duplicate check was KEPT**, and so was `career.ts`'s `rookieBenchmarkGenerator` guard: they are not redundant, they are what makes the disclosure half already closed, and route protection is never the authority.
- Evidence: **the issue's disclosure claim does not reproduce, and this is the item's main finding.** `shouldSuppressGenerator` has exactly two entries and BOTH carry a non-bypassable copy inside their own generator (`membership.ts:291-300`, `career.ts:1072`), so `bypassSuppression` lifts the engine gate and publishes nothing. Measured through the route — anonymous, passwordless, borrowed roster — the two bodies differ in `generatedAt` alone; measured at the engine with positive controls that fire (`rookie-benchmark-newowner-2025` at `usingArchivedRoster=false`; `membership-joined/left` with `seasonOwners` set), `EXTRA=[]` under bypass. #627's body says a visitor can "receive withheld editorial output"; that is false against this code. **What was live is the COMPUTE half**: the uncached branch in `loadInsights.ts` skips a 300 s cache and rebuilds per request — ~157 ms median on a 900-game file-backed fixture, higher on Neon, reachable anonymously and unboundedly. The guard's remaining value on the disclosure side is prospective: AGENTS.md Season Launch invariant 4 permits a future rule to live in the engine only, and that rule would have been liftable by query string. **The trap in the prompt did not land**: `vercel env ls` (names/environments only) shows `ADMIN_API_TOKEN` configured for Production AND Preview, so a `Request`-bearing guard is inert on local dev and the test runner only — pinned by test on both sides of that branch. `resolvePlatformAdminDecision(req)` measured `authorized` for an anonymous request with the token unset outside production, in both Clerk-secret states.
- Review / verification: `npm run lint:all` 0, `npx tsc --noEmit` 0, `npm test` 0 — each its own command. Test delta **+12 measured at both ends (5,258 → 5,270)**, 0 failures at either end; the known-failure set is empty. Three tests go red against the parent on the intended assertion (200 !== 401). The "uncached build is unreachable" assertion is the only one a second mutation caught: moving the guard AFTER the build leaves the 401 green and turns the generator counter red (`1 !== 0`), so the status code alone would not have proven it. The admin control uses a stand-in generator registered under the suppressed id WITHOUT the in-generator copy, because a control built on the two live rules would have been green for the wrong reason.
- Status: (pending merge) from `claude/627-insights-bypass-authorization`.
- **Owed to the planning lane at merge, in `AGENTS.md` → Season Launch invariant 4** (planning owns that file; the lane must not edit it): the clause **"`bypassSuppression` is NOT admin-enforced today"** and the instruction **"Pre-existing, queued as PLATFORM-101; do not write new guidance that assumes the flag is privileged until that lands"** both become false on merge. The parenthetical **"(so `career:rookie_benchmark` reaches a caller during the rollover window, which invariant 5 forbids)"** was already corrected by the owner in `fd777a92`. The sentence "The flag skips the `unstable_cache` entry (a full uncached recompute per request)" stays true and is now admin-only.
