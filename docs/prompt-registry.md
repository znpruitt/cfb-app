...
- Status: **implemented + review-converged (4 remediation cycles + a clean confirming Codex round), full gate green; opened as PR #420 to `main`, awaiting owner merge; DORMANT.** Merging does NOT start automatic Odds polling — that is the separate deployment-runbook §8g operator activation (provision `turfwar-odds-hourly`, exact-authentication proof gates-closed, then open both gates). PLATFORM-086C2 completes the 086C slice; PLATFORM-086D/086E schedule-maintenance and rankings cadences remain planned.
