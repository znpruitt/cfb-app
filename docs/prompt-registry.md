...
- Follow-ups: Production `--apply` run is an operator action (runbook step above). Next queued: `STANDINGS-PRESEASON-STATE` docs-contradiction verification; deferred product decisions (CSV current-season guard, owner-identity mapping, whether to schedule PLATFORM-040).
