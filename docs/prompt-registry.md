...
- Status: (pending merge) from `claude/627-insights-bypass-authorization`.
...
- Status: (pending merge) from `claude/770-insights-year-bound`.
...
- Status: (pending review) from `claude/774-history-year-bound`, branched at `396c45ff`.
...
- Open work: **the #627 and #770 entries above still read `Status: (pending merge)`** though both merged (`d7a97917`, `31fc892a`). Reported rather than edited — they are those branches' closeout rows, not this one's.
