...
- Result:
  ...
  - CFBD usage display continues to derive its limit from the provider-reported patron tier (`resolveCfbdUsage`), never a hardcoded 1,000 — surfaced with `remaining` as the authoritative figure (the user is on a higher tier).
