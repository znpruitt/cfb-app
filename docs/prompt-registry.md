...
- `oddsRefreshExecutor` disposition: **keeps its own read, takes the deadline with it, shares the classifier.** It persists a durable usage snapshot between headers and body — deliberately, "the request spent credits regardless" — so handing that read to the retry loop would put a durable write inside a retried callback. `startUpstreamBodyDeadline` is armed at the HEADERS on the REMAINING budget, so the lane's ceiling is one `timeoutMs` per attempt, not two plus the write. A body timeout or transport death there is now `provider-fetch-failed`; only bytes that arrived and were not JSON stay `odds-invalid-payload`.
...
- Review / verification: seven findings across both reviewers — three applied (all mine: the odds deadline armed after the durable write, reproduced at 507 ms accepted against a 300 ms deadline; a negative guard form inverted to positive; a VACUOUS timer assertion, `process._getActiveHandles()` reporting `[]` for an armed timer on Node 22, replaced with instrumented timers and a positive control), one escalated to the ruling above, three declined (one pre-existing undrained-body finding flagged; two sandbox-only command failures that pass here). Six mutations, each read for WHICH assertion fired — incl. `parse` made retryable firing only on "must bill exactly ONE provider call, expected 1, actual 3", and the odds fresh-budget regression printing `saw [12000,12000,3000]`. `npm test` 5,244/5,244 exit 0 (+15 from 5,229); `tsc`/`lint:all` exit 0; clock-shift +400 d on both new suites 12/12. See L3, L4, L6.
- Status: open on `claude/662-upstream-body-deadline` (`f4daf849`); PR and merge SHA recorded at merge, when #632 closes with it.
