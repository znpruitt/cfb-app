...
### PLATFORM-001A-ATTACHMENT-REGRESSION-TESTS-FIRST
- Purpose: Add regression coverage for schedule-based score/odds attachment and schedule eligibility BEFORE changing odds matching behavior (PLATFORM-001B). Score attachment is strong; odds attachment is still pair-only and can misattach same-pair games (rematches, bowls, CFP repeats, duplicate provider events).
