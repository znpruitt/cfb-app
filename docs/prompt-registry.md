...
- Change: `CacheEntry.firstFinalObservedAtById` records when LIVE polling first saw CFBD report a score final ([#692](https://github.com/znpruitt/cfb-app/issues/692)) — write-once; nothing reads it. **Neither a game end time nor a confirmed final:** a `/scoreboard` `completed` row stamps at its PROVISIONAL observation, and `classifyScorePackStatus` tests the status label alone, so a score-less final can be stamped. With `itemUpdatedAtById` it brackets the tail: `stamp − kickoff`, then `itemUpdatedAtById − stamp` for the straggler path.
...
- Evidence: "stamp in `scoreMerge` only ⇒ FBS only" was wrong — the merge is SHARED, and `finalScoreSweep` finalizes the non-FBS population `/scoreboard` (`classification=fbs`) never sees: 355 of 454 week-1 rows at one `2026-09-08T12:00Z` cron clock, on the read-only replica. Hence `stampFirstFinalObservation`, absent by default, on the two live sites only and not keyed on `onlyIfMissingUsableFinal`. A swept row is stamped in `itemUpdatedAtById` and absent here, so the exclusion is self-describing. First-write-wins is a code rule: two paths re-fire the branch for a stamped id. See L1, L4.
- Review / verification: reviews pending. `tsc`/`lint:all` 0; `npm test` 5,192/5,192 (+20); nine mutations, each caught at its own assertion.
