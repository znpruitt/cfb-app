...
- Change: the upstream request deadline now spans BODY consumption ([#662](https://github.com/znpruitt/cfb-app/issues/662)). It was cleared when the response HEADERS arrived, so the body downloaded unbounded — measured on `main`, a 1,000 ms body resolved against a 50 ms deadline. `fetchUpstreamJson` consumes inside the attempt loop via a shared `runUpstreamAttempts`, so one deadline covers both phases and a body failure reaches the same retry decision as a header failure. Classification survives: the discriminant was MEASURED, not assumed — bytes that arrived and did not parse throw `SyntaxError` (`parse`), a destroyed socket throws `TypeError: terminated` (`network`), a fired deadline throws `AbortError` (`timeout`).
- **Retry decision, TAKEN not inherited: body failures are NOT given a retry branch.** Once the kind is honest `isRetryableError` already decides correctly — `timeout`/`network` retry, `parse` does not, because re-requesting an unparseable payload spends quota to fail identically. Cost of the choice, stated: `maxAttempts` is unchanged, so no ceiling moves; a body failure now consumes the EXISTING budget instead of being single-shot, worst case `maxAttempts - 1` extra billed calls — +2 at the seven 3-attempt sites, **+0 on every automatic and metered path**, all of which are `maxAttempts: 1` by construction (`cron/odds` `NO_RETRY`, both live-scores sites, `cron/game-stats`, `scores`, `teamRecords`). A future body-failure retry branch means the classification above it is wrong.
- `oddsRefreshExecutor` disposition, **third model and the one that shipped: it does NOT keep its own body read.** Two earlier models put the durable usage write between the headers and the body and then argued about where the deadline went; two review rounds prescribed OPPOSITE placements for that write, which is the PLATFORM-092 signal that the placement was never the variable. `captureOddsUsageSnapshot(headers, context)` takes Headers, not a Response — proven by mutation, not read — so the snapshot has NO ordering dependency on the body, and the "capture BEFORE parsing" comment encoded a guarantee (recorded even if parsing fails) that a post-fetch capture from stashed headers provides strictly better, since it also covers the abort and transport paths where the ordering version never ran. With the write out of the way the lane has no reason to own a body read: `fetchUpstreamConsuming` gives the shared loop both phases, and `startUpstreamBodyDeadline`, `bodyAbort`, `requestStartedAtMs` and the cross-attempt remainder arithmetic are DELETED rather than placed a third time (net **-44** production lines, measured `git diff --numstat` across the two files; a "-22" in the rework commit message was asserted rather than run, and is wrong). A body timeout or transport death is `provider-fetch-failed`; only bytes that arrived and were not JSON stay `odds-invalid-payload`.
- **Second retry decision, TAKEN and owner-accepted:** giving the shared loop the odds body makes an odds BODY failure retryable through `isRetryableError`. `api/odds` carries `maxAttempts: 3` against the 50-credit reserve, so a truncated odds body can now cost three billed calls where it previously cost one; `cron/odds` is `NO_RETRY` and unaffected. Accepted on the grounds that a truncated body is a transport fault worth one retry and the condition is rare, while a MALFORMED body stays `parse` and is still never retried. Recorded here as a decision because it is a billed-spend change, not a side effect of where the `await` landed.
- Consumers changed by the changed `kind`: **none BRANCH on it — one does, and it is inside the edited module.** `isRetryableError` (`fetchUpstream.ts:158`) is the only behavioural branch in the repo. Six sites read the kind into recorded evidence: `refreshAuthority:123` and `fullSeasonScheduleFetch:76` via `classifyUpstreamFault` (durable receipt label), `scores/route.ts:86` template-building `` `cfbd-${kind}` `` into `CfbdFallbackReason` — a DURABLE value that changes from `cfbd-parse` to `cfbd-timeout`/`cfbd-network` — plus two `logDebug` details and `safeDetailFromUpstream`. Client-facing HTTP status is unchanged everywhere: every site derives `details.status ?? 502` and neither kind carries a status.
- Scope: EXTENDED by owner ruling to close [#632](https://github.com/znpruitt/cfb-app/issues/632). The prompt's SCOPE excluded "the timeout VALUES at the ten call sites" on the premise the items were separable; this fix is what couples them. Under the old semantics a site's total ceiling was effectively unbounded (headers bounded, body free), so preserving 12s would have silently moved it to 12s against PLATFORM-115's measured 8.2 s / 16.0 s / 21.5 s completions. Ten CFBD sites moved to `CFBD_PEAK_LATENCY_TIMEOUT_MS` — one existing constant, no new value.
- **Both sets are ten and they are NOT the same ten**, enumerated rather than count-matched: `scripts/recover-game-stats.ts:862` is in this set and not #632's (whose stated limit was "counted by value across `src/`"); `oddsRefreshExecutor` is in #632's and not this one (different provider, "do not sweep it in on pattern-match alone"), keeping 12 s with the reason recorded — including that #662 narrowed that lane from unbounded-body to a 12 s total, deliberately and unproven. Spend does not rise: `maxAttempts` untouched, and per #632's measurement an aborted call bills as much as a completed one, so a slow body succeeding in one attempt costs 1 where 12 s would have aborted and retried for 3. Worst-case wall time becomes ~120.9 s at the five 3-attempt sites, replacing an UNBOUNDED worst case; `cron/schedule-refresh` declares no `maxDuration` and no envelope was added here, which would have been the new number the ruling excludes.
- Historical corrections: the prompt's "23 call sites across 12 files" is **14 in `src/` across 12 files, 15 with the script**; its base SHA `54fa6d7f` was not a stale `main` but the CODEX lane's head (`origin/main` was `87212f61`); and its stated central risk — "a changed `kind` is a behaviour change at every consumer that branches on it" — is inverted, since zero consumers branch. The receipt's own headline ("zero consumers branch on it") then understated its own table, which had already recorded `isRetryableError` as a branch. `docs/next-tasks.md:486` still reads "nine CFBD call sites" against the issue's corrected ten — planning's file, flagged not edited.
- **The test gap was the real second-round finding, not the arithmetic.** Every test written in the first two rounds used `maxAttempts: 1`, so a HIGH that only manifests once a SECOND attempt runs was invisible to a fully green suite: the body budget was computed from before the first attempt while `timeoutMs` is per attempt, leaving every recovered attempt at `Math.max(0, negative)` and aborting a complete 200 response as `provider-fetch-failed` (measured: `remainingMs=-34`). Manual `/api/odds` at `maxAttempts: 3` therefore could not recover from a header timeout — strictly worse than the defect being fixed. Both suites now carry multi-attempt coverage, and mutation M7 (attempt budget made cross-attempt) fires ONLY the multi-attempt test while all eight single-attempt tests stay green, which is the property worth holding.
- Review / verification: seven findings across both reviewers — three applied (all mine: the odds deadline armed after the durable write, reproduced at 507 ms accepted against a 300 ms deadline; a negative guard form inverted to positive; a VACUOUS timer assertion, `process._getActiveHandles()` reporting `[]` for an armed timer on Node 22, replaced with instrumented timers and a positive control), one escalated to the ruling above, three declined (one pre-existing undrained-body finding flagged; two sandbox-only command failures that pass here). Nine mutations, each read for WHICH assertion fired — **one of which (M9, headers stashed after the body read) caught a VACUOUS test of my own before it shipped**, an assertion that `getLatestKnownOddsUsage() !== undefined` against a server sending no usage headers, true whether or not the request recorded anything; rewritten to assert the captured snapshot is that response's own — incl. `parse` made retryable firing only on "must bill exactly ONE provider call, expected 1, actual 3", and the odds fresh-budget regression printing `saw [12000,12000,3000]`. `npm test` 5,245/5,245 exit 0 (+16 from 5,229); `tsc`/`lint:all` exit 0; clock-shift +400 d on both new suites 12/12. See L3, L4, L6.
- Status: open on `claude/662-upstream-body-deadline` (`7f24ca77`); PR and merge SHA recorded at merge, when #632 closes with it. One final review round is pre-committed as the last on this branch — if it finds anything further in the odds seam, the odds half comes out entirely and ships beside [#756](https://github.com/znpruitt/cfb-app/issues/756), with the shared-path fix and the ten timeout raises merging regardless (owner ruling, agreed in advance rather than discovered).
