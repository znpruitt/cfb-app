...
- Purpose: `?bypassSuppression=1` was gated only by `isAuthorizedForLeague`, which admits any caller on a passwordless league ([#627](https://github.com/znpruitt/cfb-app/issues/627), audit S1, prerequisite 6 of 6 in [#610](https://github.com/znpruitt/cfb-app/issues/610)).
- Scope: the route and its suites. Both generators' non-bypassable duplicate checks KEPT — not redundant.
- Outcome: **DECIDED — 401 when the parameter is present and the caller is not a platform admin**, from `requireAdminAuth(req)`, placed after the existing 404 blend-in so a passworded or unknown league stays indistinguishable. A caller not passing the parameter is served identically, admin or not.
- Evidence: **the issue's disclosure claim does not reproduce.** Both `shouldSuppressGenerator` entries carry a copy inside their own generator, so the two responses differ in `generatedAt` alone — measured through the route, positive controls firing. #627's body says a visitor can "receive withheld editorial output"; false against this code. The guard's disclosure value is prospective: a future engine-only rule, which AGENTS.md permits, would have been liftable by query string.
- Review / verification: RV6. Delta **+13, measured at both ends** (5,258 → 5,271). Three tests red against the parent on `200 !== 401`; only the generator-invocation counter catches moving the guard after the build.
- Status: (pending merge) from `claude/627-insights-bypass-authorization`.
- Open work: **the compute half is NOT closed** — `parseYear` has no upper bound, so anonymous `?year=<any>` still forces the uncached rebuild this item was filed for; no issue number yet. Also [#768](https://github.com/znpruitt/cfb-app/issues/768), [#769](https://github.com/znpruitt/cfb-app/issues/769), and the fail-open `ADMIN_API_TOKEN` residue.
- Owed to planning at merge (planning owns `AGENTS.md`; this lane did not edit it): Season Launch invariant 4's clause **"`bypassSuppression` is NOT admin-enforced today"** and its instruction **"do not write new guidance that assumes the flag is privileged until that lands"** both go false. Its `unstable_cache` sentence stays true.
