...
- A chip must be TRUE, name a CATEGORY rather than an instance, mean exactly what it says, and have
  ...
  information
- **Chips are capped at two.** **Amended 2026-09-08**, and recorded as an amendment: this line
  previously read _"Chips are not capped. As many as are true — several ranked matchups on one slate
  all carry the chip."_ **It was never true, which is a worse failure than becoming false and is the
  reason this note is long.** `TOP_BADGE_LIMIT = 2` was already capping in `gameTags.ts` by
  **2026-04-16** (`e4b117fb` moved it, so it is older); the uncapped line was written into this file
  on **2026-08-29** (`dc1b934a`) — **four months later, into a codebase that had been capping the
  whole time.** It did not stop being true; it was false on the day it was authored. **A canonical
  document that never matched reality is worse than one that drifted**, because nothing in its
  history marks a moment when it changed, so a reader has no reason to distrust it — the same shape
  as `tailwind.config.ts` asserting `darkMode: 'media'` while never being loaded (Item 159). The
  layout reason is real and independently supports the cap — the tag slot is `flex: none` and sits
  IN the status row, so at three tags the metadata absorbs the whole squeeze and ellipses to nothing
  — but it is why the cap is CORRECT, not why this line was wrong. Reason recorded in
  `item-87-followon-recap-scoreboard.md`; ruled by the owner on Item 165. **The selector must apply
  the cap, not the renderer** — a render-time truncation of a list the selector still builds in full
  is a different behaviour wearing the same number
