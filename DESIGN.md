...
- Game cards sit on a dark surface tint with a light border — discrete, bordered objects (see Containerization). Each team line currently renders its catalog colour as an 8px muted bar (~72% opacity) at the line start, on the shared scoreboard row and therefore across Overview, Matchups and Schedule. **Item 198 replaces that global mute with full-opacity colour normalised into a lightness band**, owner decision 2026-09-09. A team with no usable catalog colour renders NO accent — never the normaliser's fallback treatment. See [`docs/campaigns/item-87-followon-team-colour-regression.md`](docs/campaigns/item-87-followon-team-colour-regression.md) and [`docs/campaigns/item-87-followon-team-colour.md`](docs/campaigns/item-87-followon-team-colour.md) §A
