...
- A hue carries exactly one meaning **within a component family**; context scopes meaning *across*
...
- Compact scoreboard state variants share that row anatomy: scheduled uses a single-line kickoff /
  ...
  loser is muted, including when the home team won. Header rows never wrap and ellipsize when
  constrained so peer team lines remain aligned
